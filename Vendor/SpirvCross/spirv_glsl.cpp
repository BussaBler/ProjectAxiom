...
bool CompilerGLSL::location_is_framebuffer_fetch(uint32_t location) const {
    return std::find_if(begin(inout_color_attachments), end(inout_color_attachments),
                        [&](const std::pair<uint32_t, bool>& elem) { return elem.first == location; }) != end(inout_color_attachments);
}

bool CompilerGLSL::location_is_non_coherent_framebuffer_fetch(uint32_t location) const {
    return std::find_if(begin(inout_color_attachments), end(inout_color_attachments),
                        [&](const std::pair<uint32_t, bool>& elem) { return elem.first == location && !elem.second; }) != end(inout_color_attachments);
}
...
void CompilerGLSL::find_static_extensions() {
    ...
    case ExecutionModelCallableKHR:
        ...
        for (auto& cap : ir.declared_capabilities) {
            if (cap == CapabilityRayTracingKHR || cap == CapabilityRayQueryKHR || cap == CapabilityRayTraversalPrimitiveCullingKHR) {
...
void CompilerGLSL::ray_tracing_khr_fixup_locations() {
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
        ...
        if (var.storage != StorageClassRayPayloadKHR && var.storage != StorageClassCallableDataKHR && var.storage != StorageClassIncomingRayPayloadKHR &&
...
void CompilerGLSL::build_workgroup_size(SmallVector<string>& arguments, const SpecializationConstant& wg_x, const SpecializationConstant& wg_y,
...
void CompilerGLSL::emit_header() {
    ...
    bool interlock_used = execution.flags.get(ExecutionModePixelInterlockOrderedEXT) || execution.flags.get(ExecutionModePixelInterlockUnorderedEXT) ||
                          execution.flags.get(ExecutionModeSampleInterlockOrderedEXT) || execution.flags.get(ExecutionModeSampleInterlockUnorderedEXT);
    ...
    if (!options.vulkan_semantics) {
        ...
        for (uint32_t feature_index = 0; feature_index < Supp::FeatureCount; feature_index++) {
            ...
            for (auto& ext : exts) {
                ...
                statement(&ext != &exts.front() ? "#elif" : "#if", " defined(", name, ")", (*extra_predicate != '\0' ? " && " : ""), extra_predicate);
    ...
    case ExecutionModelMeshEXT: {
        if (execution.workgroup_size.constant != 0 || execution.flags.get(ExecutionModeLocalSizeId)) {
            ...
            if (options.vulkan_semantics || ((wg_x.id == ConstantID(0)) && (wg_y.id == ConstantID(0)) && (wg_z.id == ConstantID(0))))
...
bool CompilerGLSL::buffer_is_packing_standard(const SPIRType& type, BufferPackingStandard packing, uint32_t* failed_validation_index, uint32_t start_offset,
                                              uint32_t end_offset) {
    ...
    // We do not have to consider shared or packed since these layouts are not allowed in Vulkan SPIR-V (they are useless anyways, and custom offsets would do
    // the same thing).
    //
    ...
    bool is_top_level_block = has_decoration(type.self, DecorationBlock) || has_decoration(type.self, DecorationBufferBlock);
    ...
    for (uint32_t i = 0; i < type.member_types.size(); i++) {
        ...
        bool member_can_be_unsized = is_top_level_block && size_t(i + 1) == type.member_types.size() && !memb_type.array.empty();
...
string CompilerGLSL::buffer_to_packing_standard(const SPIRType& type, bool support_std430_without_scalar_layout, bool support_enhanced_layouts) {
    if (support_std430_without_scalar_layout && buffer_is_packing_standard(type, BufferPackingStd430))
        ...
        return "scalar";
    } else if (support_std430_without_scalar_layout && support_enhanced_layouts && buffer_is_packing_standard(type, BufferPackingStd430EnhancedLayout)) {
        ...
        set_extended_decoration(type.self, SPIRVCrossDecorationExplicitOffset);
        return "std140";
    } else if (options.vulkan_semantics && support_enhanced_layouts && buffer_is_packing_standard(type, BufferPackingScalarEnhancedLayout)) {
        set_extended_decoration(type.self, SPIRVCrossDecorationExplicitOffset);
        require_extension_internal("GL_EXT_scalar_block_layout");
        return "scalar";
    } else if (!support_std430_without_scalar_layout && options.vulkan_semantics && buffer_is_packing_standard(type, BufferPackingStd430)) {
...
void CompilerGLSL::emit_buffer_block_legacy(const SPIRVariable& var) {
    auto& type = get<SPIRType>(var.basetype);
    bool ssbo = var.storage == StorageClassStorageBuffer || ir.meta[type.self].decoration.decoration_flags.get(DecorationBufferBlock);
    if (ssbo)
...
void CompilerGLSL::emit_buffer_reference_block(uint32_t type_id, bool forward_declaration) {
    ...
    if (forward_declaration && is_physical_pointer_to_buffer_block(type)) {
        ...
        if (ir.meta[type.self].decoration.alias.empty() || block_ssbo_names.find(buffer_name) != end(block_ssbo_names) ||
...
void CompilerGLSL::emit_buffer_block_flattened(const SPIRVariable& var) {
    ...
    if (get_common_basic_type(type, basic_type)) {
        ...
        statement("uniform ", flags_to_qualifiers_glsl(tmp, 0, flags), type_to_glsl(tmp), " ", buffer_name, "[", buffer_size, "];");
    } else
...
const char* CompilerGLSL::to_storage_qualifiers_glsl(const SPIRVariable& var) {
    ...
    } else if (var.storage == StorageClassUniformConstant || var.storage == StorageClassUniform || var.storage == StorageClassPushConstant ||
...
void CompilerGLSL::emit_flattened_io_block_member(const std::string& basename, const SPIRType& type, const char* qual, const SmallVector<uint32_t>& indices) {
...
void CompilerGLSL::emit_flattened_io_block_struct(const std::string& basename, const SPIRType& type, const char* qual, const SmallVector<uint32_t>& indices) {
...
void CompilerGLSL::emit_interface_block(const SPIRVariable& var) {
    ...
    if (block) {
        ...
        if (options.force_flattened_io_blocks || (options.es && options.version < 310) || (!options.es && options.version < 150)) {
        ...
        // To support this, flatten the struct into separate varyings instead.
        if (type.basetype == SPIRType::Struct &&
            (options.force_flattened_io_blocks || (options.es && options.version < 310) || (!options.es && options.version < 150))) {
            emit_flattened_io_block(var, qual);
        } else {
            add_resource_name(var.self);
            ...
            if (type.storage == StorageClassInput && !type.array.empty() && !has_decoration(var.self, DecorationPatch) &&
                (get_entry_point().model == ExecutionModelTessellationControl || get_entry_point().model == ExecutionModelTessellationEvaluation)) {
                newtype.array.back() = 0;
                newtype.array_size_literal.back() = true;
            }

            statement(layout_for_variable(var), to_qualifiers_glsl(var.self), variable_decl(newtype, to_name(var.self), var.self), ";");
        }
    }
}
...
void CompilerGLSL::emit_constant(const SPIRConstant& constant) {
    ...
    bool is_workgroup_size_constant = ConstantID(constant.self) == wg_x.id || ConstantID(constant.self) == wg_y.id || ConstantID(constant.self) == wg_z.id;
    ...
    if (has_decoration(constant.self, DecorationSpecId)) {
        if (options.vulkan_semantics) {
            statement("layout(constant_id = ", get_decoration(constant.self, DecorationSpecId), ") const ", variable_decl(type, name), " = ",
...
static bool is_block_builtin(BuiltIn builtin) {
    return builtin == BuiltInPosition || builtin == BuiltInPointSize || builtin == BuiltInClipDistance || builtin == BuiltInCullDistance;
}
...
bool CompilerGLSL::should_force_emit_builtin_block(StorageClass storage) {
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
        ...
        if (var.storage == storage && block && is_builtin_variable(var)) {
            ...
            if (is_block_builtin(BuiltIn(get_decoration(type.self, DecorationBuiltIn))) && has_decoration(var.self, DecorationOffset)) {
                should_force = true;
            }
        }
    });
    ...
    if ((get_execution_model() == ExecutionModelTessellationControl || get_execution_model() == ExecutionModelMeshEXT) &&
...
void CompilerGLSL::fixup_implicit_builtin_block_names(ExecutionModel model) {
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
        auto& type = this->get<SPIRType>(var.basetype);
        bool block = has_decoration(type.self, DecorationBlock);
        if ((var.storage == StorageClassOutput || var.storage == StorageClassInput) && block && is_builtin_variable(var)) {
...
void CompilerGLSL::emit_declared_builtin_block(StorageClass storage, ExecutionModel model) {
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
        ...
        if (var.storage == storage && block && is_builtin_variable(var)) {
            ...
            if (storage == StorageClassOutput && has_decoration(var.self, DecorationXfbBuffer) && has_decoration(var.self, DecorationXfbStride)) {
            ...
            if (m.builtin && builtin_is_per_vertex_set(m.builtin_type)) {
                ...
                if (is_block_builtin(m.builtin_type) && m.decoration_flags.get(DecorationXfbStride) && m.decoration_flags.get(DecorationXfbBuffer) &&
                    m.decoration_flags.get(DecorationOffset)) {
                    have_any_xfb_offset = true;
                    builtin_xfb_offsets[m.builtin_type] = m.offset;
    ...
    global_builtins = Bitset(global_builtins.get_lower() &
                             ((1ull << BuiltInPosition) | (1ull << BuiltInPointSize) | (1ull << BuiltInClipDistance) | (1ull << BuiltInCullDistance)));
    ...
    bool builtin_array = model == ExecutionModelTessellationControl || (model == ExecutionModelMeshEXT && storage == StorageClassOutput) ||
...
void CompilerGLSL::emit_resources() {
        ...
        ir.for_each_typed_id<SPIRType>([&](uint32_t id, SPIRType& type) {
            if (is_physical_pointer(type)) {
                ...
                if (!is_physical_pointer_to_buffer_block(type)) {
                    ...
                    emit_type = std::find(physical_storage_non_block_pointer_types.begin(), physical_storage_non_block_pointer_types.end(), id) !=
            ...
            if (id.get_type() == TypeConstant) {
                ...
                if (needs_declaration) {
                    if (!options.vulkan_semantics && c.specialization) {
                        c.specialization_constant_macro_name = constant_value_macro_name(get_decoration(c.self, DecorationSpecId));
                    }
                    emit_constant(c);
                    emitted = true;
                }
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
        ...
        bool is_block_storage =
            type.storage == StorageClassStorageBuffer || type.storage == StorageClassUniform || type.storage == StorageClassShaderRecordBufferKHR;
        bool has_block_flags =
            ir.meta[type.self].decoration.decoration_flags.get(DecorationBlock) || ir.meta[type.self].decoration.decoration_flags.get(DecorationBufferBlock);

        if (var.storage != StorageClassFunction && type.pointer && is_block_storage && !is_hidden_variable(var) && has_block_flags) {
            emit_buffer_block(var);
        }
    });
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
        auto& type = this->get<SPIRType>(var.basetype);
        if (var.storage != StorageClassFunction && type.pointer && type.storage == StorageClassPushConstant && !is_hidden_variable(var)) {
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
        ...
        if (var.storage != StorageClassFunction && type.pointer &&
            (type.storage == StorageClassUniformConstant || type.storage == StorageClassAtomicCounter || type.storage == StorageClassRayPayloadKHR ||
             type.storage == StorageClassIncomingRayPayloadKHR || type.storage == StorageClassCallableDataKHR ||
             type.storage == StorageClassIncomingCallableDataKHR || type.storage == StorageClassHitAttributeKHR) &&
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
        ...
        if (var.storage == StorageClassOutput && !is_legacy() && location_is_framebuffer_fetch(get_decoration(var.self, DecorationLocation)) != 0) {
            is_hidden = false;
        }

        if (var.storage != StorageClassFunction && type.pointer && (var.storage == StorageClassInput || var.storage == StorageClassOutput) &&
            interface_variable_exists_in_entry_point(var.self) && !is_hidden) {
            if (options.es && get_execution_model() == ExecutionModelVertex && var.storage == StorageClassInput && type.array.size() == 1) {
            ...
            if (!options.vulkan_semantics) {
                if (!emitted_base_instance &&
                    ((options.vertex.support_nonzero_base_instance && builtin == BuiltInInstanceIndex) || (builtin == BuiltInBaseInstance))) {
    ...
    for (auto global : global_variables) {
        ...
        if (var.storage != StorageClassOutput) {
            if (!variable_is_lut(var)) {
                ...
                string initializer;
                if (options.force_zero_initialized_variables && var.storage == StorageClassPrivate && !var.initializer && !var.static_expression &&
...
void CompilerGLSL::emit_output_variable_initializer(const SPIRVariable& var) {
    ...
    if (is_block) {
        ...
        for (uint32_t i = 0; i < member_count; i++) {
            ...
            if (has_member_decoration(type.self, i, DecorationBuiltIn)) {
                if (get_member_decoration(type.self, i, DecorationBuiltIn) == BuiltInCullDistance && !cull_distance_count)
                    continue;

                if (get_member_decoration(type.self, i, DecorationBuiltIn) == BuiltInClipDistance && !clip_distance_count)
                    continue;
            }
            ...
            if (type_is_array) {
                ...
                statement("const ", type_to_glsl(array_type), " ", lut_name, type_to_array_glsl(array_type, 0), " = ", type_to_glsl_constructor(array_type),
                          "(", merge(exprs, ", "), ");");
            }
        ...
        statement("const ", type_to_glsl(type), " ", lut_name, type_to_array_glsl(type, 0), " = ", to_expression(var.initializer), ";");
        entry_func.fixup_hooks_in.push_back([&, lut_name]() { statement(to_expression(var.self), "[gl_InvocationID] = ", lut_name, "[gl_InvocationID];"); });
    } else if (has_decoration(var.self, DecorationBuiltIn) && BuiltIn(get_decoration(var.self, DecorationBuiltIn)) == BuiltInSampleMask) {
        ...
        entry_func.fixup_hooks_in.push_back([&] {
            ...
            for (uint32_t i = 0; i < num_constants; i++) {
                ...
                statement(to_expression(var.self), "[", i, "] = ", convert_to_string(this->get<SPIRConstant>(c.subconstants[i]).scalar_i32()), ";");
            }
        });
    } else {
        auto lut_name = join("_", var.self, "_init");
        statement("const ", type_to_glsl(type), " ", lut_name, type_to_array_glsl(type, var.self), " = ", to_expression(var.initializer), ";");
...
void CompilerGLSL::emit_extension_workarounds(ExecutionModel model) {
    ...
    if (!options.vulkan_semantics) {
            ...
            for (auto& e : exts) {
                ...
                case Supp::NV_shader_thread_shuffle:
                    for (const char* t : workaround_types) {
                        statement(t, " subgroupBroadcastFirst(", t, " value) { return shuffleNV(value, findLSB(ballotThreadNV(true)), gl_WarpSizeNV); }");
                    }
                    for (const char* t : workaround_types) {
                        statement(t, " subgroupBroadcast(", t, " value, uint id) { return shuffleNV(value, id, gl_WarpSizeNV); }");
                    }
                    break;
                case Supp::ARB_shader_ballot:
                    for (const char* t : workaround_types) {
                        statement(t, " subgroupBroadcastFirst(", t, " value) { return readFirstInvocationARB(value); }");
                    }
                    for (const char* t : workaround_types) {
                        statement(t, " subgroupBroadcast(", t, " value, uint id) { return readInvocationARB(value, id); }");
                    }
                    break;
                default:
                    break;
        ...
        if (shader_subgroup_supporter.is_feature_requested(Supp::SubgroupAllEqualT)) {
            ...
            statement("#define _SPIRV_CROSS_SUBGROUP_ALL_EQUAL_WORKAROUND(type) bool subgroupAllEqual(type value) { return "
        ...
        arithmetic_feature_helper(Supp::SubgroupArithmeticIAddReduce, "subgroupAdd", OpGroupNonUniformIAdd, GroupOperationReduce);
        arithmetic_feature_helper(Supp::SubgroupArithmeticIAddExclusiveScan, "subgroupExclusiveAdd", OpGroupNonUniformIAdd, GroupOperationExclusiveScan);
        arithmetic_feature_helper(Supp::SubgroupArithmeticIAddInclusiveScan, "subgroupInclusiveAdd", OpGroupNonUniformIAdd, GroupOperationInclusiveScan);
        arithmetic_feature_helper(Supp::SubgroupArithmeticFAddReduce, "subgroupAdd", OpGroupNonUniformFAdd, GroupOperationReduce);
        arithmetic_feature_helper(Supp::SubgroupArithmeticFAddExclusiveScan, "subgroupExclusiveAdd", OpGroupNonUniformFAdd, GroupOperationExclusiveScan);
        arithmetic_feature_helper(Supp::SubgroupArithmeticFAddInclusiveScan, "subgroupInclusiveAdd", OpGroupNonUniformFAdd, GroupOperationInclusiveScan);

        arithmetic_feature_helper(Supp::SubgroupArithmeticIMulReduce, "subgroupMul", OpGroupNonUniformIMul, GroupOperationReduce);
        arithmetic_feature_helper(Supp::SubgroupArithmeticIMulExclusiveScan, "subgroupExclusiveMul", OpGroupNonUniformIMul, GroupOperationExclusiveScan);
        arithmetic_feature_helper(Supp::SubgroupArithmeticIMulInclusiveScan, "subgroupInclusiveMul", OpGroupNonUniformIMul, GroupOperationInclusiveScan);
        arithmetic_feature_helper(Supp::SubgroupArithmeticFMulReduce, "subgroupMul", OpGroupNonUniformFMul, GroupOperationReduce);
        arithmetic_feature_helper(Supp::SubgroupArithmeticFMulExclusiveScan, "subgroupExclusiveMul", OpGroupNonUniformFMul, GroupOperationExclusiveScan);
        arithmetic_feature_helper(Supp::SubgroupArithmeticFMulInclusiveScan, "subgroupInclusiveMul", OpGroupNonUniformFMul, GroupOperationInclusiveScan);
    }
...
void CompilerGLSL::emit_polyfills(uint32_t polyfills, bool relaxed) {
    ...
    if (polyfills & PolyfillDeterminant4x4) {
        ...
        statement("return dot(m[0], vec4("
                  "m[2][1] * m[3][2] * m[1][3] - m[3][1] * m[2][2] * m[1][3] + m[3][1] * m[1][2] * m[2][3] - m[1][1] * m[3][2] * m[2][3] - m[2][1] * m[1][2] * "
                  "m[3][3] + m[1][1] * m[2][2] * m[3][3], "
                  "m[3][0] * m[2][2] * m[1][3] - m[2][0] * m[3][2] * m[1][3] - m[3][0] * m[1][2] * m[2][3] + m[1][0] * m[3][2] * m[2][3] + m[2][0] * m[1][2] * "
                  "m[3][3] - m[1][0] * m[2][2] * m[3][3], "
                  "m[2][0] * m[3][1] * m[1][3] - m[3][0] * m[2][1] * m[1][3] + m[3][0] * m[1][1] * m[2][3] - m[1][0] * m[3][1] * m[2][3] - m[2][0] * m[1][1] * "
                  "m[3][3] + m[1][0] * m[2][1] * m[3][3], "
                  "m[3][0] * m[2][1] * m[1][2] - m[2][0] * m[3][1] * m[1][2] - m[3][0] * m[1][1] * m[2][2] + m[1][0] * m[3][1] * m[2][2] + m[2][0] * m[1][1] * "
                  "m[3][2] - m[1][0] * m[2][1] * m[3][2]));");
        end_scope();
        statement("");
    }
    ...
    if (polyfills & PolyfillMatrixInverse4x4) {
        ...
        statement(qual, "vec4 t = vec4("
                        "m[2][1] * m[3][2] * m[1][3] - m[3][1] * m[2][2] * m[1][3] + m[3][1] * m[1][2] * m[2][3] - m[1][1] * m[3][2] * m[2][3] - m[2][1] * "
                        "m[1][2] * m[3][3] + m[1][1] * m[2][2] * m[3][3], "
                        "m[3][0] * m[2][2] * m[1][3] - m[2][0] * m[3][2] * m[1][3] - m[3][0] * m[1][2] * m[2][3] + m[1][0] * m[3][2] * m[2][3] + m[2][0] * "
                        "m[1][2] * m[3][3] - m[1][0] * m[2][2] * m[3][3], "
                        "m[2][0] * m[3][1] * m[1][3] - m[3][0] * m[2][1] * m[1][3] + m[3][0] * m[1][1] * m[2][3] - m[1][0] * m[3][1] * m[2][3] - m[2][0] * "
                        "m[1][1] * m[3][3] + m[1][0] * m[2][1] * m[3][3], "
                        "m[3][0] * m[2][1] * m[1][2] - m[2][0] * m[3][1] * m[1][2] - m[3][0] * m[1][1] * m[2][2] + m[1][0] * m[3][1] * m[2][2] + m[2][0] * "
                        "m[1][1] * m[3][2] - m[1][0] * m[2][1] * m[3][2]);");
        statement("return mat4("
                  "t[0], "
                  "m[3][1] * m[2][2] * m[0][3] - m[2][1] * m[3][2] * m[0][3] - m[3][1] * m[0][2] * m[2][3] + m[0][1] * m[3][2] * m[2][3] + m[2][1] * m[0][2] * "
                  "m[3][3] - m[0][1] * m[2][2] * m[3][3], "
                  "m[1][1] * m[3][2] * m[0][3] - m[3][1] * m[1][2] * m[0][3] + m[3][1] * m[0][2] * m[1][3] - m[0][1] * m[3][2] * m[1][3] - m[1][1] * m[0][2] * "
                  "m[3][3] + m[0][1] * m[1][2] * m[3][3], "
                  "m[2][1] * m[1][2] * m[0][3] - m[1][1] * m[2][2] * m[0][3] - m[2][1] * m[0][2] * m[1][3] + m[0][1] * m[2][2] * m[1][3] + m[1][1] * m[0][2] * "
                  "m[2][3] - m[0][1] * m[1][2] * m[2][3], "
                  "t[1], "
                  "m[2][0] * m[3][2] * m[0][3] - m[3][0] * m[2][2] * m[0][3] + m[3][0] * m[0][2] * m[2][3] - m[0][0] * m[3][2] * m[2][3] - m[2][0] * m[0][2] * "
                  "m[3][3] + m[0][0] * m[2][2] * m[3][3], "
                  "m[3][0] * m[1][2] * m[0][3] - m[1][0] * m[3][2] * m[0][3] - m[3][0] * m[0][2] * m[1][3] + m[0][0] * m[3][2] * m[1][3] + m[1][0] * m[0][2] * "
                  "m[3][3] - m[0][0] * m[1][2] * m[3][3], "
                  "m[1][0] * m[2][2] * m[0][3] - m[2][0] * m[1][2] * m[0][3] + m[2][0] * m[0][2] * m[1][3] - m[0][0] * m[2][2] * m[1][3] - m[1][0] * m[0][2] * "
                  "m[2][3] + m[0][0] * m[1][2] * m[2][3], "
                  "t[2], "
                  "m[3][0] * m[2][1] * m[0][3] - m[2][0] * m[3][1] * m[0][3] - m[3][0] * m[0][1] * m[2][3] + m[0][0] * m[3][1] * m[2][3] + m[2][0] * m[0][1] * "
                  "m[3][3] - m[0][0] * m[2][1] * m[3][3], "
                  "m[1][0] * m[3][1] * m[0][3] - m[3][0] * m[1][1] * m[0][3] + m[3][0] * m[0][1] * m[1][3] - m[0][0] * m[3][1] * m[1][3] - m[1][0] * m[0][1] * "
                  "m[3][3] + m[0][0] * m[1][1] * m[3][3], "
                  "m[2][0] * m[1][1] * m[0][3] - m[1][0] * m[2][1] * m[0][3] - m[2][0] * m[0][1] * m[1][3] + m[0][0] * m[2][1] * m[1][3] + m[1][0] * m[0][1] * "
                  "m[2][3] - m[0][0] * m[1][1] * m[2][3], "
                  "t[3], "
                  "m[2][0] * m[3][1] * m[0][2] - m[3][0] * m[2][1] * m[0][2] + m[3][0] * m[0][1] * m[2][2] - m[0][0] * m[3][1] * m[2][2] - m[2][0] * m[0][1] * "
                  "m[3][2] + m[0][0] * m[2][1] * m[3][2], "
                  "m[3][0] * m[1][1] * m[0][2] - m[1][0] * m[3][1] * m[0][2] - m[3][0] * m[0][1] * m[1][2] + m[0][0] * m[3][1] * m[1][2] + m[1][0] * m[0][1] * "
                  "m[3][2] - m[0][0] * m[1][1] * m[3][2], "
                  "m[1][0] * m[2][1] * m[0][2] - m[2][0] * m[1][1] * m[0][2] + m[2][0] * m[0][1] * m[1][2] - m[0][0] * m[2][1] * m[1][2] - m[1][0] * m[0][1] * "
                  "m[2][2] + m[0][0] * m[1][1] * m[2][2]) "
                  "* (1.0 / dot(m[0], t));");
        end_scope();
        statement("");
    }
        ...
        for (uint32_t i = 0; i < 3; i++) {
            for (uint32_t j = 0; j < 3; j++) {
                ...
                for (uint32_t k = 0; k < 4; k++) {
                    ...
                    if (i < 2) {
                        statement("spirv_instruction(set = \"GLSL.std.450\", id = ", glsl_ops[i], ") ", type, " ", spv_ops[i], "(", type, ", ", type, ");");
                    } else {
                        statement("spirv_instruction(set = \"GLSL.std.450\", id = ", glsl_ops[i], ") ", type, " ", spv_ops[i], "(", type, ", ", type, ", ",
                                  type, ");");
                    }
        ...
        for (uint32_t i = 0; i < 3; i++) {
            for (uint32_t j = 0; j < 3; j++) {
                ...
                for (uint32_t k = 0; k < 4; k++) {
                    ...
                    if (i < 2) {
                        statement("mediump ", type, " ", spv_ops[i], "Relaxed(", "mediump ", type, " a, mediump ", type, " b)");
                        begin_scope();
                        ...
                        statement("mediump ", type, " ", spv_ops[i], "Relaxed(", "mediump ", type, " a, mediump ", type, " b, mediump ", type, " c)");
                        begin_scope();
...
string CompilerGLSL::to_unpacked_row_major_matrix_expression(uint32_t id) {
    return unpack_expression_type(to_expression(id), expression_type(id), get_extended_decoration(id, SPIRVCrossDecorationPhysicalTypeID),
...
string CompilerGLSL::to_unpacked_expression(uint32_t id, bool register_expression_read) {
    ...
    if (!need_transpose && (is_remapped || is_packed)) {
        return unpack_expression_type(to_expression(id, register_expression_read), get_pointee_type(expression_type_id(id)),
...
string CompilerGLSL::to_extract_constant_composite_expression(uint32_t result_type, const SPIRConstant& c, const uint32_t* chain, uint32_t length) {
...
string CompilerGLSL::to_rerolled_array_expression(const SPIRType& parent_type, const string& base_expr, const SPIRType& type) {
    bool remapped_boolean =
        parent_type.basetype == SPIRType::Struct && type.basetype == SPIRType::Boolean && backend.boolean_in_struct_remapped_type != SPIRType::Boolean;
...
string CompilerGLSL::to_composite_constructor_expression(const SPIRType& parent_type, uint32_t id, bool block_like_type) {
    ...
    if (is_array(type)) {
        reroll_array = !backend.array_is_value_type || (block_like_type && !backend.array_is_value_type_in_buffer_blocks);
...
string CompilerGLSL::to_expression(uint32_t id, bool register_expression_read) {
    ...
    case TypeExpression: {
        ...
        if (e.base_expression)
            ...
            return convert_row_major_matrix(e.expression, get<SPIRType>(e.expression_type), physical_type_id, is_packed, relaxed);
...
void CompilerGLSL::fill_composite_constant(SPIRConstant& constant, TypeID type_id, const SmallVector<ConstantID>& initializers) {
    auto& type = get<SPIRType>(type_id);
...
void CompilerGLSL::set_composite_constant(ConstantID const_id, TypeID type_id, const SmallVector<ConstantID>& initializers) {
...
#define GLSL_UOP(opname, x)                                                                                                                                    \
    ...
    case OpSRem: {
        ...
        return join(to_enclosed_expression(op0), " - ", to_enclosed_expression(op1), " * ", "(", to_enclosed_expression(op0), " / ",
    ...
    case OpCompositeExtract: {
        ...
        string expr;
        if (c && cop.arguments.size() == 2 && c->is_used_as_array_length && !backend.supports_spec_constant_array_size &&
            is_vector(get<SPIRType>(c->constant_type))) {
            expr = to_expression(c->specialization_constant_id(0, cop.arguments[1]));
        } else {
            expr = access_chain_internal(cop.arguments[0], &cop.arguments[1], uint32_t(cop.arguments.size() - 1), ACCESS_CHAIN_INDEX_IS_LITERAL_BIT, nullptr);
        }
        return expr;
    }
...
#undef GLSL_UOP
    if (binary) {
        ...
        string cast_op0;
        string cast_op1;
        auto expected_type = binary_op_bitcast_helper(cast_op0, cast_op1, input_type, cop.arguments[0], cop.arguments[1], skip_cast_if_equal_type);
...
string CompilerGLSL::constant_expression(const SPIRConstant& c, bool inside_block_like_struct_scope, bool inside_struct_scope) {
    auto& type = get<SPIRType>(c.constant_type);
    ...
    if (is_pointer(type)) {
        ...
        bool array_type_decays = inside_block_like_struct_scope && is_array(type) && !backend.array_is_value_type_in_buffer_blocks;

        // Allow Metal to use the array<T> template to make arrays a value type
        bool needs_trailing_tracket = false;
        if (backend.use_initializer_list && backend.use_typed_initializer_list && type.basetype == SPIRType::Struct && !is_array(type)) {
            res = type_to_glsl_constructor(type) + "{ ";
        } else if (backend.use_initializer_list && backend.use_typed_initializer_list && backend.array_is_value_type && is_array(type) && !array_type_decays) {
            const auto* p_type = &type;
            SPIRType tmp_type{OpNop};

            if (inside_struct_scope && backend.boolean_in_struct_remapped_type != SPIRType::Boolean && type.basetype == SPIRType::Boolean) {
                tmp_type = type;
                tmp_type.basetype = backend.boolean_in_struct_remapped_type;
                p_type = &tmp_type;
            }
            ...
            if (auto* op = maybe_get<SPIRConstantOp>(elem)) {
                ...
                if (subc.specialization && !expression_is_forwarded(elem))
                    ...
                    if (!is_array(type) && type.basetype == SPIRType::Struct) {
                        ...
                        inside_block_like_struct_scope = has_member_decoration(type.self, subconstant_index, DecorationOffset);
                    }
...
#ifdef _WIN32
            sprintf(print_buffer, "0x%llx%s", static_cast<unsigned long long>(u64_value), backend.long_long_literal_suffix ? "ull" : "ul");
#else
...
void CompilerGLSL::emit_uninitialized_temporary(uint32_t result_type, uint32_t result_id) {
    ...
    if (!block_temporary_hoisting && current_continue_block && !hoisted_temporaries.count(result_id)) {
        ...
        if (find_if(begin(header.declare_temporary), end(header.declare_temporary), [result_type, result_id](const pair<uint32_t, uint32_t>& tmp) {
...
SPIRExpression& CompilerGLSL::emit_op(uint32_t result_type, uint32_t result_id, const string& rhs, bool forwarding, bool suppress_usage_tracking) {
...
void CompilerGLSL::emit_mesh_tasks(SPIRBlock& block) {
    statement("EmitMeshTasksEXT(", to_unpacked_expression(block.mesh.groups[0]), ", ", to_unpacked_expression(block.mesh.groups[1]), ", ",
...
void CompilerGLSL::emit_binary_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op) {
    ...
    bool force_temporary_precise =
        backend.support_precise_qualifier && has_legacy_nocontract(result_type, result_id) && type_is_floating_point(get<SPIRType>(result_type));
    bool forward = should_forward(op0) && should_forward(op1) && !force_temporary_precise;

    emit_op(result_type, result_id, join(to_enclosed_unpacked_expression(op0), " ", op, " ", to_enclosed_unpacked_expression(op1)), forward);
...
void CompilerGLSL::emit_unrolled_binary_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op, bool negate,
...
SPIRType CompilerGLSL::binary_op_bitcast_helper(string& cast_op0, string& cast_op1, SPIRType::BaseType& input_type, uint32_t op0, uint32_t op1,
...
void CompilerGLSL::emit_binary_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op, SPIRType::BaseType input_type,
                                       bool skip_cast_if_equal_type, bool implicit_integer_promotion) {
...
void CompilerGLSL::emit_binary_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op) {
    ...
    emit_op(result_type, result_id, join(op, "(", to_unpacked_expression(op0), ", ", to_unpacked_expression(op1), ")"), forward);
    inherit_expression_dependencies(result_id, op0);
    inherit_expression_dependencies(result_id, op1);
}

void CompilerGLSL::emit_atomic_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op) {
    auto& type = get<SPIRType>(result_type);
    if (type_is_floating_point(type)) {
        if (!options.vulkan_semantics)
    ...
    emit_op(result_type, result_id, join(op, "(", to_atomic_ptr_expression(op0), ", ", to_unpacked_expression(op1), ")"), false);
    flush_all_atomic_capable_variables();
}

void CompilerGLSL::emit_atomic_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2, const char* op) {
    forced_temporaries.insert(result_id);
    emit_op(result_type, result_id,
            join(op, "(", to_non_uniform_aware_expression(op0), ", ", to_unpacked_expression(op1), ", ", to_unpacked_expression(op2), ")"), false);
    flush_all_atomic_capable_variables();
}

void CompilerGLSL::emit_unary_func_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, const char* op, SPIRType::BaseType input_type,
...
void CompilerGLSL::emit_trinary_func_op_bitextract(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2, const char* op,
                                                   SPIRType::BaseType expected_result_type, SPIRType::BaseType input_type0, SPIRType::BaseType input_type1,
                                                   SPIRType::BaseType input_type2) {
    auto& out_type = get<SPIRType>(result_type);
    auto expected_type = out_type;
    expected_type.basetype = input_type0;

    string cast_op0 = expression_type(op0).basetype != input_type0 ? bitcast_glsl(expected_type, op0) : to_unpacked_expression(op0);
    ...
    string cast_op1 = expression_type(op1).basetype != input_type1 ? join(type_to_glsl_constructor(expected_type), "(", op1_expr, ")") : op1_expr;

    expected_type.basetype = input_type2;
    expected_type.vecsize = 1;
    string cast_op2 = expression_type(op2).basetype != input_type2 ? join(type_to_glsl_constructor(expected_type), "(", op2_expr, ")") : op2_expr;
...
void CompilerGLSL::emit_trinary_func_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2, const char* op,
                                             SPIRType::BaseType input_type) {
    auto& out_type = get<SPIRType>(result_type);
    auto expected_type = out_type;
    expected_type.basetype = input_type;
    string cast_op0 = expression_type(op0).basetype != input_type ? bitcast_glsl(expected_type, op0) : to_unpacked_expression(op0);
    string cast_op1 = expression_type(op1).basetype != input_type ? bitcast_glsl(expected_type, op1) : to_unpacked_expression(op1);
    string cast_op2 = expression_type(op2).basetype != input_type ? bitcast_glsl(expected_type, op2) : to_unpacked_expression(op2);
...
void CompilerGLSL::emit_binary_func_op_cast_clustered(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op,
                                                      SPIRType::BaseType input_type) {
...
void CompilerGLSL::emit_binary_func_op_cast(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op, SPIRType::BaseType input_type,
                                            bool skip_cast_if_equal_type) {
    string cast_op0, cast_op1;
    auto expected_type = binary_op_bitcast_helper(cast_op0, cast_op1, input_type, op0, op1, skip_cast_if_equal_type);
    auto& out_type = get<SPIRType>(result_type);
...
void CompilerGLSL::emit_trinary_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2, const char* op) {
    ...
    emit_op(result_type, result_id, join(op, "(", to_unpacked_expression(op0), ", ", to_unpacked_expression(op1), ", ", to_unpacked_expression(op2), ")"),
            forward);
...
void CompilerGLSL::emit_quaternary_func_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2, uint32_t op3, const char* op) {
    ...
    emit_op(result_type, result_id,
            join(op, "(", to_unpacked_expression(op0), ", ", to_unpacked_expression(op1), ", ", to_unpacked_expression(op2), ", ", to_unpacked_expression(op3),
                 ")"),
            forward);
...
void CompilerGLSL::emit_bitfield_insert_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, uint32_t op2, uint32_t op3, const char* op,
    ...
    emit_op(result_type, result_id, join(op, "(", op0_expr, ", ", op1_expr, ", ", op2_expr, ", ", op3_expr, ")"), forward);
...
string CompilerGLSL::legacy_tex_op(const std::string& op, const SPIRType& imgtype, uint32_t tex) {
    ...
    auto& execution = get_entry_point();
    if (op == "textureGrad" || op == "textureProjGrad" || ((op == "textureLod" || op == "textureProjLod") && execution.model != ExecutionModelVertex)) {
        if (is_legacy_es()) {
            legacy_lod_ext = true;
...
string CompilerGLSL::to_ternary_expression(const SPIRType& restype, uint32_t select, uint32_t true_value, uint32_t false_value) {
    string expr;
    auto& lerptype = expression_type(select);

    if (lerptype.vecsize == 1)
        expr = join(to_enclosed_expression(select), " ? ", to_enclosed_pointer_expression(true_value), " : ", to_enclosed_pointer_expression(false_value));
    else {
...
void CompilerGLSL::emit_mix_op(uint32_t result_type, uint32_t id, uint32_t left, uint32_t right, uint32_t lerp) {
    ...
    string mix_op;
    bool has_boolean_mix = *backend.boolean_mix_function && ((options.es && options.version >= 310) || (!options.es && options.version >= 450));
...
string CompilerGLSL::to_combined_image_sampler(VariableID image_id, VariableID samp_id) {
    ...
    auto image_itr = find_if(begin(args), end(args), [image_id](const SPIRFunction::Parameter& param) { return image_id == param.id; });

    auto sampler_itr = find_if(begin(args), end(args), [samp_id](const SPIRFunction::Parameter& param) { return samp_id == param.id; });
    ...
    if (image_itr != end(args) || sampler_itr != end(args)) {
        ...
        auto itr = find_if(begin(combined), end(combined), [=](const SPIRFunction::CombinedImageSamplerParameter& p) {
            return p.global_image == global_image && p.global_sampler == global_sampler && p.image_id == iid && p.sampler_id == sid;
        });
...
bool CompilerGLSL::is_supported_subgroup_op_in_opengl(Op op, const uint32_t* ops) {
    ...
    case OpGroupNonUniformFMul: {
        ...
        if (operation == GroupOperationReduce || operation == GroupOperationInclusiveScan || operation == GroupOperationExclusiveScan) {
            return true;
        } else {
            return false;
        }
...
void CompilerGLSL::emit_sampled_image_op(uint32_t result_type, uint32_t result_id, uint32_t image_id, uint32_t samp_id) {
    if (options.vulkan_semantics && combined_image_samplers.empty()) {
        emit_binary_func_op(result_type, result_id, image_id, samp_id, type_to_glsl(get<SPIRType>(result_type), result_id).c_str());
    } else {
...
void CompilerGLSL::emit_sparse_feedback_temporaries(uint32_t result_type_id, uint32_t id, uint32_t& feedback_id, uint32_t& texel_id) {
...
std::string CompilerGLSL::to_texture_op(const Instruction& i, bool sparse, bool* forward, SmallVector<uint32_t>& inherited_expressions) {
    auto* ops = stream(i);
    auto op = static_cast<Op>(i.op);
...
string CompilerGLSL::to_function_name(const TextureFunctionNameArguments& args) {
    ...
    if (((imgtype.image.arrayed && imgtype.image.dim == Dim2D) || imgtype.image.dim == DimCube) && is_depth_image(imgtype, tex) && args.lod &&
        !args.base.is_fetch) {
        ...
        if (type.basetype == SPIRType::Image && type.image.sampled == 1 && type.image.dim != DimBuffer) {
            if (options.vulkan_semantics) {
                if (dummy_sampler_id) {
                    ...
                    return join(type_to_glsl(sampled_type), "(", to_non_uniform_aware_expression(id), ", ", to_expression(dummy_sampler_id), ")");
                } else {
...
string CompilerGLSL::to_function_args(const TextureFunctionArguments& args, bool* p_forward) {
    ...
    // The workaround will assert that the LOD is in fact constant 0, or we cannot emit correct code.
    // This happens for HLSL SampleCmpLevelZero on Texture2DArray and TextureCube.
    bool workaround_lod_array_shadow_as_grad = ((imgtype.image.arrayed && imgtype.image.dim == Dim2D) || imgtype.image.dim == DimCube) &&
        ...
        if (imgtype.image.dim == Dim1D && options.es) {
            ...
            if (type_is_floating_point(coord_type)) {
                ...
                if (imgtype.image.arrayed || args.base.is_proj) {
                    coord_expr = join("vec3(", enclose_expression(coord_expr), ".x, 0.0, ", enclose_expression(coord_expr), ".y)");
                } else
                    coord_expr = join("vec2(", coord_expr, ", 0.0)");
            } else {
                if (imgtype.image.arrayed) {
                    coord_expr = join("ivec3(", enclose_expression(coord_expr), ".x, 0, ", enclose_expression(coord_expr), ".y)");
                } else
...
void CompilerGLSL::emit_glsl_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t* args, uint32_t length) {
    ...
    case GLSLstd450Fma:
        if ((!options.es && options.version < 400) || (options.es && options.version < 320)) {
            auto expr = join(to_enclosed_expression(args[0]), " * ", to_enclosed_expression(args[1]), " + ", to_enclosed_expression(args[2]));

            emit_op(result_type, id, expr, should_forward(args[0]) && should_forward(args[1]) && should_forward(args[2]));
            for (uint32_t i = 0; i < 3; i++)
    ...
    case GLSLstd450Modf:
        ...
        if (!is_legacy()) {
            ...
            via_type.basetype = SPIRType::Int;
            statement(to_expression(args[1]), " = ", type_to_glsl(op1_type), "(", type_to_glsl(via_type), "(", to_expression(args[0]), "));");
    ...
    case GLSLstd450ModfStruct: {
        ...
        if (!is_legacy()) {
            statement(to_expression(id), ".", to_member_name(type, 0), " = ", "modf(", to_expression(args[0]), ", ", to_expression(id), ".",
                      to_member_name(type, 1), ");");
        } else {
            // NB. legacy GLSL doesn't have trunc() either, so we do a value cast
            auto& op0_type = expression_type(args[0]);
            auto via_type = op0_type;
            via_type.basetype = SPIRType::Int;
            statement(to_expression(id), ".", to_member_name(type, 1), " = ", type_to_glsl(op0_type), "(", type_to_glsl(via_type), "(", to_expression(args[0]),
                      "));");
            statement(to_expression(id), ".", to_member_name(type, 0), " = ", to_enclosed_expression(args[0]), " - ", to_expression(id), ".",
                      to_member_name(type, 1), ";");
        }
        break;
    }
    ...
    case GLSLstd450Determinant: {
        ...
        if (options.version < 150) // also matches ES 100
        {
            ...
            emit_unary_func_op(result_type, id, args[0], (options.es && relaxed) ? "spvDeterminantMP" : "spvDeterminant");
        } else
    ...
    case GLSLstd450FrexpStruct: {
        auto& type = get<SPIRType>(result_type);
        emit_uninitialized_temporary_expression(result_type, id);
        statement(to_expression(id), ".", to_member_name(type, 0), " = ", "frexp(", to_expression(args[0]), ", ", to_expression(id), ".",
                  to_member_name(type, 1), ");");
        break;
    }
...
void CompilerGLSL::emit_emulated_ahyper_op(uint32_t result_type, uint32_t id, uint32_t op0, GLSLstd450 op) {
    ...
    switch (op) {
    case GLSLstd450Asinh:
        expr = join("log(", to_enclosed_expression(op0), " + sqrt(", to_enclosed_expression(op0), " * ", to_enclosed_expression(op0), " + ", one, "))");
        emit_op(result_type, id, expr, forward);
        break;
...
void CompilerGLSL::emit_spv_amd_shader_ballot_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t* args, uint32_t) {
    ...
    enum AMDShaderBallot { SwizzleInvocationsAMD = 1, SwizzleInvocationsMaskedAMD = 2, WriteInvocationAMD = 3, MbcntAMD = 4 };
...
void CompilerGLSL::emit_spv_amd_shader_explicit_vertex_parameter_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t* args, uint32_t) {
    ...
    enum AMDShaderExplicitVertexParameter { InterpolateAtVertexAMD = 1 };
...
void CompilerGLSL::emit_spv_amd_gcn_shader_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t* args, uint32_t) {
    ...
    enum AMDGCNShader { CubeFaceIndexAMD = 1, CubeFaceCoordAMD = 2, TimeAMD = 3 };
...
#undef GLSL_GROUP_OP
    ...
    case OpGroupNonUniformLogicalXor: {
        ...
        if (operation == GroupOperationClusteredReduce) {
            require_extension_internal("GL_KHR_shader_subgroup_clustered");
        } else if (operation == GroupOperationExclusiveScan || operation == GroupOperationInclusiveScan || operation == GroupOperationReduce) {
...
std::string CompilerGLSL::bitcast_expression(const SPIRType& target_type, SPIRType::BaseType expr_type, const std::string& expr) {
...
void CompilerGLSL::access_chain_internal_append_index(std::string& expr, uint32_t /*base*/, const SPIRType* /*type*/, AccessChainFlags flags,
...
string CompilerGLSL::access_chain_internal(uint32_t base, const uint32_t* indices, uint32_t count, AccessChainFlags flags, AccessChainMeta* meta) {
    string expr;
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (ptr_chain && i == 0) {
            ...
            if (options.flatten_multidimensional_arrays && dimension_flatten) {
                ...
                if (flags & ACCESS_CHAIN_PTR_CHAIN_POINTER_ARITH_BIT) {
                    ...
                    intptr_expr += join(" + ", to_enclosed_unpacked_expression(index), " * ", get_decoration(ptr_type_id, DecorationArrayStride));
        ...
        // Arrays and OpTypeCooperativeVectorNV (aka fancy arrays)
            ...
            if (backend.force_gl_in_out_block && i == 0 && var && is_builtin_variable(*var) && !has_decoration(type->self, DecorationBlock)) {
            ...
            } else if (backend.force_merged_mesh_block && i == 0 && var && !is_builtin_variable(*var) && var->storage == StorageClassOutput) {
            ...
            if (var && has_decoration(var->self, DecorationBuiltIn) && get_decoration(var->self, DecorationBuiltIn) == BuiltInPosition &&
                get_execution_model() == ExecutionModelMeshEXT) {
                access_meshlet_position_y = true;
            }

            if (get<SPIRType>(type->parent_type).op == OpTypeStruct && has_decoration(type->parent_type, DecorationArrayStride)) {
        ...
        // Vector -> Scalar
            ...
            if (row_major_matrix_needs_conversion) {
                ...
                if (column_index != string::npos) {
                    ...
                    if (end_deferred_index != string::npos && end_deferred_index + 1 != deferred_index.size()) {
                        ...
                        end_deferred_index++;
                        deferred_index = deferred_index.substr(end_deferred_index) + deferred_index.substr(0, end_deferred_index);
                    }
            ...
            if ((flags & ACCESS_CHAIN_FORCE_COMPOSITE_BIT) == 0) {
                ...
                if (var && var->storage == StorageClassOutput && get_execution_model() == ExecutionModelTessellationControl &&
            ...
            if (!row_major_matrix_needs_conversion && !ignore_potential_sliced_writes) {
                ...
                prepare_access_chain_for_scalar_access(expr, get<SPIRType>(type->parent_type), effective_storage, is_packed);
            }
            ...
            if (row_major_matrix_needs_conversion && !ignore_potential_sliced_writes) {
                if (prepare_access_chain_for_scalar_access(expr, get<SPIRType>(type->parent_type), effective_storage, is_packed)) {
...
string CompilerGLSL::access_chain(uint32_t base, const uint32_t* indices, uint32_t count, const SPIRType& target_type, AccessChainMeta* meta, bool ptr_chain) {
    if (flattened_buffer_blocks.count(base)) {
        uint32_t matrix_stride = 0;
        uint32_t array_stride = 0;
        bool need_transpose = false;
        flattened_access_chain_offset(expression_type(base), indices, count, 0, 16, &need_transpose, &matrix_stride, &array_stride, ptr_chain);
...
void CompilerGLSL::store_flattened_struct(const string& basename, uint32_t rhs_id, const SPIRType& type, const SmallVector<uint32_t>& indices) {
...
std::string CompilerGLSL::flattened_access_chain(uint32_t base, const uint32_t* indices, uint32_t count, const SPIRType& target_type, uint32_t offset,
                                                 uint32_t matrix_stride, uint32_t /* array_stride */, bool need_transpose) {
...
std::string CompilerGLSL::flattened_access_chain_struct(uint32_t base, const uint32_t* indices, uint32_t count, const SPIRType& target_type, uint32_t offset) {
    ...
    for (uint32_t i = 0; i < uint32_t(target_type.member_types.size()); ++i) {
        ...
        auto tmp = flattened_access_chain(base, indices, count, member_type, offset + member_offset, matrix_stride, 0 /* array_stride */, need_transpose);
...
std::string CompilerGLSL::flattened_access_chain_vector(uint32_t base, const uint32_t* indices, uint32_t count, const SPIRType& target_type, uint32_t offset,
                                                        uint32_t matrix_stride, bool need_transpose) {
...
std::pair<std::string, uint32_t> CompilerGLSL::flattened_access_chain_offset(const SPIRType& basetype, const uint32_t* indices, uint32_t count, uint32_t offset,
                                                                             uint32_t word_stride, bool* need_transpose, uint32_t* out_matrix_stride,
                                                                             uint32_t* out_array_stride, bool ptr_chain) {
    // Start traversing type hierarchy at the proper non-pointer types.
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        // We also check if this member is a builtin, since we then replace the entire expression with the builtin one.
            ...
            if (type->columns > 1) {
                ...
                row_major_matrix_needs_conversion = combined_decoration_for_member(struct_type, index).get(DecorationRowMajor);
            } else
                row_major_matrix_needs_conversion = false;
...
bool CompilerGLSL::should_dereference(uint32_t id) {
    ...
    if (auto* expr = maybe_get<SPIRExpression>(id)) {
        ...
        while (expr->loaded_from && expression_is_forwarded(expr->self)) {
            ...
            if (src_type.pointer != type.pointer || src_type.pointer_depth != type.pointer_depth || src_type.parent_type != type.parent_type)
                break;
...
bool CompilerGLSL::should_forward(uint32_t id) const {
    ...
    if (expr && expr->loaded_from && has_decoration(expr->loaded_from, DecorationBuiltIn) && has_decoration(expr->loaded_from, DecorationVolatile)) {
        // Never forward volatile builtin variables, e.g. SPIR-V 1.6 HelperInvocation.
        return false;
    }
...
void CompilerGLSL::flush_variable_declaration(uint32_t id) {
    ...
    if (var && var->deferred_declaration) {
        ...
        if (options.force_zero_initialized_variables &&
            (var->storage == StorageClassFunction || var->storage == StorageClassGeneric || var->storage == StorageClassPrivate) && !var->initializer &&
            type_can_zero_initialize(get_variable_data_type(*var))) {
            initializer = join(" = ", to_zero_initialized_expression(get_variable_data_type_id(*var)));
        }
...
string CompilerGLSL::build_composite_combiner(uint32_t return_type, const uint32_t* elems, uint32_t length) {
    ...
    bool can_apply_swizzle_opt = type.basetype != SPIRType::Struct && type.array.empty() && type.columns == 1 && type.op != OpTypeCooperativeMatrixKHR;
    ...
    for (uint32_t i = 0; i < length; i++) {
        ...
        if (can_apply_swizzle_opt && e && e->base_expression && e->base_expression == base) {
            ...
            bool uses_buffer_offset = type.basetype == SPIRType::Struct && has_member_decoration(type.self, i, DecorationOffset);
...
void CompilerGLSL::analyze_precision_requirements(uint32_t type_id, uint32_t dst_id, uint32_t* args, uint32_t length) {
    ...
    if ((operation_is_highp && input_precision == Options::Mediump) || (!operation_is_highp && input_precision == Options::Highp)) {
...
#define GLSL_BOP_CAST(op, type) emit_binary_op_cast(ops[0], ops[1], ops[2], ops[3], #op, type, opcode_is_sign_invariant(opcode), implicit_integer_promotion)
...
#define GLSL_BFOP_CAST(op, type) emit_binary_func_op_cast(ops[0], ops[1], ops[2], ops[3], #op, type, opcode_is_sign_invariant(opcode))
#define GLSL_BFOP(op) emit_binary_func_op(ops[0], ops[1], ops[2], ops[3], #op)
#define GLSL_UFOP(op) emit_unary_func_op(ops[0], ops[1], ops[2], #op)
    ...
    bool implicit_integer_promotion =
        integer_width < 32 && backend.implicit_c_integer_promotion_rules && opcode_can_promote_integer_implicitly(opcode) && get<SPIRType>(ops[0]).vecsize == 1;
    ...
    case OpLoad: {
        ...
        if (forward) {
            ...
            if (has_extended_decoration(ptr, SPIRVCrossDecorationPhysicalTypeID)) {
                set_extended_decoration(id, SPIRVCrossDecorationPhysicalTypeID, get_extended_decoration(ptr, SPIRVCrossDecorationPhysicalTypeID));
            }
        } else {
    ...
    case OpPtrAccessChain: {
        ...
        auto& expr =
            requires_temporary ? emit_op(ops[0], ops[1], std::move(e), false) : set<SPIRExpression>(ops[1], std::move(e), ops[0], should_forward(ops[2]));
    ...
    case OpFunctionCall: {
        ...
        if (return_type.basetype != SPIRType::Void) {
            ...
            bool forward =
                args_will_forward(id, arg, length, pure) && !callee_has_out_variables && pure && (forced_temporaries.find(id) == end(forced_temporaries));
    ...
    case OpCompositeExtract: {
        ...
        if (has_extended_decoration(ops[2], SPIRVCrossDecorationPhysicalTypePacked) || has_extended_decoration(ops[2], SPIRVCrossDecorationPhysicalTypeID))
            allow_base_expression = false;
        ...
        if (c && !c->specialization && !composite_type_is_complex) {
            ...
            auto expr = access_chain_internal(ops[2], &ops[3], length,
                                              ACCESS_CHAIN_INDEX_IS_LITERAL_BIT | ACCESS_CHAIN_CHAIN_ONLY_BIT | ACCESS_CHAIN_FORCE_COMPOSITE_BIT, &meta);
            ...
            auto expr = access_chain_internal(ops[2], &ops[3], length, ACCESS_CHAIN_INDEX_IS_LITERAL_BIT | ACCESS_CHAIN_FORCE_COMPOSITE_BIT, &meta);
    ...
    case OpCompositeInsert: {
        ...
        if (invalid_expressions.count(composite) || block_composite_insert_overwrite.count(composite) || hoisted_temporaries.count(id) ||
            hoisted_temporaries.count(composite) || maybe_get<SPIRExpression>(composite) == nullptr) {
            can_modify_in_place = false;
        } else if (backend.requires_relaxed_precision_analysis &&
                   has_decoration(composite, DecorationRelaxedPrecision) != has_decoration(id, DecorationRelaxedPrecision) &&
    ...
    case OpMatrixTimesScalar: {
        ...
        if (a && a->need_transpose) {
            a->need_transpose = false;
            auto expr = join(enclose_expression(to_unpacked_row_major_matrix_expression(ops[2])), " * ", to_enclosed_unpacked_expression(ops[3]));
            bool forward = should_forward(ops[2]) && should_forward(ops[3]);
            emit_transposed_op(ops[0], ops[1], expr, forward);
            a->need_transpose = true;
            inherit_expression_dependencies(ops[1], ops[2]);
    ...
    case OpISubBorrow: {
        ...
        statement(to_expression(result_id), ".", to_member_name(type, 0), " = ", op, "(", to_expression(op0), ", ", to_expression(op1), ", ",
    ...
    case OpSMulExtended: {
        ...
        statement(op, "(", to_expression(op0), ", ", to_expression(op1), ", ", to_expression(result_id), ".", to_member_name(type, 1), ", ",
                  to_expression(result_id), ".", to_member_name(type, 0), ");");
        break;
    }
    ...
    case OpFRem: {
        ...
        std::string expr;
        if (!is_legacy()) {
            expr = join(to_enclosed_expression(op0), " - ", to_enclosed_expression(op1), " * ", "trunc(", to_enclosed_expression(op0), " / ",
                        to_enclosed_expression(op1), ")");
        } else {
            ...
            auto via_type = op0_type;
            via_type.basetype = SPIRType::Int;
            expr = join(to_enclosed_expression(op0), " - ", to_enclosed_expression(op1), " * ", type_to_glsl(op0_type), "(", type_to_glsl(via_type), "(",
    ...
    case OpConvertFToS: {
        ...
        expected_type.basetype = opcode == OpConvertFToS ? to_signed_basetype(type.width) : to_unsigned_basetype(type.width);
    ...
    case OpFConvert: {
        ...
        if (type.op == OpTypeCooperativeMatrixKHR && opcode == OpFConvert) {
            ...
            if (get<SPIRConstant>(type.ext.cooperative.use_id).scalar() != get<SPIRConstant>(expr_type.ext.cooperative.use_id).scalar()) {
    ...
    case OpBitFieldSExtract: {
        emit_trinary_func_op_bitextract(ops[0], ops[1], ops[2], ops[3], ops[4], "bitfieldExtract", int_type, int_type, SPIRType::Int, SPIRType::Int);
        break;
    }

    case OpBitFieldUExtract: {
        emit_trinary_func_op_bitextract(ops[0], ops[1], ops[2], ops[3], ops[4], "bitfieldExtract", uint_type, uint_type, SPIRType::Int, SPIRType::Int);
        break;
    }
    ...
    case OpAtomicLoad: {
        ...
        bool unsigned_type = (type.basetype == SPIRType::UInt) || (atomic_image && get<SPIRType>(type.image.type).basetype == SPIRType::UInt);
    ...
    case OpImageQuerySizeLod: {
        ...
        auto expr = join(fname, "(", convert_separate_image_to_expression(img), ", ", bitcast_expression(SPIRType::Int, ops[3]), ")");
    ...
    case OpImageSparseRead: {
        ...
        if (var && var->remapped_variable) // Remapped input, just read as-is without any op-code
        {
            ...
            auto itr = find_if(begin(pls_inputs), end(pls_inputs), [var](const PlsRemap& pls) { return pls.id == var->self; });
            ...
            if (var && subpass_input_is_framebuffer_fetch(var->self)) {
                ...
                if (type.image.ms) {
                    ...
                    uint32_t samples = ops[5];
                    imgexpr = join("texelFetch(", to_non_uniform_aware_expression(ops[2]), ", ivec2(gl_FragCoord.xy), ", to_expression(samples), ")");
                } else {
            ...
            if (sparse) {
                if (type.image.ms) {
                    ...
                    uint32_t samples = ops[5];
                    statement(to_expression(sparse_code_id), " = sparseImageLoadARB(", to_non_uniform_aware_expression(ops[2]), ", ", coord_expr, ", ",
                              to_expression(samples), ", ", to_expression(sparse_texel_id), ");");
                } else {
                    statement(to_expression(sparse_code_id), " = sparseImageLoadARB(", to_non_uniform_aware_expression(ops[2]), ", ", coord_expr, ", ",
                              to_expression(sparse_texel_id), ");");
                }
                imgexpr = join(type_to_glsl(get<SPIRType>(result_type)), "(", to_expression(sparse_code_id), ", ", to_expression(sparse_texel_id), ")");
            } else {
                if (type.image.ms) {
                    uint32_t operands = ops[4];
                    if (operands != ImageOperandsSampleMask || length != 6)
                    ...
                    uint32_t samples = ops[5];
                    imgexpr = join("imageLoad(", to_non_uniform_aware_expression(ops[2]), ", ", coord_expr, ", ", to_expression(samples), ")");
                } else
    ...
    case OpMemoryBarrier: {
        ...
        if (opcode == OpMemoryBarrier) {
            ...
            if (next && next->op == OpControlBarrier) {
                ...
                if (next_memory == memory)
                    ...
                    if ((next_memory == ScopeDevice || next_memory == ScopeWorkgroup) && (memory == ScopeDevice || memory == ScopeWorkgroup)) {
                        memory_scope_covered = true;
                    }
        ...
        if (memory == ScopeWorkgroup) // Only need to consider memory within a group
        {
            ...
            const uint32_t all_barriers = MemorySemanticsWorkgroupMemoryMask | MemorySemanticsUniformMemoryMask | MemorySemanticsImageMemoryMask;
    ...
    case OpExtInstWithForwardRefsKHR: {
        ...
        if (ext != SPIRExtension::SPV_debug_info && ext != SPIRExtension::NonSemanticShaderDebugInfo && ext != SPIRExtension::NonSemanticGeneric) {
    ...
    case OpTerminateRayNV:
        ...
        break;
    case OpTraceNV:
        statement("traceNV(", to_non_uniform_aware_expression(ops[0]), ", ", to_expression(ops[1]), ", ", to_expression(ops[2]), ", ", to_expression(ops[3]),
                  ", ", to_expression(ops[4]), ", ", to_expression(ops[5]), ", ", to_expression(ops[6]), ", ", to_expression(ops[7]), ", ",
    ...
    case OpTraceRayKHR:
        ...
        statement("traceRayEXT(", to_non_uniform_aware_expression(ops[0]), ", ", to_expression(ops[1]), ", ", to_expression(ops[2]), ", ",
                  to_expression(ops[3]), ", ", to_expression(ops[4]), ", ", to_expression(ops[5]), ", ", to_expression(ops[6]), ", ", to_expression(ops[7]),
    ...
    case OpRayQueryInitializeKHR:
        ...
        statement("rayQueryInitializeEXT(", to_expression(ops[0]), ", ", to_expression(ops[1]), ", ", to_expression(ops[2]), ", ", to_expression(ops[3]), ", ",
                  to_expression(ops[4]), ", ", to_expression(ops[5]), ", ", to_expression(ops[6]), ", ", to_expression(ops[7]), ");");
        break;
...
#undef GLSL_RAY_QUERY_GET_OP2
    ...
    case OpConvertUToAccelerationStructureKHR: {
        ...
        bool elide_temporary = should_forward(ops[2]) && forced_temporaries.count(ops[1]) == 0 && !hoisted_temporaries.count(ops[1]);
    ...
    case OpCooperativeVectorOuterProductAccumulateNV: {
        ...
        statement(join(
            "coopVecOuterProductAccumulateNV(", to_expression(v1), ", ", to_expression(v2), ", ", to_expression(buf), ", ", to_expression(offset), ", ",
            matrix_stride_id ? to_expression(matrix_stride_id) : "0", ", ",
            to_pretty_expression_if_int_constant(matrix_layout_id, std::begin(CoopVecMatrixLayoutNames), std::end(CoopVecMatrixLayoutNames)), ", ",
            to_pretty_expression_if_int_constant(matrix_iterpretation_id, std::begin(CoopVecComponentTypeNames), std::end(CoopVecComponentTypeNames)), ");"));
        register_write(ops[0]);
        break;
    }
    ...
    case OpCooperativeVectorReduceSumAccumulateNV: {
        ...
        auto offset = ops[1];
        auto v1 = ops[2];
        statement(join("coopVecReduceSumAccumulateNV(", to_expression(v1), ", ", to_expression(buf), ", ", to_expression(offset), ");"));
        register_write(ops[0]);
        break;
    }
    ...
    case OpCooperativeVectorMatrixMulAddNV: {
        ...
        for (uint32_t i = 1; i < length; i++) {
            ...
            if (i == 3 || i == 6 || (i == 9 && opcode == OpCooperativeVectorMatrixMulAddNV)) {
                stmt += to_pretty_expression_if_int_constant(ops[i], std::begin(CoopVecComponentTypeNames), std::end(CoopVecComponentTypeNames));
            } else if ((i == 12 && opcode == OpCooperativeVectorMatrixMulAddNV) || (i == 9 && opcode == OpCooperativeVectorMatrixMulNV)) {
                stmt += to_pretty_expression_if_int_constant(ops[i], std::begin(CoopVecMatrixLayoutNames), std::end(CoopVecMatrixLayoutNames));
            } else
    ...
    case OpCooperativeMatrixLengthKHR: {
        ...
        uint32_t result_type = ops[0];
        uint32_t id = ops[1];
        set<SPIRExpression>(id, join(type_to_glsl(get<SPIRType>(result_type)), "(", type_to_glsl(get<SPIRType>(ops[2])), "(0).length())"), result_type, true);
        break;
    }
    ...
    case OpCooperativeMatrixLoadKHR: {
        ...
        string layout_expr = to_pretty_expression_if_int_constant(ops[3], std::begin(CoopMatMatrixLayoutNames), std::end(CoopMatMatrixLayoutNames));
        statement("coopMatLoad(", to_expression(id), ", ", split_expr.first, ", ", split_expr.second, ", ", to_expression(ops[4]), ", ", layout_expr, ");");
    ...
    case OpCooperativeMatrixStoreKHR: {
        ...
        string layout_expr = to_pretty_expression_if_int_constant(ops[2], std::begin(CoopMatMatrixLayoutNames), std::end(CoopMatMatrixLayoutNames));

        statement("coopMatStore(", to_expression(ops[1]), ", ", split_expr.first, ", ", split_expr.second, ", ", to_expression(ops[3]), ", ", layout_expr,
                  ");");
    ...
    case OpCooperativeMatrixMulAddKHR: {
        ...
        emit_op(result_type, id,
                join("coopMatMulAdd(", to_unpacked_expression(A), ", ", to_unpacked_expression(B), ", ", to_unpacked_expression(C), ", ",
                     (length >= 6 ? ops[5] : 0), ")"),
                forward);
    ...
    case OpCompositeConstructReplicateEXT: {
        ...
        if (type.op == OpTypeMatrix || type.op == OpTypeArray) {
            if (type.op == OpTypeArray && type.array.size() != 1) {
                SPIRV_CROSS_THROW("Multi-dimensional arrays currently not supported for OpCompositeConstructReplicateEXT");
            }
...
string CompilerGLSL::convert_row_major_matrix(string exp_str, const SPIRType& exp_type, uint32_t /* physical_type_id */, bool /*is_packed*/, bool relaxed) {
...
void CompilerGLSL::emit_struct_member(const SPIRType& type, uint32_t member_type_id, uint32_t index, const string& qualifier, uint32_t) {
...
string CompilerGLSL::flags_to_qualifiers_glsl(const SPIRType& type, uint32_t id, const Bitset& flags) {
    ...
    if (type_is_floating_point(type) && (flags.get(DecorationNoContraction) || (type.self && has_legacy_nocontract(type.self, id))) &&
    ...
    bool type_supports_precision = type.basetype == SPIRType::Float || type.basetype == SPIRType::Int || type.basetype == SPIRType::UInt ||
                                   type.basetype == SPIRType::Image || type.basetype == SPIRType::SampledImage || type.basetype == SPIRType::Sampler;
    ...
    if (options.es) {
        ...
        if (type.basetype == SPIRType::UInt && is_legacy_es()) {
            ...
            bool implied_fmediump =
                type.basetype == SPIRType::Float && options.fragment.default_float_precision == Options::Mediump && execution.model == ExecutionModelFragment;

            bool implied_imediump = (type.basetype == SPIRType::Int || type.basetype == SPIRType::UInt) &&
                                    options.fragment.default_int_precision == Options::Mediump && execution.model == ExecutionModelFragment;

            qual += (implied_fmediump || implied_imediump) ? "" : "mediump ";
        } else {
            bool implied_fhighp = type.basetype == SPIRType::Float &&
                                  ((options.fragment.default_float_precision == Options::Highp && execution.model == ExecutionModelFragment) ||
...
string CompilerGLSL::argument_decl(const SPIRFunction::Parameter& arg) {
    ...
    if (is_pointer(type) && (type.storage == StorageClassFunction || type.storage == StorageClassPrivate || type.storage == StorageClassOutput)) {
        ...
        if (type.storage == StorageClassOutput && get_execution_model() == ExecutionModelMeshEXT && has_decoration(type.self, DecorationBlock) &&
            is_builtin_type(type) && arg.write_count) {
            direction = "out ";
...
#ifndef NDEBUG
    auto& type = get<SPIRType>(type_id);
    assert(type.storage == StorageClassPrivate || type.storage == StorageClassFunction || type.storage == StorageClassGeneric);
#endif
...
string CompilerGLSL::image_type_glsl(const SPIRType& type, uint32_t id, bool /*member*/) {
    ...
    else if (type.basetype == SPIRType::Image && type.image.dim == DimSubpassData && subpass_input_is_framebuffer_fetch(id)) {
...
string CompilerGLSL::type_to_glsl(const SPIRType& type, uint32_t id) {
    ...
    case SPIRType::Tensor:
        ...
        return join("tensorARM<", type_to_glsl(get<SPIRType>(type.ext.tensor.type)), ", ", to_expression(type.ext.tensor.rank), ">");
    ...
    if (coop_type->op == OpTypeCooperativeMatrixKHR) {
        ...
        return join("coopmat<", type_to_glsl(get<SPIRType>(coop_type->parent_type)), ", ", scope_expr, ", ", to_expression(coop_type->ext.cooperative.rows_id),
...
void CompilerGLSL::add_variable(unordered_set<string>& variables_primary, const unordered_set<string>& variables_secondary, string& name) {
    if (name.empty())
        return;
...
void CompilerGLSL::add_function_overload(const SPIRFunction& func) {
    ...
    for (auto& arg : func.arguments) {
        ...
        if (!combined_image_samplers.empty()) {
            ...
            if (type.basetype == SPIRType::SampledImage || (type.basetype == SPIRType::Image && type.image.sampled == 1) ||
    ...
    for (auto itr = begin(child.phi_variables); itr != end(child.phi_variables); ++itr) {
        ...
        if (phi.parent == from) {
            ...
            if (var.loop_variable && !var.loop_variable_enable)
                ...
                bool need_saved_temporary = find_if(itr + 1, end(child.phi_variables), [&](const SPIRBlock::Phi& future_phi) -> bool {
    ...
    if ((ir.block_meta[to] & ParsedIR::BLOCK_META_LOOP_HEADER_BIT) != 0 && get<SPIRBlock>(from).loop_dominator == to) {
        ...
        if (is_loop_break(to)) {
            for (size_t n = current_emitting_switch_stack.size(); n; n--) {
                ...
                if (current_emitting_switch && current_emitting_switch->loop_dominator != BlockID(SPIRBlock::NoDominator) &&
        ...
        bool branching_to_merge =
            (block_meta & (ParsedIR::BLOCK_META_SELECTION_MERGE_BIT | ParsedIR::BLOCK_META_MULTISELECT_MERGE_BIT | ParsedIR::BLOCK_META_LOOP_MERGE_BIT)) != 0;
...
bool CompilerGLSL::attempt_emit_loop_header(SPIRBlock& block, SPIRBlock::Method method) {
    ...
    if (method == SPIRBlock::MergeToSelectForLoop || method == SPIRBlock::MergeToSelectContinueForLoop) {
        ...
        bool flushes_phi = flush_phi_required(block.self, block.true_block) || flush_phi_required(block.self, block.false_block);
        ...
        bool flushes_phi = flush_phi_required(child.self, child.true_block) || flush_phi_required(child.self, child.false_block);
...
void CompilerGLSL::emit_hoisted_temporaries(SmallVector<pair<TypeID, ID>>& temporaries) {
    ...
    sort(begin(temporaries), end(temporaries), [](const pair<TypeID, ID>& a, const pair<TypeID, ID>& b) { return a.second < b.second; });
    ...
    for (auto& tmp : temporaries) {
        ...
        if (mirrored_precision_itr != temporary_to_mirror_precision_alias.end()) {
            ...
            statement(flags_to_qualifiers_glsl(type, mirror_id, mirror_flags), variable_decl(type, to_name(mirror_id)), initializer, ";");
            // The temporary might be read from before it's assigned, set up the expression now.
...
BlockID CompilerGLSL::emit_block_chain_inner(SPIRBlock& block) {
    ...
    for (auto var_id : block.dominated_variables) {
        ...
        if (var.phi_variable) {
            ...
            if (mirrored_precision_itr != temporary_to_mirror_precision_alias.end() &&
                find_if(block.declare_temporary.begin(), block.declare_temporary.end(), [mirrored_precision_itr](const std::pair<TypeID, VariableID>& p) {
    ...
    case SPIRBlock::MultiSelect: {
        ...
        bool unsigned_case =
            type.basetype == SPIRType::UInt || type.basetype == SPIRType::UShort || type.basetype == SPIRType::UByte || type.basetype == SPIRType::UInt64;
        ...
        const auto to_legacy_case_label = [&](uint32_t condition, const SmallVector<uint64_t>& labels, const char* suffix) -> string {
            string ret;
            size_t count = labels.size();
            for (size_t i = 0; i < count; i++) {
                if (i)
                    ret += " || ";
                ret += join(count > 1 ? "(" : "", to_enclosed_expression(condition), " == ", labels[i], suffix, count > 1 ? ")" : "");
            }
            return ret;
        };
        ...
        for (size_t i = 1; backend.support_case_fallthrough && i < num_blocks; i++) {
            if (flush_phi_required(block.self, block_declaration_order[i]) && flush_phi_required(block_declaration_order[i - 1], block_declaration_order[i])) {
                ...
                if (literals.empty()) {
                    ...
                    for (size_t j = 0; j < num_blocks; j++) {
                        ...
                        for (auto& case_label : negative_literals)
                            conditions.push_back(join(to_enclosed_expression(block.condition), " != ", to_case_label(case_label, type.width, unsigned_case)));
                    }
        ...
        if (block.return_value) {
            ...
            if (!type.array.empty() && !backend.can_return_array) {
                ...
                if (ir.ids[block.return_value].get_type() != TypeUndef) {
                    emit_array_copy("spvReturnValue", 0, block.return_value, StorageClassFunction, get_expression_effective_storage_class(block.return_value));
                }
    ...
    case SPIRBlock::Unreachable: {
        ...
        while (id) {
            ...
            if (iter_block.terminator == SPIRBlock::MultiSelect || iter_block.merge == SPIRBlock::MergeLoop) {
                ID next_block = iter_block.merge == SPIRBlock::MergeLoop ? iter_block.merge_block : iter_block.next_block;
    ...
    if (block.merge == SPIRBlock::MergeLoop) {
        if (continue_type == SPIRBlock::DoWhileLoop) {
            ...
            bool positive_test = execution_is_noop(get<SPIRBlock>(continue_block.true_block), get<SPIRBlock>(continue_block.loop_dominator));
...
uint32_t CompilerGLSL::mask_relevant_memory_semantics(uint32_t semantics) {
    return semantics & (MemorySemanticsAtomicCounterMemoryMask | MemorySemanticsImageMemoryMask | MemorySemanticsWorkgroupMemoryMask |
                        MemorySemanticsUniformMemoryMask | MemorySemanticsCrossWorkgroupMemoryMask | MemorySemanticsSubgroupMemoryMask);
}
...
bool CompilerGLSL::unroll_array_to_complex_store(uint32_t target_id, uint32_t source_id) {
    ...
    begin_scope();
    statement(to_expression(target_id), "[i] = ", bitcast_expression(target_type, type.basetype, join(to_expression(source_id), "[i]")), ";");
    end_scope();
...
void CompilerGLSL::unroll_array_from_complex_load(uint32_t target_id, uint32_t source_id, std::string& expr) {
    ...
    auto builtin = BuiltIn(get_decoration(var->self, DecorationBuiltIn));
    bool is_builtin = is_builtin_variable(*var) && (builtin == BuiltInPointSize || builtin == BuiltInPosition || builtin == BuiltInSampleMask);
...
void CompilerGLSL::convert_non_uniform_expression(string& expr, uint32_t ptr_id) {
    ...
    if (var->storage != StorageClassUniformConstant && var->storage != StorageClassStorageBuffer && var->storage != StorageClassUniform)
        return;
    ...
    expr = join(expr.substr(0, start_array_index), backend.nonuniform_qualifier, "(", expr.substr(start_array_index, end_array_index - start_array_index), ")",
...
void CompilerGLSL::emit_non_semantic_shader_debug_info(uint32_t, uint32_t result_id, uint32_t eop, const uint32_t* args, uint32_t) {
    if (!options.emit_line_directives)
        return;
...
void CompilerGLSL::emit_copy_logical_type(uint32_t lhs_id, uint32_t lhs_type_id, uint32_t rhs_id, uint32_t rhs_type_id, SmallVector<uint32_t> chain) {
...
const SPIRVariable* CompilerGLSL::find_subpass_input_by_attachment_index(uint32_t index) const {
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable& var) {
        if (has_decoration(var.self, DecorationInputAttachmentIndex) && get_decoration(var.self, DecorationInputAttachmentIndex) == index) {
            ret = &var;
        }
    });
    return ret;
...
void CompilerGLSL::emit_inout_fragment_outputs_copy_to_subpass_inputs() {
    for (auto& remap : subpass_to_framebuffer_fetch_attachment) {
        ...
        func.fixup_hooks_in.push_back([=]() {
            if (is_legacy()) {
                statement(to_expression(subpass_var->self), " = ", "gl_LastFragData[", get_decoration(output_var->self, DecorationLocation), "];");
            } else {
                uint32_t num_rt_components = this->get<SPIRType>(output_var->basetype).vecsize;
                statement(to_expression(subpass_var->self), vector_swizzle(num_rt_components, 0), " = ", to_expression(output_var->self), ";");
            }
        });
    }
}
...
CompilerGLSL::ShaderSubgroupSupportHelper::FeatureVector CompilerGLSL::ShaderSubgroupSupportHelper::get_feature_dependencies(Feature feature) {
    switch (feature) {
    ...
    case SubgroupArithmeticFMulExclusiveScan:
        return {SubgroupSize, SubgroupBallot, SubgroupBallotBitCount, SubgroupMask, SubgroupElect, SubgroupBallotBitExtract};
    default:
        return {};
    }
}

CompilerGLSL::ShaderSubgroupSupportHelper::FeatureMask CompilerGLSL::ShaderSubgroupSupportHelper::get_feature_dependency_mask(Feature feature) {
...
CompilerGLSL::ShaderSubgroupSupportHelper::Candidate CompilerGLSL::ShaderSubgroupSupportHelper::get_KHR_extension_for_feature(Feature feature) {
    static const Candidate extensions[FeatureCount] = {
        ...
        KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic,
        KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic,
        KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic, KHR_shader_subgroup_arithmetic,
    };
...
CompilerGLSL::ShaderSubgroupSupportHelper::CandidateVector CompilerGLSL::ShaderSubgroupSupportHelper::get_candidates_for_feature(Feature ft, const Result& r) {
...
CompilerGLSL::ShaderSubgroupSupportHelper::CandidateVector CompilerGLSL::ShaderSubgroupSupportHelper::get_candidates_for_feature(Feature feature) {
    switch (feature) {
    case SubgroupMask:
...
CompilerGLSL::ShaderSubgroupSupportHelper::FeatureMask CompilerGLSL::ShaderSubgroupSupportHelper::build_mask(const SmallVector<Feature>& features) {
    FeatureMask mask = 0;
...
void CompilerGLSL::request_workaround_wrapper_overload(TypeID id) {
    ...
    if (find(begin(workaround_ubo_load_overload_types), end(workaround_ubo_load_overload_types), id) == end(workaround_ubo_load_overload_types)) {
        force_recompile();
...
void CompilerGLSL::rewrite_load_for_wrapped_row_major(std::string& expr, TypeID loaded_type, ID ptr) {
    ...
    auto& backing_type = get<SPIRType>(var->basetype);
    bool is_ubo =
        backing_type.basetype == SPIRType::Struct && backing_type.storage == StorageClassUniform && has_decoration(backing_type.self, DecorationBlock);
    if (!is_ubo)
        return;
...
bool CompilerGLSL::is_stage_output_variable_masked(const SPIRVariable& var) const {
    ...
    if (is_builtin) {
        ...
        return is_stage_output_location_masked(get_decoration(var.self, DecorationLocation), get_decoration(var.self, DecorationComponent));
    }
}
...
std::string CompilerGLSL::to_pretty_expression_if_int_constant(uint32_t id, const GlslConstantNameMapping* mapping_start,
                                                               const GlslConstantNameMapping* mapping_end, bool register_expression_read) {
    auto* c = maybe_get<SPIRConstant>(id);
    if (c && !c->specialization) {
        auto value = c->scalar();
        auto pretty_name = std::find_if(mapping_start, mapping_end, [value](const GlslConstantNameMapping& mapping) { return mapping.value == value; });
