...
namespace SPIRV_CROSS_NAMESPACE {
    namespace inner {
        template <typename T> void join_helper(StringStream<>& stream, T&& t) {
            stream << std::forward<T>(t);
        }

        template <typename T, typename... Ts> void join_helper(StringStream<>& stream, T&& t, Ts&&... ts) {
            stream << std::forward<T>(t);
    ...
    class Bitset {
      public:
        Bitset() = default;
        explicit inline Bitset(uint64_t lower_) : lower(lower_) {}
        ...
        inline uint64_t get_lower() const { return lower; }
        ...
        inline bool operator!=(const Bitset& other) const { return !(*this == other); }

        template <typename Op> void for_each_bit(const Op& op) const {
        ...
        inline bool empty() const { return lower == 0 && higher.empty(); }
    ...
    template <typename T, typename std::enable_if<!std::is_floating_point<T>::value, int>::type = 0> inline std::string convert_to_string(const T& t) {
...
#pragma warning(pop)
    ...
    template <typename T> struct ValueSaver {
        explicit ValueSaver(T& current_) : current(current_), saved(current_) {}

        void release() { current = saved; }
    ...
    struct Instruction {
        ...
        inline bool is_embedded() const { return offset == 0; }
    };
    ...
    template <Types type> class TypedID;

    template <> class TypedID<TypeNone> {
      public:
        TypedID() = default;
        TypedID(uint32_t id_) : id(id_) {}

        template <Types U> TypedID(const TypedID<U>& other) { *this = other; }

        template <Types U> TypedID& operator=(const TypedID<U>& other) {
            id = uint32_t(other);
            return *this;
        }
        ...
        template <Types U> operator TypedID<U>() const { return TypedID<U>(*this); }
    ...
    template <Types type> class TypedID {
      public:
        ...
        explicit TypedID(const TypedID<TypeNone>& other) : id(uint32_t(other)) {}
...
#define SPIRV_CROSS_DECLARE_CLONE(T)                                                                                                                           \
    ...
    struct SPIRUndef : IVariant {
        ...
        explicit SPIRUndef(TypeID basetype_) : basetype(basetype_) {}
        TypeID basetype;
    ...
    struct SPIRString : IVariant {
        enum { type = TypeString };

        explicit SPIRString(std::string str_) : str(std::move(str_)) {}
    ...
    struct SPIRDebugLocalVariable : IVariant {
        enum { type = TypeDebugLocalVariable };
    ...
    struct SPIRCombinedImageSampler : IVariant {
        enum { type = TypeCombinedImageSampler };
        SPIRCombinedImageSampler(TypeID type_, VariableID image_, VariableID sampler_) : combined_type(type_), image(image_), sampler(sampler_) {}
    ...
    struct SPIRConstantOp : IVariant {
        ...
        SPIRConstantOp(TypeID result_type, spv::Op op, const uint32_t* args, uint32_t length) : opcode(op), basetype(result_type) {
    ...
    struct SPIRExtension : IVariant {
        ...
        enum ShaderDebugInfoOps { DebugLine = 103, DebugSource = 35 };

        explicit SPIRExtension(Extension ext_) : ext(ext_) {}
    ...
    struct SPIREntryPoint {
        SPIREntryPoint(FunctionID self_, spv::ExecutionModel execution_model, const std::string& entry_name)
            : self(self_), name(entry_name), orig_name(entry_name), model(execution_model) {}
    ...
    struct SPIRExpression : IVariant {
        ...
        SPIRExpression(std::string expr, TypeID expression_type_, bool immutable_)
            : expression(std::move(expr)), expression_type(expression_type_), immutable(immutable_) {}
    ...
    struct SPIRFunctionPrototype : IVariant {
        ...
        explicit SPIRFunctionPrototype(TypeID return_type_) : return_type(return_type_) {}
    ...
    struct SPIRBlock : IVariant {
        ...
        enum Hints { HintNone, HintUnroll, HintDontUnroll, HintFlatten, HintDontFlatten };
        ...
        enum : uint32_t { NoDominator = 0xffffffffu };
    ...
    struct SPIRFunction : IVariant {
        ...
        SPIRFunction(TypeID return_type_, TypeID function_type_) : return_type(return_type_), function_type(function_type_) {}
        ...
        void add_local_variable(VariableID id) { local_variables.push_back(id); }
    ...
    struct SPIRAccessChain : IVariant {
        ...
        SPIRAccessChain(TypeID basetype_, spv::StorageClass storage_, std::string base_, std::string dynamic_index_, int32_t static_index_)
            : basetype(basetype_), storage(storage_), base(std::move(base_)), dynamic_index(std::move(dynamic_index_)), static_index(static_index_) {}
    ...
    struct SPIRVariable : IVariant {
        ...
        SPIRVariable(TypeID basetype_, spv::StorageClass storage_, ID initializer_ = 0, VariableID basevariable_ = 0)
            : basetype(basetype_), storage(storage_), initializer(initializer_), basevariable(basevariable_) {}
    ...
    struct SPIRConstant : IVariant {
        ...
        struct ConstantVector {
            ...
            ConstantVector() { memset(r, 0, sizeof(r)); }
        };
        ...
        inline uint32_t specialization_constant_id(uint32_t col, uint32_t row) const { return m.c[col].id[row]; }

        inline uint32_t specialization_constant_id(uint32_t col) const { return m.id[col]; }

        inline uint32_t scalar(uint32_t col = 0, uint32_t row = 0) const { return m.c[col].r[row].u32; }

        inline int16_t scalar_i16(uint32_t col = 0, uint32_t row = 0) const { return int16_t(m.c[col].r[row].u32 & 0xffffu); }

        inline uint16_t scalar_u16(uint32_t col = 0, uint32_t row = 0) const { return uint16_t(m.c[col].r[row].u32 & 0xffffu); }

        inline int8_t scalar_i8(uint32_t col = 0, uint32_t row = 0) const { return int8_t(m.c[col].r[row].u32 & 0xffu); }

        inline uint8_t scalar_u8(uint32_t col = 0, uint32_t row = 0) const { return uint8_t(m.c[col].r[row].u32 & 0xffu); }

        inline float scalar_f16(uint32_t col = 0, uint32_t row = 0) const { return f16_to_f32(scalar_u16(col, row)); }
        ...
        inline float scalar_floate4m3(uint32_t col = 0, uint32_t row = 0) const { return fe4m3_to_f32(scalar_u8(col, row)); }

        inline float scalar_bf8(uint32_t col = 0, uint32_t row = 0) const { return f16_to_f32(uint16_t(scalar_u8(col, row) << 8)); }

        inline float scalar_f32(uint32_t col = 0, uint32_t row = 0) const { return m.c[col].r[row].f32; }

        inline int32_t scalar_i32(uint32_t col = 0, uint32_t row = 0) const { return m.c[col].r[row].i32; }

        inline double scalar_f64(uint32_t col = 0, uint32_t row = 0) const { return m.c[col].r[row].f64; }

        inline int64_t scalar_i64(uint32_t col = 0, uint32_t row = 0) const { return m.c[col].r[row].i64; }

        inline uint64_t scalar_u64(uint32_t col = 0, uint32_t row = 0) const { return m.c[col].r[row].u64; }
        ...
        explicit SPIRConstant(uint32_t constant_type_) : constant_type(constant_type_) {}
        ...
        SPIRConstant(TypeID constant_type_, const uint32_t* elements, uint32_t num_elements, bool specialized, bool replicated_ = false)
            : constant_type(constant_type_), specialization(specialized), replicated(replicated_) {
        ...
        SPIRConstant(TypeID constant_type_, uint32_t v0, bool specialized) : constant_type(constant_type_), specialization(specialized) {
            m.c[0].r[0].u32 = v0;
            m.c[0].vecsize = 1;
            m.columns = 1;
        }
        ...
        SPIRConstant(TypeID constant_type_, uint64_t v0, bool specialized) : constant_type(constant_type_), specialization(specialized) {
            m.c[0].r[0].u64 = v0;
            m.c[0].vecsize = 1;
            m.columns = 1;
        }
        ...
        SPIRConstant(TypeID constant_type_, const SPIRConstant* const* vector_elements, uint32_t num_elements, bool specialized)
    ...
    class Variant {
      public:
        explicit Variant(ObjectPoolGroup* group_) : group(group_) {}
        ...
        Variant(Variant&& other) SPIRV_CROSS_NOEXCEPT { *this = std::move(other); }
...
// #define SPIRV_CROSS_COPY_CONSTRUCTOR_SANITIZE
        ...
        template <typename T, typename... Ts> T* allocate_and_set(Types new_type, Ts&&... ts) {
        ...
        ID get_id() const { return holder ? holder->self : ID(0); }
        ...
        void set_allow_type_rewrite() { allow_type_rewrite = true; }
    ...
    template <typename T> T& variant_get(Variant& var) {
        return var.get<T>();
    }

    template <typename T> const T& variant_get(const Variant& var) {
        return var.get<T>();
    }

    template <typename T, typename... P> T& variant_set(Variant& var, P&&... args) {
    ...
    class Hasher {
      public:
        inline void u32(uint32_t value) { h = (h * 0x100000001b3ull) ^ value; }

        inline uint64_t get() const { return h; }
    ...
    static inline bool type_is_integral(const SPIRType& type) {
        return type.basetype == SPIRType::SByte || type.basetype == SPIRType::UByte || type.basetype == SPIRType::Short || type.basetype == SPIRType::UShort ||
               type.basetype == SPIRType::Int || type.basetype == SPIRType::UInt || type.basetype == SPIRType::Int64 || type.basetype == SPIRType::UInt64;
    }
    ...
    struct SetBindingPair {
        ...
        inline bool operator==(const SetBindingPair& other) const { return desc_set == other.desc_set && binding == other.binding; }
    ...
    struct LocationComponentPair {
        ...
        inline bool operator==(const LocationComponentPair& other) const { return location == other.location && component == other.component; }
    ...
    struct StageSetBinding {
        ...
        inline bool operator==(const StageSetBinding& other) const { return model == other.model && desc_set == other.desc_set && binding == other.binding; }
    };
...
namespace std {
    template <SPIRV_CROSS_NAMESPACE::Types type> struct hash<SPIRV_CROSS_NAMESPACE::TypedID<type>> {
        size_t operator()(const SPIRV_CROSS_NAMESPACE::TypedID<type>& value) const { return std::hash<uint32_t>()(value); }
    };
} // namespace std
