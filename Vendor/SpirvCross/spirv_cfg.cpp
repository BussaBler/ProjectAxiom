...
namespace SPIRV_CROSS_NAMESPACE {
    CFG::CFG(Compiler& compiler_, const SPIRFunction& func_) : compiler(compiler_), func(func_) {
    ...
    void CFG::visit_branch(uint32_t block_id) {
        ...
        if (std::find(visit_stack.begin() + last_visited_size, visit_stack.end(), block_id) == visit_stack.end() && !has_visited_branch(block_id)) {
    ...
    bool CFG::node_terminates_control_flow_in_sub_graph(BlockID from, BlockID to) const {
        ...
        while (to != from) {
            ...
            bool merges_to_nothing =
                dom.merge == SPIRBlock::MergeNone ||
                (dom.merge == SPIRBlock::MergeSelection && dom.next_block && compiler.get<SPIRBlock>(dom.next_block).terminator == SPIRBlock::Unreachable) ||
                (dom.merge == SPIRBlock::MergeLoop && dom.merge_block && compiler.get<SPIRBlock>(dom.merge_block).terminator == SPIRBlock::Unreachable);
            ...
            if ((dom.merge == SPIRBlock::MergeSelection && dom.next_block == to) || (dom.merge == SPIRBlock::MergeLoop && dom.merge_block == to) ||
    ...
    DominatorBuilder::DominatorBuilder(const CFG& cfg_) : cfg(cfg_) {
    }
