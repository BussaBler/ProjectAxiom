...
namespace SPIRV_CROSS_NAMESPACE {
    ...
    class ParsedIR {
      ...
      public:
        ...
        template <typename T, typename Op> void for_each_typed_id(const Op& op) {
        ...
        template <typename T, typename Op> void for_each_typed_id(const Op& op) const {
            auto loop_lock = create_loop_hard_lock();
            for (auto& id : ids_for_type[T::type]) {
                if (ids[id].get_type() == static_cast<Types>(T::type))
                    op(id, get<T>(id));
        ...
        template <typename T> void reset_all_of_type() { reset_all_of_type(static_cast<Types>(T::type)); }
        ...
        const std::string& get_empty_string() const { return empty_string; }
        ...
        template <typename T> T& get(uint32_t id) { return variant_get<T>(ids[id]); }

        template <typename T> const T& get(uint32_t id) const { return variant_get<T>(ids[id]); }
