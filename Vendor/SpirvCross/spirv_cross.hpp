...
namespace SPIRV_CROSS_NAMESPACE {
    ...
    class Compiler {
      public:
        ...
        void set_member_decoration_string(TypeID id, uint32_t index, Decoration decoration, const std::string& argument);
        ...
        const SmallVector<CombinedImageSampler>& get_combined_image_samplers() const { return combined_image_samplers; }
        ...
        void set_variable_type_remap_callback(VariableTypeRemapCallback cb) { variable_remap_callback = std::move(cb); }
        ...
        uint32_t get_current_id_bound() const { return uint32_t(ir.ids.size()); }
        ...
        bool is_position_invariant() const { return position_invariant; }
      ...
      protected:
        ...
        uint32_t* stream_mutable(const Instruction& instr) const { return const_cast<uint32_t*>(stream(instr)); }
        ...
        void set_initializers(SPIRExpression& e) { e.emitted_loop_level = current_loop_level; }

        template <typename T> void set_initializers(const T&) {}

        // If our IDs are out of range here as part of opcodes, throw instead of
        // undefined behavior.
        template <typename T, typename... P> T& set(uint32_t id, P&&... args) {
        ...
        template <typename T> T& get(uint32_t id) { return variant_get<T>(ir.ids[id]); }

        template <typename T> T* maybe_get(uint32_t id) {
            if (id >= ir.ids.size())
                return nullptr;
        ...
        template <typename T> const T& get(uint32_t id) const { return variant_get<T>(ir.ids[id]); }

        template <typename T> const T* maybe_get(uint32_t id) const {
            if (id >= ir.ids.size())
                return nullptr;
            else if (ir.ids[id].get_type() == static_cast<Types>(T::type))
                return &get<T>(id);
        ...
        inline bool is_continue(uint32_t next) const { return (ir.block_meta[next] & ParsedIR::BLOCK_META_CONTINUE_BIT) != 0; }
        ...
        inline bool is_break(uint32_t next) const {
            return (ir.block_meta[next] & (ParsedIR::BLOCK_META_LOOP_MERGE_BIT | ParsedIR::BLOCK_META_MULTISELECT_MERGE_BIT)) != 0;
        }

        inline bool is_loop_break(uint32_t next) const { return (ir.block_meta[next] & ParsedIR::BLOCK_META_LOOP_MERGE_BIT) != 0; }

        inline bool is_conditional(uint32_t next) const {
            return (ir.block_meta[next] & (ParsedIR::BLOCK_META_SELECTION_MERGE_BIT | ParsedIR::BLOCK_META_MULTISELECT_MERGE_BIT)) != 0;
        }
        ...
        struct OpcodeHandler {
            ...
            virtual bool handle_terminator(const SPIRBlock&) { return true; }

            virtual bool follow_function_call(const SPIRFunction&) { return true; }
            ...
            virtual bool begin_function_scope(const uint32_t*, uint32_t) { return true; }

            virtual bool end_function_scope(const uint32_t*, uint32_t) { return true; }
            ...
            template <typename T> T& get(uint32_t id) { return compiler.get<T>(id); }

            template <typename T> const T& get(uint32_t id) const { return compiler.get<T>(id); }

            template <typename T, typename... P> T& set(uint32_t id, P&&... args) { return compiler.set<T>(id, std::forward<P>(args)...); }
        };
        ...
        struct BufferAccessHandler : OpcodeHandler {
            BufferAccessHandler(const Compiler& compiler_, SmallVector<BufferRange>& ranges_, uint32_t id_)
                : OpcodeHandler(const_cast<Compiler&>(compiler_)), ranges(ranges_), id(id_) {}
        ...
        struct InterfaceVariableAccessHandler : OpcodeHandler {
            InterfaceVariableAccessHandler(const Compiler& compiler_, std::unordered_set<VariableID>& variables_)
                : OpcodeHandler(const_cast<Compiler&>(compiler_)), variables(variables_) {}
        ...
        struct CombinedImageSamplerHandler : OpcodeHandler {
            explicit CombinedImageSamplerHandler(Compiler& compiler_) : OpcodeHandler(compiler_) {}
            bool handle(Op opcode, const uint32_t* args, uint32_t length) override;
            bool begin_function_scope(const uint32_t* args, uint32_t length) override;
            bool end_function_scope(const uint32_t* args, uint32_t length) override;

            // Each function in the call stack needs its own remapping for parameters so we can deduce which global variable each texture/sampler the parameter
            // is statically bound to.
        ...
        struct DummySamplerForCombinedImageHandler : OpcodeHandler {
            explicit DummySamplerForCombinedImageHandler(Compiler& compiler_) : OpcodeHandler(compiler_) {}
            bool handle(Op opcode, const uint32_t* args, uint32_t length) override;
            bool need_dummy_sampler = false;
        };

        struct ActiveBuiltinHandler : OpcodeHandler {
            explicit ActiveBuiltinHandler(Compiler& compiler_) : OpcodeHandler(compiler_) {}
        ...
        void analyze_parameter_preservation(SPIRFunction& entry, const CFG& cfg,
        ...
        struct CombinedImageSamplerDrefHandler : OpcodeHandler {
            explicit CombinedImageSamplerDrefHandler(Compiler& compiler_) : OpcodeHandler(compiler_) {}
            bool handle(Op opcode, const uint32_t* args, uint32_t length) override;
        ...
        struct CombinedImageSamplerUsageHandler : OpcodeHandler {
            CombinedImageSamplerUsageHandler(Compiler& compiler_, const std::unordered_set<uint32_t>& dref_combined_samplers_)
                : OpcodeHandler(compiler_), dref_combined_samplers(dref_combined_samplers_) {}
        ...
        struct GeometryEmitDisocveryHandler : OpcodeHandler {
            explicit GeometryEmitDisocveryHandler(Compiler& compiler_) : OpcodeHandler(compiler_) {}
        ...
        struct InterlockedResourceAccessHandler : OpcodeHandler {
            InterlockedResourceAccessHandler(Compiler& compiler_, uint32_t entry_point_id) : OpcodeHandler(compiler_) { call_stack.push_back(entry_point_id); }
        ...
        struct InterlockedResourceAccessPrepassHandler : OpcodeHandler {
            InterlockedResourceAccessPrepassHandler(Compiler& compiler_, uint32_t entry_point_id) : OpcodeHandler(compiler_) {
        ...
        static bool instruction_to_result_type(uint32_t& result_type, uint32_t& result_id, Op op, const uint32_t* args, uint32_t length);
