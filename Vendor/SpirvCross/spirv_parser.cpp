...
namespace SPIRV_CROSS_NAMESPACE {
    ...
    void Parser::parse() {
        ...
        for (auto& source : ir.sources) {
            auto cmp = [](const ParsedIR::Source::Marker& a, const ParsedIR::Source::Marker& b) { return a.line < b.line; };
    ...
    void Parser::parse(const Instruction& instruction) {
        ...
        case OpEntryPoint: {
            auto itr = ir.entry_points.insert(
                make_pair(ops[1], SPIREntryPoint(ops[1], static_cast<ExecutionModel>(ops[0]), extract_string(ir.spirv, instruction.offset + 2))));
            ...
            for (uint32_t i = 1; i < length; i++) {
                ...
                flags.for_each_bit([&](uint32_t bit) {
                    ...
                    if (decoration_is_string(decoration)) {
                        ...
                        ir.meta[target].decoration_word_offset[decoration] = ir.meta[group_id].decoration_word_offset[decoration];
        ...
        case OpGroupMemberDecorate: {
            ...
            for (uint32_t i = 1; i + 1 < length; i += 2) {
                ...
                flags.for_each_bit([&](uint32_t bit) {
                    ...
                    if (decoration_is_string(decoration))
                        ir.set_member_decoration_string(target, index, decoration, ir.get_decoration_string(group_id, decoration));
                    else
        ...
        case OpTypeStruct: {
            ...
            if (consider_aliasing) {
                for (auto& other : global_struct_cache) {
                    if (ir.get_name(type.self) == ir.get_name(other) && types_are_logically_equivalent(type, get<SPIRType>(other))) {
        ...
        case OpFunctionEnd: {
            if (current_block) {
                ...
                SPIRV_CROSS_THROW("Cannot end a function before ending the current block.\n"
        ...
        case OpBranchConditional: {
            ...
            if (current_block->true_block == current_block->false_block) {
                ...
                if (current_block->true_block != current_block->next_block && current_block->merge == SPIRBlock::MergeSelection) {
                    ...
                    bool collapsed_loop = current_block->true_block == current_block->merge_block && current_block->merge == SPIRBlock::MergeLoop;
    ...
    bool Parser::variable_storage_is_aliased(const SPIRVariable& v) const {
        ...
        bool ssbo = v.storage == StorageClassStorageBuffer || (type_meta && type_meta->decoration.decoration_flags.get(DecorationBufferBlock));
