...
namespace SPIRV_CROSS_NAMESPACE {
    ...
    template <typename T, size_t N> class AlignedBuffer {
      public:
        T* data() {
    ...
    template <typename T> class VectorView {
      public:
        T& operator[](size_t i) SPIRV_CROSS_NOEXCEPT { return ptr[i]; }

        const T& operator[](size_t i) const SPIRV_CROSS_NOEXCEPT { return ptr[i]; }

        bool empty() const SPIRV_CROSS_NOEXCEPT { return buffer_size == 0; }

        size_t size() const SPIRV_CROSS_NOEXCEPT { return buffer_size; }

        T* data() SPIRV_CROSS_NOEXCEPT { return ptr; }

        const T* data() const SPIRV_CROSS_NOEXCEPT { return ptr; }

        T* begin() SPIRV_CROSS_NOEXCEPT { return ptr; }

        T* end() SPIRV_CROSS_NOEXCEPT { return ptr + buffer_size; }

        const T* begin() const SPIRV_CROSS_NOEXCEPT { return ptr; }

        const T* end() const SPIRV_CROSS_NOEXCEPT { return ptr + buffer_size; }

        T& front() SPIRV_CROSS_NOEXCEPT { return ptr[0]; }

        const T& front() const SPIRV_CROSS_NOEXCEPT { return ptr[0]; }

        T& back() SPIRV_CROSS_NOEXCEPT { return ptr[buffer_size - 1]; }

        const T& back() const SPIRV_CROSS_NOEXCEPT { return ptr[buffer_size - 1]; }
...
#else
        // Makes it easier to consume SmallVector.
        explicit operator std::vector<T>() const& { return std::vector<T>(ptr, ptr + buffer_size); }

        // If we are converting as an r-value, we can pilfer our elements.
        explicit operator std::vector<T>() && { return std::vector<T>(std::make_move_iterator(ptr), std::make_move_iterator(ptr + buffer_size)); }
#endif
    ...
    template <typename T, size_t N = 8> class SmallVector : public VectorView<T> {
      public:
        ...
        template <typename U> SmallVector(const U* arg_list_begin, const U* arg_list_end) SPIRV_CROSS_NOEXCEPT : SmallVector() {
        ...
        template <typename U> SmallVector(std::initializer_list<U> init) SPIRV_CROSS_NOEXCEPT : SmallVector(init.begin(), init.end()) {}

        template <typename U, size_t M> explicit SmallVector(const U (&init)[M]) SPIRV_CROSS_NOEXCEPT : SmallVector(init, init + M) {}

        SmallVector(SmallVector&& other) SPIRV_CROSS_NOEXCEPT : SmallVector() { *this = std::move(other); }
        ...
        SmallVector(const SmallVector& other) SPIRV_CROSS_NOEXCEPT : SmallVector() { *this = other; }
        ...
        explicit SmallVector(size_t count) SPIRV_CROSS_NOEXCEPT : SmallVector() { resize(count); }
        ...
        template <typename... Ts> void emplace_back(Ts&&... ts) SPIRV_CROSS_NOEXCEPT {
            reserve(this->buffer_size + 1);
        ...
        void reserve(size_t count) SPIRV_CROSS_NOEXCEPT {
            if ((count > (std::numeric_limits<size_t>::max)() / sizeof(T)) || (count > (std::numeric_limits<size_t>::max)() / 2)) {
        ...
        void insert(T* itr, const T& value) SPIRV_CROSS_NOEXCEPT { insert(itr, &value, &value + 1); }
    ...
    template <typename T, size_t N = 8> using SmallVector = std::vector<T>;
    template <typename T> using Vector = std::vector<T>;
    template <typename T> using VectorView = std::vector<T>;
    ...
    template <typename T> class ObjectPool : public ObjectPoolBase {
      public:
        explicit ObjectPool(unsigned start_object_count_ = 16) : start_object_count(start_object_count_) {}

        template <typename... P> T* allocate(P&&... p) {
            if (vacants.empty()) {
        ...
        void deallocate_opaque(void* ptr) override { deallocate(static_cast<T*>(ptr)); }
    ...
    template <size_t StackSize = 4096, size_t BlockSize = 4096> class StringStream {
      public:
        StringStream() { reset(); }
        ...
        template <typename T, typename std::enable_if<!std::is_floating_point<T>::value, int>::type = 0> StringStream& operator<<(const T& t) {
        ...
        template <size_t N> StringStream& operator<<(const char (&s)[N]) {
            append(s, strlen(s));
            return *this;
        }
