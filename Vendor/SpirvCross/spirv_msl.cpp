...
CompilerMSL::CompilerMSL(std::vector<uint32_t> spirv_) : CompilerGLSL(std::move(spirv_)) {
}

CompilerMSL::CompilerMSL(const uint32_t* ir_, size_t word_count) : CompilerGLSL(ir_, word_count) {
}

CompilerMSL::CompilerMSL(const ParsedIR& ir_) : CompilerGLSL(ir_) {
}

CompilerMSL::CompilerMSL(ParsedIR&& ir_) : CompilerGLSL(std::move(ir_)) {
}
...
bool CompilerMSL::is_msl_shader_input_used(uint32_t location) {
    ...
    return location_inputs_in_use.count(location) != 0 && location_inputs_in_use_fallback.count(location) == 0;
}

bool CompilerMSL::is_msl_shader_output_used(uint32_t location) {
    // Don't report internal location allocations to app.
    return location_outputs_in_use.count(location) != 0 && location_outputs_in_use_fallback.count(location) == 0;
}
...
void CompilerMSL::build_implicit_builtins() {
    ...
    bool need_vertex_params = capture_output_to_buffer && get_execution_model() == ExecutionModelVertex && !msl_options.vertex_for_tessellation;
    bool need_tesc_params = is_tesc_shader();
    bool need_tese_params = is_tese_shader() && msl_options.raw_buffer_tese_input;
    bool need_subgroup_mask = active_input_builtins.get(BuiltInSubgroupEqMask) || active_input_builtins.get(BuiltInSubgroupGeMask) ||
    ...
    bool need_subgroup_ge_mask =
        !msl_options.is_ios() && (active_input_builtins.get(BuiltInSubgroupGeMask) || active_input_builtins.get(BuiltInSubgroupGtMask));
    bool need_multiview = get_execution_model() == ExecutionModelVertex && !msl_options.view_index_from_device_index &&
                          msl_options.multiview_layered_rendering && (msl_options.multiview || active_input_builtins.get(BuiltInViewIndex));
    bool need_dispatch_base = msl_options.dispatch_base && get_execution_model() == ExecutionModelGLCompute &&
    ...
    bool need_vertex_base_params = need_grid_params && (active_input_builtins.get(BuiltInVertexId) || active_input_builtins.get(BuiltInVertexIndex) ||
    ...
    bool force_frag_depth_passthrough = get_execution_model() == ExecutionModelFragment && !uses_explicit_early_fragment_test() && need_subpass_input &&
                                        msl_options.enable_frag_depth_builtin && msl_options.input_attachment_is_ds_attachment;
    needs_point_size_output = msl_options.enable_point_size_builtin && msl_options.enable_point_size_default && entry_point_is_vertex();

    if (need_subpass_input || need_sample_pos || need_subgroup_mask || need_vertex_params || need_tesc_params || need_tese_params || need_multiview ||
        need_dispatch_base || need_vertex_base_params || need_grid_params || needs_sample_id || needs_subgroup_invocation_id || needs_subgroup_size ||
        needs_helper_invocation || has_additional_fixed_sample_mask() || need_local_invocation_index || need_workgroup_size || force_frag_depth_passthrough ||
        needs_point_size_output || is_mesh_shader()) {
        ...
        ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
            ...
            if (builtin == BuiltInPrimitivePointIndicesEXT || builtin == BuiltInPrimitiveLineIndicesEXT || builtin == BuiltInPrimitiveTriangleIndicesEXT) {
            ...
            // was used.
            if (need_dispatch_base && workgroup_id_type == 0 && (builtin == BuiltInWorkgroupId || builtin == BuiltInGlobalInvocationId))
                workgroup_id_type = var.basetype;
        });

        // Use Metal's native frame-buffer fetch API for subpass inputs.
        if ((!has_frag_coord || (msl_options.multiview && !has_view_idx) || (msl_options.arrayed_subpass_input && !msl_options.multiview && !has_layer)) &&
        ...
        if ((need_tesc_params && (msl_options.multi_patch_workgroup || !has_invocation_id || !has_primitive_id)) || (need_tese_params && !has_primitive_id) ||
            need_grid_params) {
            uint32_t type_ptr_id = ir.increase_bound_by(1);
        ...
        if (need_dispatch_base || need_vertex_base_params) {
            ...
            if (msl_options.supports_msl_version(1, 2)) {
                ...
                set_extended_decoration(var_id, SPIRVCrossDecorationResourceIndexPrimary, msl_options.indirect_params_buffer_index);
            }
        ...
        if (need_workgroup_size && !has_workgroup_size) {
            ...
            if (builtin_workgroup_size_id == 0) {
                ...
                if (execution.flags.get(ExecutionModeLocalSizeId)) {
                    const SPIRConstant* init[] = {&get<SPIRConstant>(execution.workgroup_size.id_x), &get<SPIRConstant>(execution.workgroup_size.id_y),
                    ...
                    uint32_t offset = ir.increase_bound_by(3);
                    const SPIRConstant* init[] = {&set<SPIRConstant>(offset, get_uint_type_id(), execution.workgroup_size.x, false),
    ...
    if (!buffers_requiring_dynamic_offset.empty()) {
        ...
        set_extended_decoration(var_id, SPIRVCrossDecorationResourceIndexPrimary, msl_options.dynamic_offsets_buffer_index);
    ...
    bool need_position = (get_execution_model() == ExecutionModelVertex || is_tese_shader()) && !capture_output_to_buffer && !get_is_rasterization_disabled() &&
                         !msl_options.auto_disable_rasterization && !active_output_builtins.get(BuiltInPosition);
...
void CompilerMSL::emit_entry_point_declarations() {
    ...
    for (auto& samp : constexpr_samplers_by_id) {
        ...
        if (args.empty())
            statement("constexpr sampler ", type.basetype == SPIRType::SampledImage ? to_sampler_expression(samp.first) : to_name(samp.first), ";");
        else
            statement("constexpr sampler ", type.basetype == SPIRType::SampledImage ? to_sampler_expression(samp.first) : to_name(samp.first), "(", merge(args),
                      ");");
    }
    ...
    for (auto& dynamic_buffer : buffers_requiring_dynamic_offset) {
        ...
        if (is_array(type)) {
            ...
            statement(get_variable_address_space(var), " ", type_to_glsl(type), "* ", to_restrict(var_id, true), name, type_to_array_glsl(type, var_id), " =");
            ...
            for (uint32_t i = 0; i < array_size; i++) {
                statement("(", get_variable_address_space(var), " ", type_to_glsl(type), "* ", to_restrict(var_id, false), ")((",
                          get_variable_address_space(var), " char* ", to_restrict(var_id, false), ")", to_name(arg_id), ".", dynamic_buffer.second.mbr_name,
            ...
            statement(get_variable_address_space(var), " auto& ", to_restrict(var_id, true), name, " = *(", get_variable_address_space(var), " ",
                      type_to_glsl(type), "* ", to_restrict(var_id, false), ")((", get_variable_address_space(var), " char* ", to_restrict(var_id, false), ")",
                      to_name(arg_id), ".", dynamic_buffer.second.mbr_name, " + ", to_name(dynamic_offsets_buffer_id), "[", base_index, "]);");
        }
    }
    ...
    for (SPIRVariable* arg : entry_point_bindings) {
        ...
        if (is_var_runtime_size_array(var)) {
            ...
            if (descriptor_set_is_argument_buffer(get_decoration(var.self, DecorationDescriptorSet))) {
                ...
                bool is_aliased = std::find_if(buffer_aliases_discrete.begin(), buffer_aliases_discrete.end(), [&](uint32_t id) { return var.self == id; }) !=
                                  buffer_aliases_discrete.end();
            ...
            case SPIRType::Struct:
                statement("spvDescriptorArray<", get_variable_address_space(var), " ", type_to_glsl(buffer_type), "*> ", name, " {", resource_name, "};");
                break;
            default:
                break;
            }
            ...
            statement(get_variable_address_space(var), " ", type_to_glsl(buffer_type), "* ", to_restrict(var.self, true), name, "[] =");
            begin_scope();
    ...
    for (auto& var_id : buffer_aliases_discrete) {
        ...
        statement(addr_space, " auto& ", to_restrict(var_id, true), name, " = *(", addr_space, " ", type_to_glsl(type), "*)", alias_name, ";");
    }
...
void CompilerMSL::preprocess_op_codes() {
    ...
    if ((preproc.uses_buffer_write && !msl_options.supports_msl_version(2, 1)) || (preproc.uses_image_write && !msl_options.supports_msl_version(2, 2)))
        is_rasterization_disabled = true;
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
        ...
        if (var.storage == StorageClassInput && has_decoration(var.self, DecorationBuiltIn)) {
            ...
            if (bi_type == BuiltInHelperInvocation && needs_manual_helper_invocation_updates()) {
                ...
                entry_func.fixup_hooks_in.push_back([this, &var]() { statement(to_name(var.self), " = simd_is_helper_thread();"); });
            }
        }

        if (var.storage == StorageClassInput || var.storage == StorageClassOutput || var.storage == StorageClassUniform ||
            var.storage == StorageClassUniformConstant || var.storage == StorageClassPushConstant || var.storage == StorageClassStorageBuffer) {
...
void CompilerMSL::extract_global_variables_from_function(uint32_t func_id, std::set<uint32_t>& added_arg_ids, unordered_set<uint32_t>& global_var_ids,
                                                         unordered_set<uint32_t>& processed_func_ids) {
    ...
    for (auto block : func.blocks) {
        ...
        for (auto& i : b.ops) {
            ...
            case OpArrayLength: {
                ...
                if (type.basetype == SPIRType::Image && type.image.dim == DimSubpassData && (!msl_options.use_framebuffer_fetch_subpasses)) {
            ...
            case OpImageTexelPointer: {
                ...
                if (var) {
                    if (atomic_image_vars_emulated.count(var->self) && !get<SPIRType>(var->basetype).array.empty()) {
            ...
            case OpGroupNonUniformRotateKHR: {
                ...
                if (i.length > 5)
                    added_arg_ids.insert(static_cast<Scope>(evaluate_constant_u32(ops[2])) == ScopeSubgroup ? builtin_subgroup_invocation_id_id
            ...
            case OpGroupNonUniformLogicalXor:
                if ((get_execution_model() != ExecutionModelFragment || msl_options.supports_msl_version(2, 2)) && ops[3] == GroupOperationClusteredReduce)
                    added_arg_ids.insert(builtin_subgroup_invocation_id_id);
                break;
            ...
            if (needs_manual_helper_invocation_updates() && b.terminator == SPIRBlock::Kill && needs_helper_invocation)
                added_arg_ids.insert(builtin_helper_invocation_id);
    ...
    if (func_id != ir.default_entry_point) {
        ...
        for (uint32_t arg_id : added_arg_ids) {
            ...
            bool is_control_point_storage =
                !is_patch && ((is_tessellation_shader() && var.storage == StorageClassInput) || (is_tesc_shader() && var.storage == StorageClassOutput));
            bool is_patch_block_storage = is_patch && is_block && var.storage == StorageClassOutput;
            bool is_builtin = is_builtin_variable(var);
            bool variable_is_stage_io = !is_builtin || bi_type == BuiltInPosition || bi_type == BuiltInPointSize || bi_type == BuiltInClipDistance ||
                                        bi_type == BuiltInCullDistance || p_type->basetype == SPIRType::Struct;
            bool is_redirected_to_global_stage_io = (is_control_point_storage || is_patch_block_storage) && variable_is_stage_io;
...
void CompilerMSL::mark_packable_structs() {
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, SPIRVariable& var) {
        if (var.storage != StorageClassFunction && !is_hidden_variable(var)) {
            auto& type = this->get<SPIRType>(var.basetype);
            if (type.pointer &&
                (type.storage == StorageClassUniform || type.storage == StorageClassUniformConstant || type.storage == StorageClassPushConstant ||
...
void CompilerMSL::mark_location_as_used_by_shader(uint32_t location, const SPIRType& type, StorageClass storage, bool fallback) {
...
bool CompilerMSL::add_component_variable_to_interface_block(StorageClass storage, const std::string& ib_var_ref, SPIRVariable& var, const SPIRType& type,
        ...
        if (var.storage == StorageClassInput) {
            entry_func.fixup_hooks_in.push_back([=, &type, &var]() {
                if (!type.array.empty()) {
                    ...
                    for (uint32_t loc_off = 0; loc_off < array_size; loc_off++) {
                        statement(to_name(var.self), "[", loc_off, "]", " = ", ib_var_ref, ".m_location_", location + loc_off,
                                  vector_swizzle(type_components, start_component), ";");
                    }
                } else {
                    statement(to_name(var.self), " = ", ib_var_ref, ".m_location_", location, vector_swizzle(type_components, start_component), ";");
                }
            });
        } else {
            entry_func.fixup_hooks_out.push_back([=, &type, &var]() {
                if (!type.array.empty()) {
                    uint32_t array_size = to_array_size_literal(type);
                    for (uint32_t loc_off = 0; loc_off < array_size; loc_off++) {
                        statement(ib_var_ref, ".m_location_", location + loc_off, vector_swizzle(type_components, start_component), " = ", to_name(var.self),
                                  "[", loc_off, "];");
                    }
                } else {
                    statement(ib_var_ref, ".m_location_", location, vector_swizzle(type_components, start_component), " = ", to_name(var.self), ";");
                }
            });
        }
        return true;
...
void CompilerMSL::add_plain_variable_to_interface_block(StorageClass storage, const string& ib_var_ref, SPIRType& ib_type, SPIRVariable& var,
                                                        InterfaceBlockMeta& meta) {
    bool is_builtin = is_builtin_variable(var);
    ...
    if (padded_output || padded_input) {
        ...
        if (padded_output) {
            entry_func.fixup_hooks_out.push_back(
                [=, &var]() { statement(qual_var_name, vector_swizzle(type_components, start_component), " = ", to_name(var.self), ";"); });
        } else {
            entry_func.fixup_hooks_in.push_back(
                [=, &var]() { statement(to_name(var.self), " = ", qual_var_name, vector_swizzle(type_components, start_component), ";"); });
        }
    ...
    if (var.storage == StorageClassOutput && var.initializer != ID(0)) {
        if (padded_output || padded_input) {
            entry_func.fixup_hooks_in.push_back([=, &var]() { statement(to_name(var.self), " = ", to_expression(var.initializer), ";"); });
        } else {
            if (meta.strip_array) {
                entry_func.fixup_hooks_in.push_back([=, &var]() {
                    ...
                    statement(to_expression(stage_out_ptr_var_id), "[", invocation, "].", to_member_name(ib_type, index), " = ", to_expression(var.initializer),
                              "[", invocation, "];");
                });
            } else {
                entry_func.fixup_hooks_in.push_back([=, &var]() { statement(qual_var_name, " = ", to_expression(var.initializer), ";"); });
            }
        }
    }
...
void CompilerMSL::add_composite_variable_to_interface_block(StorageClass storage, const string& ib_var_ref, SPIRType& ib_type, SPIRVariable& var,
                                                            InterfaceBlockMeta& meta) {
    auto& entry_func = get<SPIRFunction>(ir.default_entry_point);
            ...
            case StorageClassOutput:
                entry_func.fixup_hooks_out.push_back([=, &var]() {
                    if (padded_output) {
                        ...
                        statement(ib_var_ref, ".", mbr_name, " = ", remap_swizzle(padded_type, usable_type->vecsize, join(to_name(var.self), "[", i, "]")),
                                  ";");
...
void CompilerMSL::add_composite_member_variable_to_interface_block(StorageClass storage, const string& ib_var_ref, SPIRType& ib_type, SPIRVariable& var,
                                                                   SPIRType& var_type, uint32_t mbr_idx, InterfaceBlockMeta& meta, const string& mbr_name_qual,
                                                                   const string& var_chain_qual, uint32_t& location, uint32_t& var_mbr_idx,
    ...
    bool is_builtin = is_member_builtin(var_type, mbr_idx, &builtin);
    bool is_flat =
        interpolation_qual.get(DecorationFlat) || has_member_decoration(var_type.self, mbr_idx, DecorationFlat) || has_decoration(var.self, DecorationFlat);
    bool is_noperspective = interpolation_qual.get(DecorationNoPerspective) || has_member_decoration(var_type.self, mbr_idx, DecorationNoPerspective) ||
                            has_decoration(var.self, DecorationNoPerspective);
    bool is_centroid = interpolation_qual.get(DecorationCentroid) || has_member_decoration(var_type.self, mbr_idx, DecorationCentroid) ||
                       has_decoration(var.self, DecorationCentroid);
    bool is_sample = interpolation_qual.get(DecorationSample) || has_member_decoration(var_type.self, mbr_idx, DecorationSample) ||
    ...
    if (mbr_type.basetype == SPIRType::Struct) {
        for (uint32_t i = 0; i < elem_cnt; i++) {
            ...
            for (uint32_t sub_mbr_idx = 0; sub_mbr_idx < sub_mbr_cnt; sub_mbr_idx++) {
                add_composite_member_variable_to_interface_block(storage, ib_var_ref, ib_type, var, mbr_type, sub_mbr_idx, meta, mbr_name, var_chain, location,
                                                                 var_mbr_idx, inherited_qual);
...
void CompilerMSL::add_plain_member_variable_to_interface_block(StorageClass storage, const string& ib_var_ref, SPIRType& ib_type, SPIRVariable& var,
                                                               SPIRType& var_type, uint32_t mbr_idx, InterfaceBlockMeta& meta, const string& mbr_name_qual,
                                                               const string& var_chain_qual, uint32_t& location, uint32_t& var_mbr_idx) {
    auto& entry_func = get<SPIRFunction>(ir.default_entry_point);
    ...
    BuiltIn builtin = BuiltInMax;
    bool is_builtin = is_member_builtin(var_type, mbr_idx, &builtin);
    bool is_flat = has_member_decoration(var_type.self, mbr_idx, DecorationFlat) || has_decoration(var.self, DecorationFlat);
    bool is_noperspective = has_member_decoration(var_type.self, mbr_idx, DecorationNoPerspective) || has_decoration(var.self, DecorationNoPerspective);
    bool is_centroid = has_member_decoration(var_type.self, mbr_idx, DecorationCentroid) || has_decoration(var.self, DecorationCentroid);
    bool is_sample = has_member_decoration(var_type.self, mbr_idx, DecorationSample) || has_decoration(var.self, DecorationSample);
    ...
    if (is_builtin && !meta.strip_array) {
        ...
        case StorageClassOutput:
            ...
            entry_func.fixup_hooks_out.push_back([=]() { statement(qual_var_name, " = ", var_chain, ";"); });
            break;
    ...
    if (!flatten_stage_out && var.storage == StorageClassOutput && var.initializer != ID(0) && (c = maybe_get<SPIRConstant>(var.initializer))) {
        if (meta.strip_array) {
            entry_func.fixup_hooks_in.push_back([=, &var]() {
                auto& type = this->get<SPIRType>(var.basetype);
                ...
                statement(to_expression(stage_out_ptr_var_id), "[", invocation, "].", to_member_name(ib_type, index), " = ", constant_chain, ".",
...
void CompilerMSL::add_tess_level_input_to_interface_block(const std::string& ib_var_ref, SPIRType& ib_type, SPIRVariable& var) {
    auto& var_type = get_variable_element_type(var);
...
bool CompilerMSL::variable_storage_requires_stage_io(StorageClass storage) const {
    if (storage == StorageClassOutput)
        ...
        return !(is_tesc_shader() && msl_options.multi_patch_workgroup) && !(is_tese_shader() && msl_options.raw_buffer_tese_input);
    else
        return false;
}
...
void CompilerMSL::emit_local_masked_variable(const SPIRVariable& masked_var, bool strip_array) {
    ...
    if (threadgroup_storage && msl_options.multi_patch_workgroup) {
        ...
        entry_func.fixup_hooks_in.push_back([this, &masked_var]() {
            ...
            uint32_t max_num_instances = (max_control_points_per_patch + get_entry_point().output_vertices - 1u) / get_entry_point().output_vertices;
            statement("threadgroup ", type_to_glsl(type), " ", "spvStorage", to_name(masked_var.self), "[", max_num_instances, "]", type_to_array_glsl(type, 0),
                      ";");
            ...
            statement("threadgroup auto ", "&", to_name(masked_var.self), " = spvStorage", to_name(masked_var.self), "[", "(",
                      to_expression(builtin_invocation_id_id), ".x / ", get_entry_point().output_vertices, ") % ", max_num_instances, "];");
        });
    } else {
    ...
    if (!threadgroup_storage) {
        ...
        if (strip_array) {
            entry_func.fixup_hooks_in.push_back([this, &masked_var, initializer]() {
                auto invocation = to_tesc_invocation_id();
                statement(to_expression(masked_var.self), "[", invocation, "] = ", to_expression(initializer), "[", invocation, "];");
            });
        } else {
            entry_func.fixup_hooks_in.push_back(
                [this, &masked_var, initializer]() { statement(to_expression(masked_var.self), " = ", to_expression(initializer), ";"); });
        }
    }
}

void CompilerMSL::add_variable_to_interface_block(StorageClass storage, const string& ib_var_ref, SPIRType& ib_type, SPIRVariable& var,
                                                  InterfaceBlockMeta& meta) {
    auto& entry_func = get<SPIRFunction>(ir.default_entry_point);
    ...
    if (storage == StorageClassInput && var_type.basetype == SPIRType::Struct &&
        ((is_tesc_shader() && msl_options.multi_patch_workgroup) || (is_tese_shader() && msl_options.raw_buffer_tese_input)) &&
    ...
    if (var_type.basetype == SPIRType::Struct) {
        bool block_requires_flattening = variable_storage_requires_stage_io(storage) || (is_block && var_type.array.empty());
        ...
        if (!block_requires_flattening) {
                ...
                for (uint32_t mbr_idx = 0; mbr_idx < uint32_t(var_type.member_types.size()); mbr_idx++) {
                    ...
                    if (storage == StorageClassOutput && is_stage_output_block_member_masked(var, mbr_idx, meta.strip_array)) {
                        ...
                        if ((!is_builtin || attribute_load_store) && storage_is_stage_io && is_composite_type) {
                            add_composite_member_variable_to_interface_block(storage, ib_var_ref, ib_type, var, var_type, mbr_idx, meta, mbr_name_qual,
                                                                             var_chain_qual, location, var_mbr_idx, {});
                        } else {
                            add_plain_member_variable_to_interface_block(storage, ib_var_ref, ib_type, var, var_type, mbr_idx, meta, mbr_name_qual,
                                                                         var_chain_qual, location, var_mbr_idx);
                        }
                    }
                    var_mbr_idx++;
            ...
            if (masked_block && !needs_local_declaration && (!is_builtin_variable(var) || is_tesc_shader())) {
                if (is_builtin_variable(var)) {
                    ...
                    for (uint32_t mbr_idx = 0; mbr_idx < uint32_t(var_type.member_types.size()); mbr_idx++) {
                        set_member_name(var_type.self, mbr_idx,
                                        builtin_to_glsl(BuiltIn(get_member_decoration(var_type.self, mbr_idx, DecorationBuiltIn)), StorageClassOutput));
                    }
    ...
    } else if (var_type.basetype == SPIRType::Boolean || var_type.basetype == SPIRType::Char || type_is_integral(var_type) ||
               type_is_floating_point(var_type)) {
...
void CompilerMSL::fix_up_interface_member_indices(StorageClass storage, uint32_t ib_type_id) {
    ...
    if (!is_tesc_shader() && !(is_tese_shader() && storage == StorageClassInput) &&
        !(get_execution_model() == ExecutionModelFragment && storage == StorageClassInput && !pull_model_inputs.empty()))
        return;
...
uint32_t CompilerMSL::add_interface_block(StorageClass storage, bool patch) {
    ...
    bool pack_components = (storage == StorageClassInput && get_execution_model() == ExecutionModelVertex) ||
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t var_id, SPIRVariable& var) {
        ...
        if (is_builtin && !is_block) {
            ...
            builtin_is_gl_in_out =
                bi_type == BuiltInPosition || bi_type == BuiltInPointSize || bi_type == BuiltInClipDistance || bi_type == BuiltInCullDistance;
        }
        ...
        bool builtin_is_stage_in_out = builtin_is_gl_in_out || bi_type == BuiltInLayer || bi_type == BuiltInViewportIndex || bi_type == BuiltInBaryCoordKHR ||
                                       bi_type == BuiltInBaryCoordNoPerspKHR || bi_type == BuiltInFragDepth || bi_type == BuiltInFragStencilRefEXT ||
                                       bi_type == BuiltInSampleMask;

        // These builtins are part of the stage in/out structs.
        bool is_interface_block_builtin = builtin_is_stage_in_out || (is_tese_shader() && !msl_options.raw_buffer_tese_input &&
        ...
        if (is_active && !hidden && type.pointer && filter_patch_decoration && (!is_builtin || is_interface_block_builtin)) {
            vars.push_back(&var);
        ...
        if (is_tese_shader() && msl_options.raw_buffer_tese_input && patch && storage == StorageClassInput &&
            ...
            const char* mbr_name = bi_type == BuiltInTessLevelOuter ? "edgeTessellationFactor" : "insideTessellationFactor";
    ...
    if (vars.empty() && !(!msl_options.raw_buffer_tese_input && storage == StorageClassInput && patch && stage_in_var_id))
        return 0;
    ...
    case StorageClassInput:
        ...
        case ExecutionModelTessellationControl:
            ...
            entry_func.fixup_hooks_in.push_back([=]() {
                ...
                if (msl_options.multi_patch_workgroup) {
                    // n.b. builtin_invocation_id_id here is the dispatch global invocation ID,
                    // not the TC invocation ID.
                    statement("device ", to_name(ir.default_entry_point), "_", ib_var_ref, "* gl_in = &", input_buffer_var_name, "[min(",
                              to_expression(builtin_invocation_id_id), ".x / ", get_entry_point().output_vertices,
        ...
        case ExecutionModelTessellationEvaluation:
            ...
            if (patch) {
                entry_func.fixup_hooks_in.push_back([=]() {
                    statement("const device ", to_name(ir.default_entry_point), "_", ib_var_ref, "& ", ib_var_ref, " = ", patch_input_buffer_var_name, "[",
                              to_expression(builtin_primitive_id_id), "];");
                });
            } else {
                entry_func.fixup_hooks_in.push_back([=]() {
                    statement("const device ", to_name(ir.default_entry_point), "_", ib_var_ref, "* gl_in = &", input_buffer_var_name, "[",
                              to_expression(builtin_primitive_id_id), " * ", get_entry_point().output_vertices, "];");
                });
            }
            break;
        default:
        ...
        if (!capture_output_to_buffer) {
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    if (stage_out_var_id) {
                        ...
                        if (get_execution_model() == ExecutionModelVertex && msl_options.vertex_for_tessellation) {
                            statement("device ", to_name(ir.default_entry_point), "_", ib_var_ref, "& ", ib_var_ref, " = ", output_buffer_var_name, "[",
                                      to_expression(builtin_invocation_id_id), ".y * ", to_expression(builtin_stage_input_size_id), ".x + ",
                                      to_expression(builtin_invocation_id_id), ".x];");
                        } else if (msl_options.enable_base_index_zero) {
                            statement("device ", to_name(ir.default_entry_point), "_", ib_var_ref, "& ", ib_var_ref, " = ", output_buffer_var_name, "[",
                                      to_expression(builtin_instance_idx_id), " * spvIndirectParams[0] + ", to_expression(builtin_vertex_idx_id), "];");
                        } else {
                            statement("device ", to_name(ir.default_entry_point), "_", ib_var_ref, "& ", ib_var_ref, " = ", output_buffer_var_name, "[(",
                                      to_expression(builtin_instance_idx_id), " - ", to_expression(builtin_base_instance_id), ") * spvIndirectParams[0] + ",
            ...
            case ExecutionModelTessellationControl:
                if (msl_options.multi_patch_workgroup) {
                    ...
                    if (patch) {
                        entry_func.fixup_hooks_in.push_back([=]() {
                            statement("device ", to_name(ir.default_entry_point), "_", ib_var_ref, "& ", ib_var_ref, " = ", patch_output_buffer_var_name, "[",
                                      to_expression(builtin_invocation_id_id), ".x / ", get_entry_point().output_vertices, "];");
                        });
                    } else {
                        entry_func.fixup_hooks_in.push_back([=]() {
                            statement("device ", to_name(ir.default_entry_point), "_", ib_var_ref, "* gl_out = &", output_buffer_var_name, "[",
                                      to_expression(builtin_invocation_id_id), ".x - ", to_expression(builtin_invocation_id_id), ".x % ",
    ...
    if (((is_tesc_shader() && msl_options.multi_patch_workgroup) || (is_tese_shader() && msl_options.raw_buffer_tese_input)) && storage == StorageClassInput) {
    ...
    if (is_tese_shader() && !msl_options.raw_buffer_tese_input && storage == StorageClassInput && patch && stage_in_var_id) {
...
uint32_t CompilerMSL::ensure_correct_builtin_type(uint32_t type_id, BuiltIn builtin) {
    ...
    if ((builtin == BuiltInSampleMask && is_array(pointee_type)) ||
        ((builtin == BuiltInLayer || builtin == BuiltInViewportIndex || builtin == BuiltInFragStencilRefEXT) && pointee_type.basetype != SPIRType::UInt)) {
...
uint32_t CompilerMSL::ensure_correct_input_type(uint32_t type_id, uint32_t location, uint32_t component, uint32_t num_components, bool strip_array) {
    ...
    case MSL_SHADER_VARIABLE_FORMAT_UINT8: {
        ...
        case SPIRType::Short:
            return build_extended_vector_type(type_id, num_components > type.vecsize ? num_components : type.vecsize, SPIRType::UShort);
        case SPIRType::Int:
            return build_extended_vector_type(type_id, num_components > type.vecsize ? num_components : type.vecsize, SPIRType::UInt);
...
void CompilerMSL::emit_store_statement(uint32_t lhs_expression, uint32_t rhs_expression) {
    ...
    if (has_decoration(lhs_expression, DecorationBuiltIn) && BuiltIn(get_decoration(lhs_expression, DecorationBuiltIn)) == BuiltInSampleMask &&
        is_array(type)) {
        ...
        if (is_matrix(type) && lhs_e && lhs_e->need_transpose) {
            ...
            for (uint32_t c = 0; c < type.vecsize; c++) {
                ...
                if (column_index != string::npos) {
                    statement(lhs_expr.insert(column_index, join('[', c, ']')), " = ", to_extract_component_expression(rhs_expression, c), ";");
                }
            }
            lhs_e->need_transpose = true;
            register_write(lhs_expression);
        ...
        TypeID physical_type_id = lhs_remapped_type ? ID(get_extended_decoration(lhs_expression, SPIRVCrossDecorationPhysicalTypeID)) : type.self;
        ...
        if (is_matrix(type)) {
            ...
            if (transpose) {
                ...
                if (rhs_transpose) {
                    ...
                    for (uint32_t i = 0; i < type.columns; i++) {
                        statement(cast_expr, to_enclosed_expression(lhs_expression), "[", i, "]", " = ", to_enclosed_unpacked_expression(rhs_expression), "[",
                                  i, "];");
                    }
                }
            }
            ...
            for (uint32_t c = 0; c < type.vecsize; c++) {
                ...
                if (column_index != string::npos) {
                    statement("((", cast_addr_space, " ", type_to_glsl(write_type), "*)&", lhs_expr.insert(column_index, join('[', c, ']', ")")), " = ",
                              to_extract_component_expression(rhs_expression, c), ";");
                }
            }

            lhs_e->need_transpose = true;
        } else if ((is_matrix(physical_type) || is_array(physical_type)) && physical_type.vecsize <= 4 && physical_type.vecsize > type.vecsize) {
...
string CompilerMSL::unpack_expression_type(string expr_str, const SPIRType& type, uint32_t physical_type_id, bool packed, bool row_major) {
    ...
    if (physical_type && is_vector(*physical_type) && is_array(*physical_type) && !uses_std140_wrapper && physical_type->vecsize > type.vecsize &&
        !expression_ends_with(expr_str, swizzle_lut[type.vecsize - 1])) {
        // std140 array cases for vectors.
        assert(type.vecsize >= 1 && type.vecsize <= 3);
        return enclose_expression(expr_str) + swizzle_lut[type.vecsize - 1];
    } else if (physical_type && is_matrix(*physical_type) && is_vector(type) && !uses_std140_wrapper && physical_type->vecsize > type.vecsize) {
...
void CompilerMSL::emit_custom_templates() {
    static const char* const address_spaces[] = {"thread", "constant", "device", "threadgroup", "threadgroup_imageblock", "ray_data", "object_data"};
    ...
    for (const auto& spv_func : spv_function_implementations) {
        ...
        case SPVFuncImplUnsafeArray:
            ...
            statement("return elements[pos];");
            end_scope();
            if (get_execution_model() == ExecutionModelMeshEXT || get_execution_model() == ExecutionModelTaskEXT) {
                statement("");
        ...
        case SPVFuncImplStorageMatrix:
            ...
            for (size_t method_idx = 0; method_idx < sizeof(address_spaces) / sizeof(address_spaces[0]); ++method_idx) {
                ...
                {
                    statement(method_as, " spvStorageMatrix& operator=(initializer_list<vec<T, Rows>> cols) ", method_as);
                    begin_scope();
                    statement("size_t i;");
                ...
                for (size_t param_idx = 0; param_idx < sizeof(address_spaces) / sizeof(address_spaces[0]); ++param_idx) {
                    ...
                    if (method_idx != 1) // constant
                    {
                        statement(method_as, " spvStorageMatrix& operator=(const ", param_as, " matrix<T, Cols, Rows>& m) ", method_as);
                        begin_scope();
                        statement("for (size_t i = 0; i < Cols; ++i)");
                        statement("    columns[i] = m.columns[i];");
                        statement("return *this;");
                        end_scope();
                        statement(method_as, " spvStorageMatrix& operator=(const ", param_as, " spvStorageMatrix& m) ", method_as, " = default;");
                    }
...
void CompilerMSL::emit_custom_functions() {
    ...
    if (spv_function_implementations.count(SPVFuncImplDynamicImageSampler)) {
        ...
        if (!msl_options.supports_msl_version(2))
            SPIRV_CROSS_THROW("spvDynamicImageSampler requires default-constructible texture objects, which require MSL 2.0.");
        ...
        for (uint32_t i = SPVFuncImplChromaReconstructNearest2Plane; i <= SPVFuncImplChromaReconstructLinear420XMidpointYMidpoint3Plane; i++)
    ...
    if (spv_function_implementations.count(SPVFuncImplGatherSwizzle) || spv_function_implementations.count(SPVFuncImplGatherConstOffsets)) {
        spv_function_implementations.insert(SPVFuncImplGatherReturn);
    }

    if (spv_function_implementations.count(SPVFuncImplGatherCompareSwizzle) || spv_function_implementations.count(SPVFuncImplGatherCompareConstOffsets)) {
        spv_function_implementations.insert(SPVFuncImplGatherCompareReturn);
    }

    if (spv_function_implementations.count(SPVFuncImplTextureSwizzle) || spv_function_implementations.count(SPVFuncImplGatherSwizzle) ||
    ...
    for (const auto& spv_func : spv_function_implementations) {
        ...
        case SPVFuncImplArrayCopyMultidim: {
            ...
            static const char* function_name_tags[] = {
                "FromConstantToStack",    "FromConstantToThreadGroup",    "FromStackToStack",   "FromStackToThreadGroup",
                "FromThreadGroupToStack", "FromThreadGroupToThreadGroup", "FromDeviceToDevice", "FromConstantToDevice",
                "FromStackToDevice",      "FromThreadGroupToDevice",      "FromDeviceToStack",  "FromDeviceToThreadGroup",
            };
            ...
            static const char* dst_address_space[] = {
                "thread", "threadgroup", "thread", "threadgroup", "thread", "threadgroup", "device", "device", "device", "device", "thread", "threadgroup",
            };
            ...
            for (uint32_t variant = 0; variant < 12; variant++) {
                ...
                statement("inline void spvArrayCopy", function_name_tags[variant], "(", dst_address_space[variant], " T (&dst)", dim, ", ",
                ...
                if (spv_function_implementations.count(SPVFuncImplArrayCopyExtendedSrc) && src_is_physical_with_mismatch[variant]) {
                    ...
                    statement("inline enable_if_t<Vdst != Vsrc> spvArrayCopy", function_name_tags[variant], "(", dst_address_space[variant],
                              " vec<T, Vdst> (&dst)", dim, ", ", src_address_space[variant], " vec<T, Vsrc> (&src)", dim, ")");
                    begin_scope();
                    statement("for (uint i = 0; i < N; i++)");
                    begin_scope();
                    if (is_multidim)
                ...
                if (spv_function_implementations.count(SPVFuncImplArrayCopyExtendedDst) && dst_is_physical_with_mismatch[variant]) {
                    // 1st overload, src can be magic vector where dst is a scalar.
                    // Need reinterpret casts to be memory model correct. LLVM vectors are broken otherwise.
                    statement("template<typename T, uint V, uint N", is_multidim ? ", uint M>" : ">");
                    statement("inline void spvArrayCopy", function_name_tags[variant], "(", dst_address_space[variant], " vec<T, V> (&dst)", dim, ", ",
                              src_address_space[variant], " T (&src)", dim, ")");
                    begin_scope();
                    statement("for (uint i = 0; i < N; i++)");
                    begin_scope();
        ...
        case SPVFuncImplImage2DAtomicCoords: {
            if (msl_options.supports_msl_version(1, 2)) {
                ...
                statement("constant uint spvLinearTextureAlignmentOverride [[function_constant(", msl_options.r32ui_alignment_constant_id, ")]];");
                statement("constant uint spvLinearTextureAlignment = ", "is_function_constant_defined(spvLinearTextureAlignmentOverride) ? ",
            ...
            statement("#define spvImage2DAtomicCoord(tc, tex) (((((tex).get_width() + ", " spvLinearTextureAlignment / 4 - 1) & ~(",
        ...
        case SPVFuncImplGatherConstOffsets:
            ...
            for (uint32_t i = 0; i < texture_addr_space_count; i++) {
                ...
                statement("inline spvGatherReturn<Tex, Tp...> spvGatherConstOffsets(const ", texture_addr_spaces[i],
                          " Tex& t, sampler s, "
        ...
        case SPVFuncImplGatherCompareConstOffsets:
            ...
            for (uint32_t i = 0; i < texture_addr_space_count; i++) {
                statement("// Wrapper function that processes a ", texture_addr_spaces[i], " texture gather with a constant offset array.");
                statement("template<typename Tex, typename Toff, typename... Tp>");
                statement("inline spvGatherCompareReturn<Tex, Tp...> spvGatherCompareConstOffsets(const ", texture_addr_spaces[i],
                          " Tex& t, sampler s, "
...
#undef BINFUNC
        ...
        case SPVFuncImplDynamicImageSampler:
            ...
            statement("");
            statement("constexpr static constant ushort resolution_mask = ((1 << resolution_bits) - 1) << resolution_base;");
            ...
            statement("constexpr spvDynamicImageSampler(texture2d<T> plane0, texture2d<T> plane1, texture2d<T> plane2,");
            statement("                                 sampler samp, spvYCbCrSampler ycbcr_samp, uint sw) thread :");
            ...
            end_scope();
            statement("vec<T, 4> sample(float2 coord, gradient2d grad, min_lod_clamp min_lod, int2 offset) const thread");
            begin_scope();
        ...
        case SPVFuncImplVariableDescriptorArray:
            ...
            if (msl_options.runtime_array_rich_descriptor && spv_function_implementations.count(SPVFuncImplVariableSizedDescriptor) != 0) {
                statement("template<typename T>");
...
static string inject_top_level_storage_qualifier(const string& expr, const string& qualifier) {
    ...
    if (last_significant == string::npos)
        ...
        return join(expr.substr(0, last_significant + 1), " ", qualifier, expr.substr(last_significant + 1, string::npos));
    }
}
...
void CompilerMSL::declare_constant_arrays() {
    ...
    ir.for_each_typed_id<SPIRConstant>([&](uint32_t, SPIRConstant& c) {
        ...
        if (is_array(type) && (!fully_inlined || is_scalar(type) || is_vector(type))) {
            ...
            statement(inject_top_level_storage_qualifier(variable_decl(type, name), "constant"), " = ", constant_expression(c), ";");
            emitted = true;
        }
    });
...
void CompilerMSL::emit_resources() {
    ...
    if (get_execution_model() == ExecutionModelMeshEXT) {
        ...
        statement("using spvMesh_t = mesh<", per_vertex, ", ", per_primitive, ", ", execution.output_vertices, ", ", execution.output_primitives, ", ",
                  topology, ">;");
        statement("");
    }
}
...
void CompilerMSL::emit_specialization_constants_and_structs() {
    ...
    if (workgroup_size_id == 0 && is_mesh_shader()) {
        auto& execution = get_entry_point();
        statement("constant uint3 ", builtin_to_glsl(BuiltInWorkgroupSize, StorageClassWorkgroup), " [[maybe_unused]] = ", "uint3(", execution.workgroup_size.x,
    ...
    ir.for_each_typed_id<SPIRType>([&](uint32_t /* type_id */, const SPIRType& type) {
        if (type.basetype == SPIRType::Struct && type.pointer && type.storage == StorageClassPhysicalStorageBuffer && declared_structs.count(type.self) == 0) {
    ...
    for (auto& id_ : ir.ids_for_constant_undef_or_type) {
        ...
        if (id.get_type() == TypeConstant) {
            ...
            if (c.self == workgroup_size_id) {
                ...
                if (msl_options.supports_msl_version(1, 2) && has_decoration(c.self, DecorationSpecId) && !c.is_used_as_array_length) {
                    ...
                    // This preserves expected toggle semantics and prevents illegal MSL like as_type<uint>(bool_tmp).
                    {
                        ...
                        statement("constant ", sc_type_name, " ", sc_name, " = is_function_constant_defined(", sc_tmp_name, ") ? ", sc_true_expr, " : ",
            ...
            statement(inject_top_level_storage_qualifier(variable_decl(type, to_name(undef.self), undef.self), "constant"), " = {};");
            emitted = true;
        }
    }
...
void CompilerMSL::emit_binary_unord_op(uint32_t result_type, uint32_t result_id, uint32_t op0, uint32_t op1, const char* op) {
    bool forward = should_forward(op0) && should_forward(op1);
...
bool CompilerMSL::emit_tessellation_io_load(uint32_t result_type_id, uint32_t id, uint32_t ptr) {
    ...
    bool flat_data_type = flattened_io && (is_matrix(result_type) || is_array(result_type) || result_type.basetype == SPIRType::Struct);
    ...
    if (!flattened_io) {
        ...
        for (uint32_t i = 0; i < num_control_points; i++) {
            ...
            expr += access_chain_internal(stage_in_ptr_var_id, indices, 2, ACCESS_CHAIN_INDEX_IS_LITERAL_BIT | ACCESS_CHAIN_PTR_CHAIN_BIT, &meta);
        ...
        for (uint32_t i = 0; i < num_control_points; i++) {
            ...
            for (uint32_t j = 0; j < array_size; j++, interface_index++) {
                ...
                AccessChainMeta meta;
                expr += access_chain_internal(stage_in_ptr_var_id, indices, 2, ACCESS_CHAIN_INDEX_IS_LITERAL_BIT | ACCESS_CHAIN_PTR_CHAIN_BIT, &meta);
                if (!is_matrix(sub_type) && sub_type.basetype != SPIRType::Struct && expr_type.vecsize > sub_type.vecsize)
        ...
        for (uint32_t i = 0; i < num_control_points; i++) {
            ...
            for (uint32_t j = 0; j < uint32_t(struct_type.member_types.size()); j++) {
                ...
                if (var) {
                    interface_index = get_extended_member_decoration(var->self, j, SPIRVCrossDecorationInterfaceMemberIndex);
                }
                ...
                if (is_matrix(mbr_type) && ptr_type.storage == StorageClassInput) {
                    ...
                    for (uint32_t k = 0; k < mbr_type.columns; k++, interface_index++) {
                        if (is_array_of_struct) {
                            ...
                            AccessChainMeta meta;
                            expr +=
                                access_chain_internal(stage_in_ptr_var_id, indices, 2, ACCESS_CHAIN_INDEX_IS_LITERAL_BIT | ACCESS_CHAIN_PTR_CHAIN_BIT, &meta);
                        } else
...
bool CompilerMSL::emit_tessellation_access_chain(const uint32_t* ops, uint32_t length) {
    ...
    if (var && flat_data) {
        ...
        variable_is_flat = !builtin_variable || is_block || bi_type == BuiltInPosition || bi_type == BuiltInPointSize || bi_type == BuiltInClipDistance ||
                           bi_type == BuiltInCullDistance;
    }
    ...
    if (variable_is_flat) {
        ...
        if (!further_access_chain_is_trivial && (flatten_composites || is_block)) {
            ...
            if (index == uint32_t(-1) && length >= (first_non_array_index + 1)) {
                ...
                index = get_extended_member_decoration(var->self, mbr_idx, SPIRVCrossDecorationInterfaceMemberIndex);
            ...
            bool defer_access_chain =
                flatten_composites && (is_matrix(result_pointee_type) || is_array(result_pointee_type) || result_pointee_type.basetype == SPIRType::Struct);
        ...
        if (!ptr_is_chain) {
            ...
            bool split_access_chain_formulation =
                flatten_composites && ptr_expr && ptr_expr->implied_read_expressions.size() == 2 && !further_access_chain_is_trivial;

            if (split_access_chain_formulation) {
                e = join(to_expression(ptr), access_chain_internal(stage_var_id, indices.data(), uint32_t(indices.size()), ACCESS_CHAIN_CHAIN_ONLY_BIT, &meta));
            } else {
        ...
        if (!is_array(*expr_type) && !is_matrix(*expr_type) && expr_type->basetype != SPIRType::Struct && expr_type->vecsize > result_ptr_type.vecsize)
    ...
    if (is_tesc_shader() && var && m && m->decoration.builtin_type == BuiltInTessLevelInner && is_tessellating_triangles()) {
...
bool CompilerMSL::prepare_access_chain_for_scalar_access(std::string& expr, const SPIRType& type, StorageClass storage, bool& is_packed) {
...
void CompilerMSL::fix_up_interpolant_access_chain(const uint32_t* ops, uint32_t length) {
    ...
    if (has_extended_decoration(ops[2], SPIRVCrossDecorationInterfaceMemberIndex)) {
        ...
        interface_index = get_extended_member_decoration(var->self, c.scalar(), SPIRVCrossDecorationInterfaceMemberIndex);
    }
    ...
    for (uint32_t i = 3; i < length; ++i) {
        ...
        if (!has_extended_decoration(ops[2], SPIRVCrossDecorationInterfaceMemberIndex) && i - 3 == var_type.array.size())
            continue;
...
bool CompilerMSL::check_physical_type_cast(std::string& expr, const SPIRType* type, uint32_t physical_type) {
    ...
    if (p_physical_type && p_physical_type->storage == StorageClassPhysicalStorageBuffer && p_physical_type->basetype == to_unsigned_basetype(64)) {
...
#define MSL_BOP_CAST(op, type) emit_binary_op_cast(ops[0], ops[1], ops[2], ops[3], #op, type, opcode_is_sign_invariant(opcode), false)
...
#define MSL_UNORD_BOP(op) emit_binary_unord_op(ops[0], ops[1], ops[2], ops[3], #op)
    ...
    case OpBitFieldSExtract: {
        emit_trinary_func_op_bitextract(ops[0], ops[1], ops[2], ops[3], ops[4], "extract_bits", int_type, int_type, SPIRType::UInt, SPIRType::UInt);
        break;
    }

    case OpBitFieldUExtract: {
        emit_trinary_func_op_bitextract(ops[0], ops[1], ops[2], ops[3], ops[4], "extract_bits", uint_type, uint_type, SPIRType::UInt, SPIRType::UInt);
        break;
    }
    ...
    case OpAtomicCompareExchange: {
        ...
        uint32_t val = ops[6];
        uint32_t comp = ops[7];
        emit_atomic_func_op(result_type, id, "atomic_compare_exchange_weak", opcode, mem_sem_pass, mem_sem_fail, true, ptr, comp, true, false, val);
        break;
    }
...
#define MSL_AFMO_IMPL(op, valsrc, valconst)                                                                                                                    \
    do {                                                                                                                                                       \
        ...
        uint32_t mem_sem = ops[4];                                                                                                                             \
        uint32_t val = valsrc;                                                                                                                                 \
        emit_atomic_func_op(result_type, id, "atomic_fetch_" #op, opcode, mem_sem, mem_sem, false, ptr, val, false, valconst);                                 \
    } while (false)
...
#define MSL_AFMIO(op) MSL_AFMO_IMPL(op, 1, true)
    ...
    case OpImageTexelPointer: {
        ...
        if (var && atomic_image_vars_emulated.count(var->self)) {
            ...
            auto& e = set<SPIRExpression>(id, join(to_expression(ops[2]), "@", bitcast_expression(SPIRType::UInt, ops[3])), result_type, true);
    ...
    case OpImageWrite: {
        ...
        expr += join(to_expression(img_id), ".write(", remap_swizzle(store_type, texel_type.vecsize, to_expression(texel_id)), ", ",
    ...
    case OpImageQueryLod: {
        ...
        statement(to_expression(id), ".x = ", image_expr, ".calculate_clamped_lod(", sampler_expr, ", ", coord_expr, ");");
        statement(to_expression(id), ".y = ", image_expr, ".calculate_unclamped_lod(", sampler_expr, ", ", coord_expr, ");");
...
#define MSL_ImgQry(qrytype)                                                                                                                                    \
    ...
    case OpStore: {
        ...
        if (needs_frag_discard_checks() && (type.storage == StorageClassStorageBuffer || type.storage == StorageClassUniform)) {
    ...
    case OpMatrixTimesVector: {
        ...
        if (e && e->need_transpose) {
            ...
            if (opcode == OpMatrixTimesVector) {
                expr = join("spvFMulVectorMatrix(", to_enclosed_unpacked_expression(ops[3]), ", ", to_unpacked_row_major_matrix_expression(ops[2]), ")");
            } else {
                expr = join("spvFMulMatrixVector(", to_unpacked_row_major_matrix_expression(ops[3]), ", ", to_enclosed_unpacked_expression(ops[2]), ")");
            }
    ...
    case OpISubBorrow: {
        ...
        if (opcode == OpIAddCarry) {
            ...
            statement(to_expression(result_id), ".", to_member_name(type, 1), " = select(", type_to_glsl(res_type), "(1), ", type_to_glsl(res_type), "(0), ",
                      to_unpacked_expression(result_id), ".", to_member_name(type, 0), " >= max(", to_unpacked_expression(op0), ", ",
                      to_unpacked_expression(op1), "));");
        } else {
            statement(to_expression(result_id), ".", to_member_name(type, 0), " = ", to_enclosed_unpacked_expression(op0), " - ",
                      to_enclosed_unpacked_expression(op1), ";");
            statement(to_expression(result_id), ".", to_member_name(type, 1), " = select(", type_to_glsl(res_type), "(1), ", type_to_glsl(res_type), "(0), ",
    ...
    case OpIsHelperInvocationEXT:
        ...
        emit_op(ops[0], ops[1],
                needs_manual_helper_invocation_updates() ? builtin_to_glsl(BuiltInHelperInvocation, StorageClassInput) : "simd_is_helper_thread()", false);
        break;
    ...
    case OpRayQueryInitializeKHR: {
        ...
        statement(to_expression(ops[0]), ".reset(", "ray(", to_expression(ops[4]), ", ", to_expression(ops[6]), ", ", to_expression(ops[5]), ", ",
                  to_expression(ops[7]), "), ", to_expression(ops[1]), ", ", to_expression(ops[3]), ", spvMakeIntersectionParams(", to_expression(ops[2]),
                  "));");
        break;
    }
    case OpRayQueryProceedKHR: {
...
#define MSL_RAY_QUERY_IS_OP2(op, msl_op) MSL_RAY_QUERY_OP_INNER2(op, .is, msl_op)
    ...
    case OpRayQueryGetIntersectionTypeKHR:
        flush_variable_declaration(ops[2]);
        if (MSL_RAY_QUERY_IS_CANDIDATE)
            emit_op(ops[0], ops[1], join("uint(", to_expression(ops[2]), ".get_candidate_intersection_type()) - 1"), false);
        else
...
#undef MSL_RAY_QUERY_OP_INNER2
    ...
    case OpSUDot: {
        ...
        if (instruction.length == 5) {
            ...
            SPIRType::BaseType vec1_expected_type = opcode != OpUDot ? to_signed_basetype(input_type1.width) : to_unsigned_basetype(input_type1.width);

            SPIRType::BaseType vec2_expected_type = opcode != OpSDot ? to_unsigned_basetype(input_type2.width) : to_signed_basetype(input_type2.width);
        ...
        string exp = join("reduce_add(", result_type_cast, "(", vec1input, ") * ", result_type_cast, "(", vec2input, "))");
    ...
    case OpSUDotAccSat: {
        ...
        if (instruction.length == 6) {
            ...
            // Inputs are sign or zero-extended to their target width.
            SPIRType::BaseType vec1_expected_type = opcode != OpUDotAccSat ? to_signed_basetype(input_type1.width) : to_unsigned_basetype(input_type1.width);

            SPIRType::BaseType vec2_expected_type = opcode != OpSDotAccSat ? to_unsigned_basetype(input_type2.width) : to_signed_basetype(input_type2.width);
        ...
        SPIRType::BaseType pre_saturate_type = opcode != OpUDotAccSat ? to_signed_basetype(type.width) : to_unsigned_basetype(type.width);

        input_type1.basetype = pre_saturate_type;
        input_type2.basetype = pre_saturate_type;

        string exp = join(type_to_glsl(type), "(addsat(reduce_add(", type_to_glsl(input_type1), "(", vec1input, ") * ", type_to_glsl(input_type2), "(",
                          vec2input, ")), ", bitcast_expression(pre_saturate_type, acc), "))");
    ...
    case OpCooperativeMatrixLoadKHR: {
        ...
        if (col_major)
            statement("simdgroup_load(", to_expression(id), ", ", ptr_expr, ", ", stride_expr, ", ulong2(0), true);");
        else
            statement("simdgroup_load(", to_expression(id), ", ", ptr_expr, ", ", stride_expr, ");");
    ...
    case OpCooperativeMatrixStoreKHR: {
        ...
        if (col_major)
            statement("simdgroup_store(", to_expression(obj), ", ", ptr_expr, ", ", stride_expr, ", ulong2(0), true);");
        else
            statement("simdgroup_store(", to_expression(obj), ", ", ptr_expr, ", ", stride_expr, ");");
    ...
    case OpCooperativeMatrixMulAddKHR: {
        ...
        emit_uninitialized_temporary_expression(result_type, id);
        statement("simdgroup_multiply_accumulate(", to_expression(id), ", ", to_unpacked_expression(A), ", ", to_unpacked_expression(B), ", ",
    ...
    case OpCooperativeMatrixLengthKHR: {
        ...
        auto expr = join(type_to_glsl(get<SPIRType>(result_type)), "(sizeof(", coop_type_name, "::storage_type) / sizeof(", component_type_name, "))");
    ...
    default: {
        ...
        if (instruction.length >= 2) {
            ...
            if (opcode == OpCompositeExtract || opcode == OpVectorExtractDynamic) {
                ...
                if ((instruction.length >= 3 && is_cooperative_matrix_typed_id(ops[2])) || (instruction.length >= 4 && is_cooperative_matrix_typed_id(ops[3])))
                    SPIRV_CROSS_THROW("Unsupported operation on cooperative matrix in MSL backend.");
            }
        }
        CompilerGLSL::emit_instruction(instruction);
...
void CompilerMSL::emit_barrier(uint32_t id_exe_scope, uint32_t id_mem_scope, uint32_t id_mem_sem) {
    ...
    if (msl_options.supports_msl_version(1, 2)) {
        ...
        if (is_tesc_shader() || (mem_sem & (MemorySemanticsUniformMemoryMask | MemorySemanticsCrossWorkgroupMemoryMask)))
    ...
    if (!id_exe_scope && msl_options.supports_msl_version(3, 2)) {
        ...
        if (mem_scope == ScopeDevice &&
            (mem_sem & (MemorySemanticsUniformMemoryMask | MemorySemanticsImageMemoryMask | MemorySemanticsCrossWorkgroupMemoryMask)) == 0) {
            mem_scope = ScopeWorkgroup;
        }

        // MSL 3.2 only supports seq_cst or relaxed.
        if (mem_sem &
            (MemorySemanticsAcquireReleaseMask | MemorySemanticsAcquireMask | MemorySemanticsReleaseMask | MemorySemanticsSequentiallyConsistentMask)) {
...
bool CompilerMSL::emit_array_copy(const char* expr, uint32_t lhs_id, uint32_t rhs_id, StorageClass lhs_storage, StorageClass rhs_storage) {
    ...
    if (lhs_is_array_template && rhs_is_array_template && !using_builtin_array()) {
        ...
        if (ir.ids[rhs_id].get_type() == TypeConstant) {
            is_constant = true;
        } else if (var && var->remapped_variable && var->statically_assigned && ir.ids[var->static_expression].get_type() == TypeConstant) {
            is_constant = true;
...
void CompilerMSL::emit_atomic_func_op(uint32_t result_type, uint32_t result_id, const char* op, Op opcode, uint32_t mem_order_1, uint32_t mem_order_2,
                                      bool has_mem_order_2, uint32_t obj, uint32_t op1, bool op1_is_pointer, bool op1_is_literal, uint32_t op2) {
    string exp;
    ...
    bool check_discard = opcode != OpAtomicLoad && needs_frag_discard_checks() && ptr_type.storage != StorageClassWorkgroup;
...
#undef EMIT_PRECISE_OVERRIDE
    ...
    case GLSLstd450Atan2:
        if (restype.basetype == SPIRType::Half) {
            ...
            auto expr = join(type_to_glsl(restype), "(", preserve_str, "::atan2(", type_to_glsl(ftype), "(", to_unpacked_expression(args[0]), "), ",
    ...
    case GLSLstd450InterpolateAtCentroid: {
        ...
        emit_op(result_type, id,
                join(to_name(stage_in_var_id), ".", to_member_name(get_stage_in_struct_type(), interface_index), ".interpolate_at_centroid()", component),
    ...
    case GLSLstd450InterpolateAtSample: {
        ...
        emit_op(result_type, id,
                join(to_name(stage_in_var_id), ".", to_member_name(get_stage_in_struct_type(), interface_index), ".interpolate_at_sample(",
                     to_expression(args[1]), ")", component),
    ...
    case GLSLstd450InterpolateAtOffset: {
        ...
        emit_op(result_type, id,
                join(to_name(stage_in_var_id), ".", to_member_name(get_stage_in_struct_type(), interface_index), ".interpolate_at_offset(",
                     to_expression(args[1]), " + 0.4375)", component),
                should_forward(args[0]) && should_forward(args[1]));
        break;
    }
    ...
    case GLSLstd450Distance:
        ...
        if (expression_type(args[0]).vecsize == 1) {
            ...
            emit_op(result_type, id, join("abs(", to_enclosed_unpacked_expression(args[0]), " - ", to_enclosed_unpacked_expression(args[1]), ")"),
                    should_forward(args[0]) && should_forward(args[1]));
            inherit_expression_dependencies(id, args[0]);
            inherit_expression_dependencies(id, args[1]);
        } else
    ...
    case GLSLstd450Frexp: {
        ...
        if (!is_thread_storage || (ptr && ptr->access_chain && is_scalar(expression_type(args[1])))) {
...
void CompilerMSL::emit_spv_amd_shader_trinary_minmax_op(uint32_t result_type, uint32_t id, uint32_t eop, const uint32_t* args, uint32_t count) {
        ...
        if (arg_type.basetype == SPIRType::SampledImage && !is_dynamic_img_sampler) {
            ...
            if (arg_type.image.dim != DimBuffer) {
                if (arg_type.array.empty() || (var ? is_var_runtime_size_array(*var) : is_runtime_size_array(arg_type))) {
                    ...
                    const char* sampler_address_space = descriptor_address_space(name_id, StorageClassUniformConstant, "thread const");
                    decl += join(", ", sampler_address_space, " ", sampler_type(arg_type, name_id, false), "& ", to_sampler_expression(name_id));
                }
            }
        }

        // Manufacture automatic swizzle arg.
        if (msl_options.swizzle_texture_samples && has_sampled_images && is_sampled_image_type(arg_type) && !is_dynamic_img_sampler) {
...
string CompilerMSL::to_function_name(const TextureFunctionNameArguments& args) {
    ...
    if (args.has_array_offsets && !is_dynamic_img_sampler && (!constexpr_sampler || !constexpr_sampler->ycbcr_conversion_enable)) {
        bool is_compare = comparison_ids.count(img);
    ...
    if (needs_chroma_reconstruction(constexpr_sampler) && !is_dynamic_img_sampler) {
        ...
        if (constexpr_sampler->resolution == MSL_FORMAT_RESOLUTION_444 || constexpr_sampler->chroma_filter == MSL_SAMPLER_FILTER_NEAREST) {
        ...
        } else // Linear with a downsampled format
        {
            ...
            case MSL_FORMAT_RESOLUTION_420:
                ...
                case MSL_CHROMA_LOCATION_COSITED_EVEN:
                    ...
                    case MSL_CHROMA_LOCATION_COSITED_EVEN:
                        if (constexpr_sampler->planes == 2)
                            add_spv_func_and_recompile(SPVFuncImplChromaReconstructLinear420XCositedEvenYCositedEven2Plane);
                        else
                            add_spv_func_and_recompile(SPVFuncImplChromaReconstructLinear420XCositedEvenYCositedEven3Plane);
                        fname += "XCositedEvenYCositedEven";
                        break;
                    case MSL_CHROMA_LOCATION_MIDPOINT:
                        if (constexpr_sampler->planes == 2)
                            add_spv_func_and_recompile(SPVFuncImplChromaReconstructLinear420XCositedEvenYMidpoint2Plane);
                        else
                            add_spv_func_and_recompile(SPVFuncImplChromaReconstructLinear420XCositedEvenYMidpoint3Plane);
                ...
                case MSL_CHROMA_LOCATION_MIDPOINT:
                    ...
                    case MSL_CHROMA_LOCATION_COSITED_EVEN:
                        if (constexpr_sampler->planes == 2)
                            add_spv_func_and_recompile(SPVFuncImplChromaReconstructLinear420XMidpointYCositedEven2Plane);
                        else
                            add_spv_func_and_recompile(SPVFuncImplChromaReconstructLinear420XMidpointYCositedEven3Plane);
...
string CompilerMSL::to_function_args(const TextureFunctionArguments& args, bool* p_forward) {
    ...
    if (!is_dynamic_img_sampler) {
        ...
        } else if ((!constexpr_sampler || !constexpr_sampler->ycbcr_conversion_enable) && msl_options.swizzle_texture_samples && args.base.is_gather) {
            auto* combined = maybe_get<SPIRCombinedImageSampler>(img);
        ...
        if ((!constexpr_sampler || !constexpr_sampler->ycbcr_conversion_enable) && msl_options.swizzle_texture_samples && args.base.is_gather) {
    ...
    if (imgtype.image.dim == DimCube && imgtype.image.arrayed && msl_options.emulate_cube_array) {
        ...
        if (is_cube_fetch)
            ...
            farg_str += ", uint(spvCubemapTo2DArrayFace(" + tex_coords + ").z) + (uint(" +
                        round_fp_tex_coords(to_extract_component_expression(args.coord, alt_coord_component), coord_is_fp) + ") * 6u)";
        ...
        if (is_cube_fetch) {
            ...
            if (imgtype.image.arrayed)
                ...
                farg_str += ", uint(" + round_fp_tex_coords(to_extract_component_expression(args.coord, 2), coord_is_fp) + ")";
        }
        ...
        if (imgtype.image.arrayed) {
            ...
            if (imgtype.image.dim == DimCube && args.base.is_fetch) {
                ...
                farg_str += ", uint(" + round_fp_tex_coords(to_extract_component_expression(args.coord, alt_coord_component), coord_is_fp) + ")";
                if (imgtype.image.dim == DimSubpassData) {
                    if (msl_options.multiview)
    ...
    if (args.dref) {
        ...
        string dref_expr;
        if (args.base.is_proj)
            dref_expr = join(to_enclosed_unpacked_expression(args.dref), " / ", to_extract_component_expression(args.coord, alt_coord_component));
        else
        ...
        if (msl_options.is_macos() && (grad_x || grad_y)) {
            ...
            if (constant_zero_x && constant_zero_y && (!imgtype.image.arrayed || !msl_options.sample_dref_lod_array_as_grad)) {
                lod = 0;
                grad_x = 0;
                grad_y = 0;
                farg_str += ", level(0)";
    ...
    if (lod && (imgtype.image.dim != Dim1D || msl_options.texture_1D_as_2D)) {
        ...
        if (args.base.is_fetch) {
            ...
            farg_str += join(", ", grad_opt, "(", grad_coord, "exp2(", to_unpacked_expression(lod), " - 0.5) / ", extent, ", exp2(",
                             to_unpacked_expression(lod), " - 0.5) / ", extent, ")");
        } else {
            farg_str += ", level(" + to_unpacked_expression(lod) + ")";
        }
    } else if (args.base.is_fetch && !lod && (imgtype.image.dim != Dim1D || msl_options.texture_1D_as_2D) && imgtype.image.dim != DimBuffer &&
               !imgtype.image.ms && imgtype.image.sampled != 2) {
...
string CompilerMSL::to_texture_op(const Instruction& i, bool sparse, bool* forward, SmallVector<uint32_t>& inherited_expressions) {
    ...
    } else if (msl_options.swizzle_texture_samples && !is_gather && is_sampled_image_type(imgtype) && !is_dynamic_img_sampler) {
    ...
    if (constexpr_sampler && constexpr_sampler->ycbcr_conversion_enable && !is_dynamic_img_sampler) {
        if (!constexpr_sampler->swizzle_is_identity()) {
            ...
            if (!constexpr_sampler->swizzle_has_one_or_zero()) {
                ...
                for (uint32_t c = 0; c < 4; c++) {
                    ...
                    case MSL_COMPONENT_SWIZZLE_B:
                    case MSL_COMPONENT_SWIZZLE_A:
                        expr += to_expression(temp_id) + "." + swizzle_names[constexpr_sampler->swizzle[c] - MSL_COMPONENT_SWIZZLE_R];
                        break;
                    default:
                        SPIRV_CROSS_THROW("Invalid component swizzle.");
                    }
        ...
        expr += inner_expr;
        if (msl_options.swizzle_texture_samples && !is_gather && is_sampled_image_type(imgtype) && !is_dynamic_img_sampler) {
            // Add the swizzle constant from the swizzle buffer.
...
string CompilerMSL::to_func_call_arg(const SPIRFunction::Parameter& arg, uint32_t id) {
    ...
    if (!arg_is_dynamic_img_sampler) {
        ...
        if (is_dynamic_img_sampler && constexpr_sampler && constexpr_sampler->ycbcr_conversion_enable)
            arg_str +=
                join(", (uint(", create_swizzle(constexpr_sampler->swizzle[3]), ") << 24) | (uint(", create_swizzle(constexpr_sampler->swizzle[2]),
                     ") << 16) | (uint(", create_swizzle(constexpr_sampler->swizzle[1]), ") << 8) | uint(", create_swizzle(constexpr_sampler->swizzle[0]), ")");
...
string CompilerMSL::convert_row_major_matrix(string exp_str, const SPIRType& exp_type, uint32_t physical_type_id, bool is_packed, bool relaxed) {
...
string CompilerMSL::to_struct_member(const SPIRType& type, uint32_t member_type_id, uint32_t index, const string& qualifier) {
    ...
    if (physical_type.basetype == SPIRType::Image && physical_type.image.sampled == 2 && msl_options.is_ios() &&
        msl_options.argument_buffers_tier <= Options::ArgumentBuffersTier::Tier1 && !has_decoration(orig_id, DecorationNonWritable)) {
    ...
    string array_type;
    if (physical_type.basetype != SPIRType::Image && physical_type.basetype != SPIRType::Sampler && physical_type.basetype != SPIRType::SampledImage) {
        BuiltIn builtin = BuiltInMax;
        ...
        bool is_ib_in_out = ((stage_out_var_id && get_stage_out_struct_type().self == type.self && variable_storage_requires_stage_io(StorageClassOutput)) ||
                             (stage_in_var_id && get_stage_in_struct_type().self == type.self && variable_storage_requires_stage_io(StorageClassInput))) ||
                            is_mesh_shader();
    ...
    if (is_mesh_shader()) {
        ...
        if (is_member_builtin(type, index, &builtin)) {
            ...
            if (has_member_decoration(type.self, orig_id, DecorationBuiltIn)) {
                ...
                result = join(type_to_glsl(metallic_type, orig_id, false), " ", qualifier, builtin_to_glsl(builtin, StorageClassOutput),
                              member_attribute_qualifier(type, index), array_type, ";");
            } else {
                result = join(type_to_glsl(metallic_type, orig_id, false), " ", qualifier, to_member_name(type, index), member_attribute_qualifier(type, index),
                              array_type, ";");
            }
            is_using_builtin_array = false;
            return result;
    ...
    if (physical_type.basetype == SPIRType::Struct && has_decoration(physical_type.self, DecorationArrayStride) && is_array(physical_type)) {
    ...
    const char* overlapping_binding_tag =
        has_extended_member_decoration(type.self, index, SPIRVCrossDecorationOverlappingBinding) ? "// Overlapping binding: " : "";

    auto result = join(overlapping_binding_tag, pack_pfx, decl_type, " ", qualifier, to_member_name(type, index), member_attribute_qualifier(type, index),
                       array_type, ";");
...
void CompilerMSL::emit_struct_member(const SPIRType& type, uint32_t member_type_id, uint32_t index, const string& qualifier, uint32_t) {
...
string CompilerMSL::member_attribute_qualifier(const SPIRType& type, uint32_t index) {
    ...
    if (has_extended_member_decoration(type.self, index, SPIRVCrossDecorationResourceIndexPrimary)) {
        string quals = join(" [[id(", get_extended_member_decoration(type.self, index, SPIRVCrossDecorationResourceIndexPrimary), ")");
        if (interlocked_resources.count(get_extended_member_decoration(type.self, index, SPIRVCrossDecorationInterfaceOrigID)))
    ...
    bool use_semantic_stage_output = is_mesh_shader() || is_tese_shader() || (execution.model == ExecutionModelVertex && !msl_options.vertex_for_tessellation);
...
uint32_t CompilerMSL::get_or_allocate_builtin_input_member_location(BuiltIn builtin, uint32_t type_id, uint32_t index, uint32_t* comp) {
    ...
    if (!msl_options.raw_buffer_tese_input && is_tessellating_triangles() && (builtin == BuiltInTessLevelInner || builtin == BuiltInTessLevelOuter)) {
...
uint32_t CompilerMSL::get_or_allocate_builtin_output_member_location(BuiltIn builtin, uint32_t type_id, uint32_t index, uint32_t* comp) {
    uint32_t loc = get_member_location(type_id, index, comp);
    if (loc != k_unknown_location)
        return loc;
    loc = 0;
...
string CompilerMSL::func_type_decl(SPIRType& type) {
    ...
    case ExecutionModelTessellationEvaluation:
        ...
        if (msl_options.is_ios())
            ...
            entry_type = join("[[ patch(", is_tessellating_triangles() ? "triangle" : "quad", ", ", execution.output_vertices, ") ]] vertex");
        break;
...
bool CompilerMSL::decoration_flags_signal_volatile(const Bitset& flags) const {
    ...
    return flags.get(DecorationVolatile) || (flags.get(DecorationCoherent) && !msl_options.supports_msl_version(3, 2));
}
...
string CompilerMSL::get_type_address_space(const SPIRType& type, uint32_t id, bool argument) {
    ...
    Bitset flags;
    auto* var = maybe_get<SPIRVariable>(id);
    if (var && type.basetype == SPIRType::Struct && (has_decoration(type.self, DecorationBlock) || has_decoration(type.self, DecorationBufferBlock)))
        flags = get_buffer_block_flags(id);
    else {
        flags = get_decoration_bitset(id);

        if (type.basetype == SPIRType::Struct && (has_decoration(type.self, DecorationBlock) || has_decoration(type.self, DecorationBufferBlock))) {
...
const char* CompilerMSL::to_restrict(uint32_t id, bool space) {
    ...
    if (ir.ids[id].get_type() == TypeVariable) {
        ...
        auto& type = expression_type(id);
        if (type.basetype == SPIRType::Struct && (has_decoration(type_id, DecorationBlock) || has_decoration(type_id, DecorationBufferBlock)))
            flags = get_buffer_block_flags(id);
        else
            flags = get_decoration_bitset(id);
    } else
    ...
    return flags.get(DecorationRestrict) || flags.get(DecorationRestrictPointerEXT) ? (space ? "__restrict " : "__restrict") : "";
}

string CompilerMSL::entry_point_arg_stage_in() {
    string decl;

    if ((is_tesc_shader() && msl_options.multi_patch_workgroup) || (is_tese_shader() && msl_options.raw_buffer_tese_input))
        return decl;
...
bool CompilerMSL::is_direct_input_builtin(BuiltIn bi_type) {
    ...
    case BuiltInBaryCoordNoPerspKHR:
        return false;
    case BuiltInViewIndex:
        return get_execution_model() == ExecutionModelFragment && msl_options.multiview && msl_options.multiview_layered_rendering;
    // Compute function in
    case BuiltInSubgroupId:
    case BuiltInNumSubgroups:
...
bool CompilerMSL::is_sample_rate() const {
    ...
    return get_execution_model() == ExecutionModelFragment &&
           (msl_options.force_sample_rate_shading || std::find(caps.begin(), caps.end(), CapabilitySampleRateShading) != caps.end() ||
...
void CompilerMSL::entry_point_args_builtin(string& ep_args) {
    ...
    if (capture_output_to_buffer) {
        ...
        if (stage_out_var_id) {
            if (!ep_args.empty())
                ep_args += ", ";
            ep_args += join("device ", type_to_glsl(get_stage_out_struct_type()), "* ", output_buffer_var_name, " [[buffer(",
        ...
        if (is_tesc_shader()) {
            if (!ep_args.empty())
                ep_args += ", ";
            ep_args += join("constant uint* spvIndirectParams [[buffer(", msl_options.indirect_params_buffer_index, ")]]");
        } else if (stage_out_var_id && !(get_execution_model() == ExecutionModelVertex && msl_options.vertex_for_tessellation)) {
            if (!ep_args.empty())
                ep_args += ", ";
            ep_args += join("device uint* spvIndirectParams [[buffer(", msl_options.indirect_params_buffer_index, ")]]");
        }
        ...
        if (get_execution_model() == ExecutionModelVertex && msl_options.vertex_for_tessellation &&
            ...
            case Options::IndexType::UInt16:
                ep_args += join("const device ushort* ", index_buffer_var_name, " [[buffer(", msl_options.shader_index_buffer_index, ")]]");
                break;
            case Options::IndexType::UInt32:
                ep_args += join("const device uint* ", index_buffer_var_name, " [[buffer(", msl_options.shader_index_buffer_index, ")]]");
                break;
            }
        }
        ...
        if (is_tesc_shader()) {
            if (patch_stage_out_var_id) {
                if (!ep_args.empty())
                    ep_args += ", ";
                ep_args += join("device ", type_to_glsl(get_patch_stage_out_struct_type()), "* ", patch_output_buffer_var_name, " [[buffer(",
            ...
            if (outer_factor_initializer_id && (c = maybe_get<SPIRConstant>(outer_factor_initializer_id))) {
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    ...
                    for (uint32_t i = 0; i < components; i++) {
                        statement(builtin_to_glsl(BuiltInTessLevelOuter, StorageClassOutput), "[", i, "] = ", "half(", to_expression(c->subconstants[i]), ");");
                    }
                });
            }
            ...
            if (inner_factor_initializer_id && (c = maybe_get<SPIRConstant>(inner_factor_initializer_id))) {
                ...
                if (is_tessellating_triangles()) {
                    entry_func.fixup_hooks_in.push_back([=]() {
                        statement(builtin_to_glsl(BuiltInTessLevelInner, StorageClassOutput), " = ", "half(", to_expression(c->subconstants[0]), ");");
                    });
                } else {
                    entry_func.fixup_hooks_in.push_back([=]() {
                        for (uint32_t i = 0; i < 2; i++) {
                            statement(builtin_to_glsl(BuiltInTessLevelInner, StorageClassOutput), "[", i, "] = ", "half(", to_expression(c->subconstants[i]),
                                      ");");
                        }
                    });
                }
            ...
            if (stage_in_var_id) {
                if (!ep_args.empty())
                    ep_args += ", ";
                if (msl_options.multi_patch_workgroup) {
                    ep_args += join("device ", type_to_glsl(get_stage_in_struct_type()), "* ", input_buffer_var_name, " [[buffer(",
                                    convert_to_string(msl_options.shader_input_buffer_index), ")]]");
                } else {
                    ep_args += join("threadgroup ", type_to_glsl(get_stage_in_struct_type()), "* ", input_wg_var_name, " [[threadgroup(",
                                    convert_to_string(msl_options.shader_input_wg_index), ")]]");
                }
            }
        }
    }
    ...
    if (is_tese_shader() && msl_options.raw_buffer_tese_input) {
        if (patch_stage_in_var_id) {
            if (!ep_args.empty())
                ep_args += ", ";
            ep_args += join("const device ", type_to_glsl(get_patch_stage_in_struct_type()), "* ", patch_input_buffer_var_name, " [[buffer(",
                            convert_to_string(msl_options.shader_patch_input_buffer_index), ")]]");
        }

        if (tess_level_inner_var_id || tess_level_outer_var_id) {
            if (!ep_args.empty())
                ep_args += ", ";
            ep_args += join("const device ", get_tess_factor_struct_name(), "* ", tess_factor_buffer_var_name, " [[buffer(",
                            convert_to_string(msl_options.shader_tess_factor_buffer_index), ")]]");
        }
        ...
        if (stage_in_var_id) {
            if (!ep_args.empty())
                ep_args += ", ";
            ep_args += join("const device ", type_to_glsl(get_stage_in_struct_type()), "* ", input_buffer_var_name, " [[buffer(",
                            convert_to_string(msl_options.shader_input_buffer_index), ")]]");
        }
    }
...
void CompilerMSL::entry_point_args_discrete_descriptors(string& ep_args) {
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t var_id, SPIRVariable& var) {
        if ((var.storage == StorageClassUniform || var.storage == StorageClassUniformConstant || var.storage == StorageClassPushConstant ||
            ...
            const auto resource_is_aliasing_candidate = [this](const SPIRVariable& var_) {
                return is_var_runtime_size_array(var_) || var_.storage == StorageClassUniform || var_.storage == StorageClassStorageBuffer;
            };

            if (resource_is_aliasing_candidate(var)) {
                for (auto& resource : resources) {
                    if (resource_is_aliasing_candidate(*resource.var) &&
                        get_decoration(resource.var->self, DecorationDescriptorSet) == get_decoration(var_id, DecorationDescriptorSet) &&
                        ...
                        if ((is_array(get_variable_data_type(*resource.var)) && !is_var_runtime_size_array(*resource.var)) ||
                            ...
                            SPIRV_CROSS_THROW("Attempting to alias same binding with a descriptor array which is not implemented through argument buffers. "
                                              "This is unsupported.");
                        }
                        break;
                    }
            ...
            if (type.basetype == SPIRType::SampledImage) {
                ...
                for (uint32_t i = 0; i < plane_count; i++)
                    resources.push_back({&var, discrete_descriptor_alias, to_name(var_id), SPIRType::Image, get_metal_resource_index(var, SPIRType::Image, i),
                                         i, secondary_index});
                ...
                entry_point_bindings.push_back(&var);
                resources.push_back({&var, discrete_descriptor_alias, to_name(var_id), type.basetype, resource_index, 0, secondary_index});
            }
        }
    });

    stable_sort(resources.begin(), resources.end(),
                [](const Resource& lhs, const Resource& rhs) { return tie(lhs.basetype, lhs.index) < tie(rhs.basetype, rhs.index); });
    ...
    for (auto& r : resources) {
        ...
        if (r.discrete_descriptor_alias) {
            if (r.var == r.discrete_descriptor_alias) {
                auto primary_name =
                    join("spvBufferAliasSet", get_decoration(var_id, DecorationDescriptorSet), "Binding", get_decoration(var_id, DecorationBinding));
            ...
            if (!type.array.empty()) {
                ...
                for (uint32_t i = 0; i < array_size; ++i) {
                    if (!ep_args.empty())
                        ep_args += ", ";
                    ep_args +=
                        get_variable_address_space(var) + " " + type_to_glsl(type) + "* " + to_restrict(var_id, true) + r.name + "_" + convert_to_string(i);
        ...
        default:
            if (!ep_args.empty())
                ep_args += ", ";
            if (!type.pointer)
                ep_args += get_type_address_space(get<SPIRType>(var.basetype), var_id) + " " + type_to_glsl(type, var_id) + "& " + r.name;
            else
                ep_args += type_to_glsl(type, var_id) + " " + r.name;
            ep_args += " [[buffer(" + convert_to_string(r.index) + ")";
            if (interlocked_resources.count(var_id))
...
void CompilerMSL::fix_up_shader_inputs_outputs() {
    ...
    if (get_execution_model() == ExecutionModelVertex && msl_options.vertex_for_tessellation) {
        entry_func.fixup_hooks_in.push_back([this]() {
            statement("if (any(", to_expression(builtin_invocation_id_id), " >= ", to_expression(builtin_stage_input_size_id), "))");
            statement("    return;");
        });
    }
        ...
        if (var.storage == StorageClassUniformConstant && !is_hidden_variable(var)) {
            if (msl_options.swizzle_texture_samples && has_sampled_images && is_sampled_image_type(type)) {
                entry_func.fixup_hooks_in.push_back([this, &type, &var, var_id]() {
                    ...
                    uint32_t desc_set = get_decoration(var_id, DecorationDescriptorSet);
                    if (descriptor_set_is_argument_buffer(desc_set)) {
                        statement("constant uint", is_array_type ? "* " : "& ", to_swizzle_expression(var_id), is_array_type ? " = &" : " = ",
                                  to_name(argument_buffer_ids[desc_set]), ".spvSwizzleConstants", "[",
                        ...
                        statement("constant uint", is_array_type ? "* " : "& ", to_swizzle_expression(var_id), is_array_type ? " = &" : " = ",
                                  to_name(swizzle_buffer_id), "[", convert_to_string(get_metal_resource_index(var, SPIRType::Image)), "];");
                    }
                });
            }
            ...
            if (buffer_requires_array_length(var.self)) {
                entry_func.fixup_hooks_in.push_back([this, &type, &var, var_id]() {
                    ...
                    uint32_t desc_set = get_decoration(var_id, DecorationDescriptorSet);
                    if (descriptor_set_is_argument_buffer(desc_set)) {
                        statement("constant uint", is_array_type ? "* " : "& ", to_buffer_size_expression(var_id), is_array_type ? " = &" : " = ",
                                  to_name(argument_buffer_ids[desc_set]), ".spvBufferSizeConstants", "[",
                                  convert_to_string(get_metal_resource_index(var, SPIRType::UInt)), "];");
                    } else {
                        // If we have an array of images, we need to be able to index into it, so take a pointer instead.
                        statement("constant uint", is_array_type ? "* " : "& ", to_buffer_size_expression(var_id), is_array_type ? " = &" : " = ",
                                  to_name(buffer_size_buffer_id), "[", convert_to_string(get_metal_resource_index(var, type.basetype)), "];");
                    }
                });
            }
        }

        if (!msl_options.argument_buffers && msl_options.replace_recursive_inputs && type_contains_recursion(type) &&
            (var.storage == StorageClassUniform || var.storage == StorageClassUniformConstant || var.storage == StorageClassPushConstant ||
             var.storage == StorageClassStorageBuffer)) {
            recursive_inputs.insert(type.self);
            entry_func.fixup_hooks_in.push_back([this, &type, &var, var_id]() {
                auto addr_space = get_variable_address_space(var);
                auto var_name = to_name(var_id);
                statement(addr_space, " auto& ", to_restrict(var_id, true), var_name, " = *(", addr_space, " ", type_to_glsl(type), "*)", var_name, "_vp;");
            });
        }
    });
    ...
    ir.for_each_typed_id<SPIRVariable>([this, &entry_func](uint32_t, SPIRVariable& var) {
        ...
        if (var.storage == StorageClassInput && is_builtin_variable(var) && active_input_builtins.get(bi_type)) {
            ...
            case BuiltInSamplePosition:
                entry_func.fixup_hooks_in.push_back([=]() {
                    statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = get_sample_position(", to_expression(builtin_sample_id_id), ");");
                });
                break;
            case BuiltInFragCoord:
                if (is_sample_rate()) {
                    entry_func.fixup_hooks_in.push_back(
                        [=]() { statement(to_expression(var_id), ".xy += get_sample_position(", to_expression(builtin_sample_id_id), ") - 0.5;"); });
                }
                break;
            case BuiltInInvocationId:
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_invocation_id_id), ".x % ",
            ...
            case BuiltInPrimitiveId:
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = min(", to_expression(builtin_invocation_id_id), ".x / ",
                              this->get_entry_point().output_vertices, ", spvIndirectParams[1] - 1);");
                });
                break;
            case BuiltInPatchVertices:
                if (is_tese_shader()) {
                    if (msl_options.raw_buffer_tese_input) {
                        entry_func.fixup_hooks_in.push_back(
                            [=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", get_entry_point().output_vertices, ";"); });
                    } else {
                        entry_func.fixup_hooks_in.push_back([=]() {
                            statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(patch_stage_in_var_id), ".gl_in.size();");
                        });
                    }
                } else {
                    entry_func.fixup_hooks_in.push_back(
            ...
            case BuiltInSubgroupId:
                ...
                entry_func.fixup_hooks_in.push_back(
                    [=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_local_invocation_index_id), ";"); });
                break;
            case BuiltInNumSubgroups:
                if (!msl_options.emulate_subgroups)
                    break;
            ...
            case BuiltInSubgroupLocalInvocationId:
                ...
                entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = 0;"); });
                break;
            case BuiltInSubgroupSize:
                if (msl_options.emulate_subgroups) {
                    // For subgroup emulation, assume subgroups of size 1.
                    entry_func.fixup_hooks_in.push_back([=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = 1;"); });
                } else if (msl_options.fixed_subgroup_size != 0) {
                    entry_func.fixup_hooks_in.push_back(
                        [=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", msl_options.fixed_subgroup_size, ";"); });
                }
                break;
            case BuiltInSubgroupEqMask:
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    if (msl_options.is_ios()) {
                        ...
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_subgroup_invocation_id_id),
                                  " >= 32 ? uint4(0, (1 << (", to_expression(builtin_subgroup_invocation_id_id), " - 32)), uint2(0)) : uint4(1 << ",
                                  to_expression(builtin_subgroup_invocation_id_id), ", uint3(0));");
                    }
                });
                break;
            case BuiltInSubgroupGeMask:
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    ...
                    if (msl_options.fixed_subgroup_size > 32) {
                        ...
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(insert_bits(0u, 0xFFFFFFFF, min(",
                                  ...
                                  " (uint)max((int)",
                                  to_expression(builtin_subgroup_invocation_id_id), " - 32, 0), ", msl_options.fixed_subgroup_size, " - max(",
                                  to_expression(builtin_subgroup_invocation_id_id), ", 32u)), uint2(0));");
                    } else if (msl_options.fixed_subgroup_size != 0) {
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(insert_bits(0u, 0xFFFFFFFF, ",
                                  to_expression(builtin_subgroup_invocation_id_id), ", ", msl_options.fixed_subgroup_size, " - ",
                                  to_expression(builtin_subgroup_invocation_id_id), "), uint3(0));");
                    } else if (msl_options.is_ios()) {
                        // On iOS, the SIMD-group size will currently never exceed 32.
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(insert_bits(0u, 0xFFFFFFFF, ",
                                  to_expression(builtin_subgroup_invocation_id_id), ", ", to_expression(builtin_subgroup_size_id), " - ",
                                  to_expression(builtin_subgroup_invocation_id_id), "), uint3(0));");
                    } else {
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(insert_bits(0u, 0xFFFFFFFF, min(",
                                  to_expression(builtin_subgroup_invocation_id_id), ", 32u), (uint)max(min((int)", to_expression(builtin_subgroup_size_id),
                                  ", 32) - (int)", to_expression(builtin_subgroup_invocation_id_id), ", 0)), insert_bits(0u, 0xFFFFFFFF, (uint)max((int)",
                                  to_expression(builtin_subgroup_invocation_id_id), " - 32, 0), (uint)max((int)", to_expression(builtin_subgroup_size_id),
                                  " - (int)max(", to_expression(builtin_subgroup_invocation_id_id), ", 32u), 0)), uint2(0));");
                    }
                });
                break;
            case BuiltInSubgroupGtMask:
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    ...
                    if (msl_options.fixed_subgroup_size > 32) {
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(insert_bits(0u, 0xFFFFFFFF, min(",
                                  ...
                                  to_expression(builtin_subgroup_invocation_id_id), " - 1, 0)), insert_bits(0u, 0xFFFFFFFF, (uint)max((int)",
                                  to_expression(builtin_subgroup_invocation_id_id), " + 1 - 32, 0), ", msl_options.fixed_subgroup_size, " - max(",
                                  to_expression(builtin_subgroup_invocation_id_id), " + 1, 32u)), uint2(0));");
                    } else if (msl_options.fixed_subgroup_size != 0) {
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(insert_bits(0u, 0xFFFFFFFF, ",
                                  to_expression(builtin_subgroup_invocation_id_id), " + 1, ", msl_options.fixed_subgroup_size, " - ",
                                  to_expression(builtin_subgroup_invocation_id_id), " - 1), uint3(0));");
                    } else if (msl_options.is_ios()) {
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(insert_bits(0u, 0xFFFFFFFF, ",
                                  to_expression(builtin_subgroup_invocation_id_id), " + 1, ", to_expression(builtin_subgroup_size_id), " - ",
                                  to_expression(builtin_subgroup_invocation_id_id), " - 1), uint3(0));");
                    } else {
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(insert_bits(0u, 0xFFFFFFFF, min(",
                                  to_expression(builtin_subgroup_invocation_id_id), " + 1, 32u), (uint)max(min((int)", to_expression(builtin_subgroup_size_id),
                                  ", 32) - (int)", to_expression(builtin_subgroup_invocation_id_id), " - 1, 0)), insert_bits(0u, 0xFFFFFFFF, (uint)max((int)",
                                  to_expression(builtin_subgroup_invocation_id_id), " + 1 - 32, 0), (uint)max((int)", to_expression(builtin_subgroup_size_id),
                                  " - (int)max(", to_expression(builtin_subgroup_invocation_id_id), " + 1, 32u), 0)), uint2(0));");
                    }
                });
                break;
            case BuiltInSubgroupLeMask:
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    if (msl_options.is_ios()) {
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(extract_bits(0xFFFFFFFF, 0, ",
                                  to_expression(builtin_subgroup_invocation_id_id), " + 1), uint3(0));");
                    } else {
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(extract_bits(0xFFFFFFFF, 0, min(",
                                  to_expression(builtin_subgroup_invocation_id_id), " + 1, 32u)), extract_bits(0xFFFFFFFF, 0, (uint)max((int)",
            ...
            case BuiltInSubgroupLtMask:
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    if (msl_options.is_ios()) {
                        ...
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = uint4(extract_bits(0xFFFFFFFF, 0, min(",
                                  to_expression(builtin_subgroup_invocation_id_id), ", 32u)), extract_bits(0xFFFFFFFF, 0, (uint)max((int)",
            ...
            case BuiltInViewIndex:
                if (!msl_options.multiview) {
                    ...
                    entry_func.fixup_hooks_in.push_back([=]() { statement("const ", builtin_type_decl(bi_type), " ", to_expression(var_id), " = 0;"); });
                    ...
                    entry_func.fixup_hooks_in.push_back(
                        [=]() { statement("const ", builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", msl_options.device_index, ";"); });
                    ...
                    entry_func.fixup_hooks_in.push_back([=]() {
                        statement("const ", builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(view_mask_buffer_id), "[0];");
                    });
                } else if (get_execution_model() == ExecutionModelFragment) {
                    // Because we adjusted the view index in the vertex shader, we have to
                    // adjust it back here.
                    entry_func.fixup_hooks_in.push_back([=]() { statement(to_expression(var_id), " += ", to_expression(view_mask_buffer_id), "[0];"); });
                    ...
                    entry_func.fixup_hooks_in.push_back([=]() {
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(view_mask_buffer_id), "[0] + (",
                                  to_expression(builtin_instance_idx_id), " - ", to_expression(builtin_base_instance_id), ") % ",
                                  to_expression(view_mask_buffer_id), "[1];");
                        statement(to_expression(builtin_instance_idx_id), " = (", to_expression(builtin_instance_idx_id), " - ",
                                  to_expression(builtin_base_instance_id), ") / ", to_expression(view_mask_buffer_id), "[1] + ",
                    ...
                    entry_func.fixup_hooks_out.push_back(
                        [=]() { statement(to_expression(builtin_layer_id), " = ", to_expression(var_id), " - ", to_expression(view_mask_buffer_id), "[0];"); });
                }
                break;
            case BuiltInDeviceIndex:
            ...
            case BuiltInGlobalInvocationId:
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    ...
                    if (workgroup_size_id)
                        statement(to_expression(var_id), " += ", to_dereferenced_expression(builtin_dispatch_base_id), " * ", to_expression(workgroup_size_id),
                                  ";");
                    else
                        statement(to_expression(var_id), " += ", to_dereferenced_expression(builtin_dispatch_base_id), " * uint3(", execution.workgroup_size.x,
                                  ", ", execution.workgroup_size.y, ", ", execution.workgroup_size.z, ");");
                });
                break;
            case BuiltInVertexId:
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    ...
                    case Options::IndexType::UInt32:
                        statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", index_buffer_var_name, "[",
                                  to_expression(builtin_invocation_id_id), ".x] + ", to_expression(builtin_dispatch_base_id), ".x;");
                        break;
                    }
                    builtin_declaration = false;
                });
            ...
            case BuiltInBaseVertex:
                ...
                entry_func.fixup_hooks_in.push_back(
                    [=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_dispatch_base_id), ".x;"); });
                break;
            case BuiltInInstanceId:
            case BuiltInInstanceIndex:
                // This is direct-mapped normally.
                ...
                entry_func.fixup_hooks_in.push_back([=]() {
                    builtin_declaration = true;
                    statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_invocation_id_id), ".y + ",
            ...
            case BuiltInBaseInstance:
                ...
                entry_func.fixup_hooks_in.push_back(
                    [=]() { statement(builtin_type_decl(bi_type), " ", to_expression(var_id), " = ", to_expression(builtin_dispatch_base_id), ".y;"); });
                break;
            default:
                break;
            }
        } else if (var.storage == StorageClassOutput && get_execution_model() == ExecutionModelFragment && is_builtin_variable(var) &&
                   active_output_builtins.get(bi_type)) {
            switch (bi_type) {
            case BuiltInSampleMask:
...
uint32_t CompilerMSL::get_metal_resource_index(SPIRVariable& var, SPIRType::BaseType basetype, uint32_t plane) {
    ...
    bool use_secondary_binding = (var_type.basetype == SPIRType::SampledImage && basetype == SPIRType::Sampler) || basetype == SPIRType::AtomicCounter;

    auto resource_decoration = use_secondary_binding ? SPIRVCrossDecorationResourceIndexSecondary : SPIRVCrossDecorationResourceIndexPrimary;
...
bool CompilerMSL::type_is_msl_framebuffer_fetch(const SPIRType& type) const {
    return type.basetype == SPIRType::Image && type.image.dim == DimSubpassData && msl_options.use_framebuffer_fetch_subpasses;
}

const char* CompilerMSL::descriptor_address_space(uint32_t id, StorageClass storage, const char* plain_address_space) const {
    if (msl_options.argument_buffers) {
        bool storage_class_is_descriptor = storage == StorageClassUniform || storage == StorageClassStorageBuffer || storage == StorageClassUniformConstant;
...
string CompilerMSL::argument_decl(const SPIRFunction::Parameter& arg) {
    ...
    if (type_is_msl_framebuffer_fetch(type) || type_storage == StorageClassStorageBuffer || type_storage == StorageClassUniform ||
    ...
    bool type_is_image = type.basetype == SPIRType::Image || type.basetype == SPIRType::SampledImage || type.basetype == SPIRType::Sampler;
    ...
    bool is_dynamic_img_sampler = !arg.alias_global_variable && type.basetype == SPIRType::SampledImage && type.image.dim == Dim2D &&
                                  type_is_floating_point(get<SPIRType>(type.image.type)) && spv_function_implementations.count(SPVFuncImplDynamicImageSampler);
    ...
    if (var.basevariable && (var.basevariable == stage_in_ptr_var_id || var.basevariable == stage_out_ptr_var_id))
        ...
        if (storage == StorageClassInput && (builtin_type == BuiltInTessLevelInner || builtin_type == BuiltInTessLevelOuter)) {
            is_using_builtin_array = false;
        } else if (builtin_type != BuiltInClipDistance && builtin_type != BuiltInCullDistance) {
            is_using_builtin_array = true;
        }

        if (storage == StorageClassOutput && variable_storage_requires_stage_io(storage) && !is_stage_output_builtin_masked(builtin_type))
            is_using_builtin_array = true;
    ...
    if (passed_by_value || (!builtin && !is_pointer(var_type) && (type_storage == StorageClassFunction || type_storage == StorageClassGeneric))) {
        ...
        if (msl_options.force_native_arrays && is_array(type)) {
            ...
            bool pointer_to_logical_buffer_array = !is_physical_pointer(type) && is_pointer(type) && has_decoration(type.parent_type, DecorationArrayStride);
        ...
        if (is_tesc_shader() && builtin && (builtin_type == BuiltInTessLevelInner || builtin_type == BuiltInTessLevelOuter)) {
    ...
    } else if (!type_is_image && !type_is_tlas && (!pull_model_inputs.count(var.basevariable) || type.basetype == SPIRType::Struct)) {
...
string CompilerMSL::to_member_reference(uint32_t base, const SPIRType& type, uint32_t index, bool ptr_chain_is_resolved) {
    ...
    if (var) {
        ...
        const bool is_block = has_decoration(type.self, DecorationBlock) || has_decoration(type.self, DecorationBufferBlock);

        bool is_buffer_variable = is_block && (var->storage == StorageClassUniform || var->storage == StorageClassStorageBuffer);
...
string CompilerMSL::type_to_glsl(const SPIRType& type, uint32_t id, bool member) {
    ...
    if (is_pointer(type) || type_is_array_of_pointers(type)) {
        ...
        if (member && id) {
            ...
            if (is_var_runtime_size_array(var) && is_runtime_size_array(*p_parent_type)) {
                ...
                bool buffer_desc = (var.storage == StorageClassStorageBuffer || ssbo) && msl_options.runtime_array_rich_descriptor;
    ...
    case SPIRType::Interpolant:
        return join("interpolant<", type_to_glsl(get<SPIRType>(type.parent_type), id),
                    ", interpolation::", has_decoration(type.self, DecorationNoPerspective) ? "no_perspective" : "perspective", ">");
    ...
    if (type.columns > 1) {
        ...
        if (((var && var->storage == StorageClassWorkgroup) || type.storage == StorageClassWorkgroup) && !msl_options.supports_msl_version(3, 0)) {
            add_spv_func_and_recompile(SPVFuncImplStorageMatrix);
...
string CompilerMSL::type_to_array_glsl(const SPIRType& type, uint32_t variable_id) {
    ...
    default:
        if (type_is_array_of_pointers(type) || using_builtin_array()) {
            ...
            if (var && (var->storage == StorageClassUniform || var->storage == StorageClassStorageBuffer) && is_array(get_variable_data_type(*var))) {
...
bool CompilerMSL::variable_decl_is_remapped_storage(const SPIRVariable& variable, StorageClass storage) const {
    ...
    if (storage == StorageClassWorkgroup) {
        ...
        if (is_tesc_shader() && variable.storage == StorageClassOutput && has_decoration(get<SPIRType>(variable.basetype).self, DecorationBlock)) {
            return true;
        }
        ...
        return (variable.storage == StorageClassOutput || variable.storage == StorageClassInput) && !variable_storage_requires_stage_io(variable.storage) &&
...
std::string CompilerMSL::sampler_type(const SPIRType& type, uint32_t id, bool member) {
    ...
    if (!type.array.empty()) {
        ...
        if (array_size == 0) {
            ...
            return join(descriptor_wrapper, "<", sampler_type(parent, id, false), ">", processing_entry_point ? "*" : "");
        } else {
...
string CompilerMSL::image_type_glsl(const SPIRType& type, uint32_t id, bool member) {
    ...
    if (!type.array.empty()) {
        ...
        if (array_size == 0) {
            ...
            return join(descriptor_wrapper, "<", image_type_glsl(parent, id, false), ">", processing_entry_point ? "*" : "");
        } else {
    ...
    if (is_depth_image(type, id)) {
        ...
        case Dim2D:
        case DimSubpassData: {
            bool subpass_array = img_type.dim == DimSubpassData && (msl_options.multiview || msl_options.arrayed_subpass_input);
            if (img_type.dim == Dim1D && !msl_options.texture_1D_as_2D) {
                // Use a native Metal 1D texture
...
void CompilerMSL::emit_subgroup_op(const Instruction& i) {
    ...
    case OpGroupNonUniformBallotBitCount: {
        ...
        case GroupOperationReduce:
            ...
            break;
        case GroupOperationInclusiveScan:
            emit_binary_func_op(result_type, id, ops[op_idx], builtin_subgroup_invocation_id_id, "spvSubgroupBallotInclusiveBitCount");
            break;
        case GroupOperationExclusiveScan:
            emit_binary_func_op(result_type, id, ops[op_idx], builtin_subgroup_invocation_id_id, "spvSubgroupBallotExclusiveBitCount");
            break;
        default:
    ...
    case OpGroupNonUniformRotateKHR: {
        if (i.length > 5) {
            ...
            auto local_id_expr = to_unpacked_expression(scope == ScopeSubgroup ? builtin_subgroup_invocation_id_id : builtin_local_invocation_index_id);
            auto shuffle_idx = join("((", local_id_expr, " + ", delta_expr, ")", " & ", std::to_string(cluster_size_minus_one), ") + (", local_id_expr, " & ",
                                    std::to_string(~cluster_size_minus_one), ")");
...
void CompilerMSL::emit_subgroup_cluster_op(uint32_t result_type, uint32_t result_id, uint32_t cluster_size, uint32_t op0, const char* op) {
    if (get_execution_model() == ExecutionModelFragment && !msl_options.supports_msl_version(2, 2)) {
        if (cluster_size == 4) {
    ...
    emit_op(result_type, result_id,
            join("spvClustered_", op, "<", cluster_size, ">(", to_unpacked_expression(op0), ", ", to_expression(builtin_subgroup_invocation_id_id), ")"),
            forward);
    inherit_expression_dependencies(result_id, op0);
}

void CompilerMSL::emit_subgroup_cluster_op_cast(uint32_t result_type, uint32_t result_id, uint32_t cluster_size, uint32_t op0, const char* op,
                                                SPIRType::BaseType input_type, SPIRType::BaseType expected_result_type) {
    if (get_execution_model() == ExecutionModelFragment && !msl_options.supports_msl_version(2, 2)) {
        if (cluster_size == 4) {
            emit_unary_func_op_cast(result_type, result_id, op0, join("quad_", op).c_str(), input_type, expected_result_type);
            return;
        }
        SPIRV_CROSS_THROW("Cluster sizes other than 4 in fragment shaders require MSL 2.2.");
    }
...
string CompilerMSL::builtin_to_glsl(BuiltIn builtin, StorageClass storage) {
    ...
    case BuiltInLayer:
        ...
        if (is_mesh_shader())
            break;
        if (storage != StorageClassInput && current_function && (current_function->self == ir.default_entry_point) && !is_stage_output_builtin_masked(builtin))
    ...
    case BuiltInTessLevelOuter:
        if (is_tesc_shader() && storage != StorageClassInput && current_function && (current_function->self == ir.default_entry_point)) {
            return join(tess_factor_buffer_var_name, "[", to_expression(builtin_primitive_id_id), "].edgeTessellationFactor");
        }
        break;

    case BuiltInTessLevelInner:
        if (is_tesc_shader() && storage != StorageClassInput && current_function && (current_function->self == ir.default_entry_point)) {
            return join(tess_factor_buffer_var_name, "[", to_expression(builtin_primitive_id_id), "].insideTessellationFactor");
        }
        break;
...
string CompilerMSL::builtin_qualifier(BuiltIn builtin) {
    ...
    case BuiltInSubgroupLocalInvocationId:
        ...
        } else if (execution.model == ExecutionModelKernel || execution.model == ExecutionModelGLCompute || execution.model == ExecutionModelTaskEXT ||
                   execution.model == ExecutionModelMeshEXT || execution.model == ExecutionModelTessellationControl ||
...
uint32_t CompilerMSL::get_declared_struct_member_array_stride_msl(const SPIRType& type, uint32_t index) const {
    return get_declared_type_array_stride_msl(get_physical_member_type_id(type, index), nullptr, member_is_packed_physical_type(type, index),
...
uint32_t CompilerMSL::get_declared_input_array_stride_msl(const SPIRType& type, uint32_t index) const {
    ...
    return get_declared_type_array_stride_msl(0, &presumed_type, false, has_member_decoration(type.self, index, DecorationRowMajor));
}

uint32_t CompilerMSL::get_declared_type_matrix_stride_msl(TypeID type_id, const SPIRType* special_type, bool packed, bool row_major) const {
    auto& type = type_id ? get<SPIRType>(type_id) : *special_type;
...
uint32_t CompilerMSL::get_declared_struct_member_matrix_stride_msl(const SPIRType& type, uint32_t index) const {
    return get_declared_type_matrix_stride_msl(get_physical_member_type_id(type, index), nullptr, member_is_packed_physical_type(type, index),
                                               has_member_decoration(type.self, index, DecorationRowMajor));
}

uint32_t CompilerMSL::get_declared_input_matrix_stride_msl(const SPIRType& type, uint32_t index) const {
    auto presumed_type = get_presumed_input_type(type, index);
    return get_declared_type_matrix_stride_msl(0, &presumed_type, false, has_member_decoration(type.self, index, DecorationRowMajor));
}
...
uint32_t CompilerMSL::get_declared_type_size_msl(TypeID type_id, const SPIRType* special_type, bool is_packed, bool row_major) const {
    auto& type = type_id ? get<SPIRType>(type_id) : *special_type;
...
uint32_t CompilerMSL::get_declared_struct_member_size_msl(const SPIRType& type, uint32_t index) const {
    return get_declared_type_size_msl(get_physical_member_type_id(type, index), nullptr, member_is_packed_physical_type(type, index),
                                      has_member_decoration(type.self, index, DecorationRowMajor));
}

uint32_t CompilerMSL::get_declared_input_size_msl(const SPIRType& type, uint32_t index) const {
    auto presumed_type = get_presumed_input_type(type, index);
    return get_declared_type_size_msl(0, &presumed_type, false, has_member_decoration(type.self, index, DecorationRowMajor));
}

// Returns the byte alignment of a type.
uint32_t CompilerMSL::get_declared_type_alignment_msl(TypeID type_id, const SPIRType* special_type, bool is_packed, bool row_major) const {
    auto& type = type_id ? get<SPIRType>(type_id) : *special_type;
...
uint32_t CompilerMSL::get_declared_struct_member_alignment_msl(const SPIRType& type, uint32_t index) const {
    return get_declared_type_alignment_msl(get_physical_member_type_id(type, index), nullptr, member_is_packed_physical_type(type, index),
                                           has_member_decoration(type.self, index, DecorationRowMajor));
}

uint32_t CompilerMSL::get_declared_input_alignment_msl(const SPIRType& type, uint32_t index) const {
    auto presumed_type = get_presumed_input_type(type, index);
    return get_declared_type_alignment_msl(0, &presumed_type, false, has_member_decoration(type.self, index, DecorationRowMajor));
}
...
bool CompilerMSL::SampledImageScanner::handle(Op opcode, const uint32_t* args, uint32_t length) {
    ...
    case OpImageDrefGather:
        self.has_sampled_images = self.has_sampled_images || self.is_sampled_image_type(self.expression_type(args[2]));
...
bool CompilerMSL::OpCodePreprocessor::handle(Op opcode, const uint32_t* args, uint32_t length) {
    ...
    case OpGroupNonUniformLogicalOr:
    case OpGroupNonUniformLogicalXor:
        if ((compiler.get_execution_model() != ExecutionModelFragment || self.msl_options.supports_msl_version(2, 2)) &&
...
CompilerMSL::MemberSorter::MemberSorter(SPIRType& t, Meta& m, SortAspect sa) : type(t), meta(m), sort_aspect(sa) {
...
void CompilerMSL::cast_from_variable_load(uint32_t source_id, std::string& expr, const SPIRType& expr_type) {
    ...
    if (!msl_options.supports_msl_version(3, 0) && var &&
        (var->storage == StorageClassWorkgroup ||
         (var_type->basetype == SPIRType::Struct && has_extended_decoration(var_type->self, SPIRVCrossDecorationWorkgroupStruct) && !is_packed)) &&
        expr_type.columns > 1) {
...
bool CompilerMSL::is_supported_argument_buffer_type(const SPIRType& type) const {
    ...
    bool is_supported_type = !(type.basetype == SPIRType::Image && type.image.sampled == 2 && msl_options.is_ios() &&
...
void CompilerMSL::emit_argument_buffer_aliased_descriptor(const SPIRVariable& aliased_var, const SPIRVariable& base_var) {
    ...
    if (aliased_var.storage == StorageClassUniformConstant) {
        if (is_var_runtime_size_array(aliased_var)) {
            ...
            name = join("reinterpret_cast<", descriptor_storage, " ", type_to_glsl(get_variable_data_type(aliased_var), aliased_var.self, true), ">(&",
                        unqualified_name, ")");
        } else {
            name = join("reinterpret_cast<", descriptor_storage, " ", type_to_glsl(get_variable_data_type(aliased_var), aliased_var.self, true), " &>(",
                        unqualified_name, ");");
        ...
        if (is_var_runtime_size_array(aliased_var)) {
            name = join("reinterpret_cast<", type_to_glsl(var_type, aliased_var.self, true), " ", descriptor_storage, " *>(&", unqualified_name, ")");
        } else {
            name = join(needs_post_cast_deref ? "*" : "", "reinterpret_cast<", type_to_glsl(var_type, aliased_var.self, true), " ", descriptor_storage, " ",
                        ref_type, ">(", unqualified_name, ");");
        }
...
void CompilerMSL::analyze_argument_buffers() {
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t self, SPIRVariable& var) {
        if ((var.storage == StorageClassUniform || var.storage == StorageClassUniformConstant || var.storage == StorageClassStorageBuffer) &&
            !is_hidden_variable(var)) {
            ...
            if (type.basetype == SPIRType::SampledImage) {
                ...
                for (uint32_t i = 0; i < plane_count; i++) {
                    ...
                    resources_in_set[desc_set].push_back({&var, to_name(var_id), SPIRType::Image, image_resource_index, plane_count, i, 0});
                }

                if (type.image.dim != DimBuffer && !constexpr_sampler) {
                    uint32_t sampler_resource_index = get_metal_resource_index(var, SPIRType::Sampler);
                    resources_in_set[desc_set].push_back({&var, to_sampler_expression(var_id), SPIRType::Sampler, sampler_resource_index, 1, 0, 0});
                }
                ...
                resources_in_set[desc_set].push_back({&var, to_name(var_id), type.basetype, resource_index, 1, 0, 0});

                // Emulate texture2D atomic operations
                if (atomic_image_vars_emulated.count(var.self)) {
                    uint32_t buffer_resource_index = get_metal_resource_index(var, SPIRType::AtomicCounter, 0);
                    resources_in_set[desc_set].push_back({&var, to_name(var_id) + "_atomic", SPIRType::Struct, buffer_resource_index, 1, 0, 0});
                }
            }
    ...
    if (needs_swizzle_buffer_def || needs_buffer_sizes) {
        ...
        for (uint32_t desc_set = 0; desc_set < kMaxArgumentBuffers; desc_set++) {
            ...
            if (set_needs_swizzle_buffer[desc_set]) {
                ...
                resources_in_set[desc_set].push_back({&var, to_name(var_id), SPIRType::UInt, get_metal_resource_index(var, SPIRType::UInt), 1, 0, 0});
            }
    ...
    for (uint32_t var_id : inline_block_vars) {
        ...
        uint32_t desc_set = get_decoration(var_id, DecorationDescriptorSet);
        add_resource_name(var_id);
        resources_in_set[desc_set].push_back({&var, to_name(var_id), SPIRType::Struct, get_metal_resource_index(var, SPIRType::Struct), 1, 0, 0});
    }
    ...
    for (uint32_t desc_set = 0; desc_set < kMaxArgumentBuffers; desc_set++) {
        ...
        stable_sort(begin(resources), end(resources),
                    [&](const Resource& lhs, const Resource& rhs) -> bool { return tie(lhs.index, lhs.basetype) < tie(rhs.index, rhs.basetype); });
        ...
        for (auto& resource : resources) {
            ...
            if (resource.basetype == SPIRType::Sampler && type.basetype != SPIRType::Sampler) {
                ...
                if (resource.basetype == SPIRType::Image || resource.basetype == SPIRType::Sampler || resource.basetype == SPIRType::SampledImage ||
                    ...
                    if (has_extended_decoration(var.self, SPIRVCrossDecorationOverlappingBinding)) {
                        ...
                        if (get<SPIRVariable>(resource.overlapping_var_id).storage == StorageClassUniformConstant && !msl_options.supports_msl_version(3, 0)) {
                            SPIRV_CROSS_THROW("Full mutable aliasing of argument buffer descriptors only works on Metal 3+.");
                        }
            ...
            set_extended_member_decoration(buffer_type.self, member_index, SPIRVCrossDecorationResourceIndexPrimary, resource.index);
            set_extended_member_decoration(buffer_type.self, member_index, SPIRVCrossDecorationInterfaceOrigID, var.self);
            if (has_extended_decoration(var.self, SPIRVCrossDecorationOverlappingBinding))
...
void CompilerMSL::add_argument_buffer_padding_buffer_type(SPIRType& struct_type, uint32_t& mbr_idx, uint32_t& arg_buff_index, MSLResourceBinding& rez_bind) {
...
void CompilerMSL::add_argument_buffer_padding_image_type(SPIRType& struct_type, uint32_t& mbr_idx, uint32_t& arg_buff_index, MSLResourceBinding& rez_bind) {
...
void CompilerMSL::add_argument_buffer_padding_sampler_type(SPIRType& struct_type, uint32_t& mbr_idx, uint32_t& arg_buff_index, MSLResourceBinding& rez_bind) {
...
void CompilerMSL::add_argument_buffer_padding_type(uint32_t mbr_type_id, SPIRType& struct_type, uint32_t& mbr_idx, uint32_t& arg_buff_index, uint32_t count) {
    ...
    if (mesh_out_per_vertex != 0) {
        ...
        for (uint32_t index = 0; index < uint32_t(type_vert.member_types.size()); ++index) {
            ...
            if (options.vertex.flip_vert_y && builtin == BuiltInPosition) {
                statement("spvV.", to_member_name(type_vert, index), ".y = -(", "spvV.", to_member_name(type_vert, index), ".y);",
...
void CompilerMSL::emit_mesh_tasks(SPIRBlock& block) {
    ...
    statement("spvMgp.set_threadgroups_per_grid(uint3(", to_unpacked_expression(block.mesh.groups[0]), ", ", to_unpacked_expression(block.mesh.groups[1]), ", ",
