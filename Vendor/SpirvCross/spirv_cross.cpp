...
bool Compiler::variable_storage_is_aliased(const SPIRVariable& v) {
    ...
    bool ssbo = v.storage == StorageClassStorageBuffer || ir.meta[type.self].decoration.decoration_flags.get(DecorationBufferBlock);
...
bool Compiler::block_is_pure(const SPIRBlock& block) {
    ...
    if (block.terminator == SPIRBlock::Kill || block.terminator == SPIRBlock::TerminateRay || block.terminator == SPIRBlock::IgnoreIntersection ||
...
bool Compiler::is_immutable(uint32_t id) const {
    ...
    else if (ir.ids[id].get_type() == TypeConstant || ir.ids[id].get_type() == TypeConstantOp || ir.ids[id].get_type() == TypeUndef)
        return true;
    else
        return false;
}
...
bool Compiler::is_hidden_variable(const SPIRVariable& var, bool include_builtins) const {
    ...
    if (find_if(begin(combined_image_samplers), end(combined_image_samplers),
                [&var](const CombinedImageSampler& samp) { return samp.combined_id == var.self; }) != end(combined_image_samplers)) {
        return false;
    }
    ...
    if (ir.get_spirv_version() >= 0x10400 && var.storage != StorageClassGeneric && var.storage != StorageClassFunction &&
...
bool Compiler::is_physical_or_buffer_pointer(const SPIRType& type) const {
    return type.op == OpTypePointer &&
           (type.storage == StorageClassPhysicalStorageBuffer || type.storage == StorageClassUniform || type.storage == StorageClassStorageBuffer ||
            type.storage == StorageClassWorkgroup || type.storage == StorageClassPushConstant);
}
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable& var) {
        ...
        if (is_builtin) {
            ...
            if (has_decoration(type.self, DecorationBlock)) {
                resource.resource = {var.self, var.basetype, type.self, get_remapped_declared_block_name(var.self, false)};
                ...
                bool strip_array = !has_decoration(var.self, DecorationPatch) &&
                                   (get_execution_model() == ExecutionModelTessellationControl ||
                                    (get_execution_model() == ExecutionModelTessellationEvaluation && var.storage == StorageClassInput));
        ...
        if (var.storage == StorageClassInput) {
            if (has_decoration(type.self, DecorationBlock)) {
                res.stage_inputs.push_back({var.self, var.basetype, type.self, get_remapped_declared_block_name(var.self, false)});
            } else
        ...
        // Outputs
            ...
            if (has_decoration(type.self, DecorationBlock)) {
                res.stage_outputs.push_back({var.self, var.basetype, type.self, get_remapped_declared_block_name(var.self, false)});
            } else
        ...
        // UBOs
            ...
            res.uniform_buffers.push_back({var.self, var.basetype, type.self, get_remapped_declared_block_name(var.self, false)});
        }
        // Old way to declare SSBOs.
        else if (type.storage == StorageClassUniform && has_decoration(type.self, DecorationBufferBlock)) {
            res.storage_buffers.push_back({var.self, var.basetype, type.self, get_remapped_declared_block_name(var.self, ssbo_instance_name)});
        }
...
void Compiler::parse_fixup() {
    ...
    for (auto id_ : ir.ids_for_constant_or_variable) {
        ...
        if (id.get_type() == TypeConstant) {
            ...
            if (has_decoration(c.self, DecorationBuiltIn) && BuiltIn(get_decoration(c.self, DecorationBuiltIn)) == BuiltInWorkgroupSize) {
...
void Compiler::update_name_cache(unordered_set<string>& cache_primary, const unordered_set<string>& cache_secondary, string& name) {
    if (name.empty())
        return;
...
bool Compiler::is_sampled_image_type(const SPIRType& type) {
    return (type.basetype == SPIRType::Image || type.basetype == SPIRType::SampledImage) && type.image.sampled == 1 && type.image.dim != DimBuffer;
}

void Compiler::set_member_decoration_string(TypeID id, uint32_t index, Decoration decoration, const std::string& argument) {
    ...
    for (auto& i : block.ops) {
        ...
        case OpExtInst: {
            ...
            bool ext_is_nonsemantic_only =
                ext == SPIRExtension::NonSemanticShaderDebugInfo || ext == SPIRExtension::SPV_debug_info || ext == SPIRExtension::NonSemanticGeneric;
...
bool Compiler::block_is_loop_candidate(const SPIRBlock& block, SPIRBlock::Method method) const {
    ...
    if (method == SPIRBlock::MergeToSelectForLoop || method == SPIRBlock::MergeToSelectContinueForLoop) {
        ...
        bool false_block_is_merge = block.false_block == block.merge_block || (false_block && merge_block && execution_is_noop(*false_block, *merge_block));

        bool true_block_is_merge = block.true_block == block.merge_block || (true_block && merge_block && execution_is_noop(*true_block, *merge_block));

        bool positive_candidate = block.true_block != block.merge_block && block.true_block != block.self && false_block_is_merge;

        bool negative_candidate = block.false_block != block.merge_block && block.false_block != block.self && true_block_is_merge;

        bool ret = block.terminator == SPIRBlock::Select && block.merge == SPIRBlock::MergeLoop && (positive_candidate || negative_candidate);
        ...
        bool false_block_is_merge = child.false_block == block.merge_block || (false_block && merge_block && execution_is_noop(*false_block, *merge_block));

        bool true_block_is_merge = child.true_block == block.merge_block || (true_block && merge_block && execution_is_noop(*true_block, *merge_block));

        bool positive_candidate = child.true_block != block.merge_block && child.true_block != block.self && false_block_is_merge;

        bool negative_candidate = child.false_block != block.merge_block && child.false_block != block.self && true_block_is_merge;

        ret = child.terminator == SPIRBlock::Select && child.merge == SPIRBlock::MergeNone && (positive_candidate || negative_candidate);
...
SPIRBlock::ContinueBlockType Compiler::continue_block_type(const SPIRBlock& block) const {
    ...
    if (execution_is_noop(block, dominator))
        ...
        bool positive_do_while = block.true_block == dominator.self &&
                                 (block.false_block == dominator.merge_block || (false_block && merge_block && execution_is_noop(*false_block, *merge_block)));

        bool negative_do_while = block.false_block == dominator.self &&
                                 (block.true_block == dominator.merge_block || (true_block && merge_block && execution_is_noop(*true_block, *merge_block)));

        if (block.merge == SPIRBlock::MergeNone && block.terminator == SPIRBlock::Select && (positive_do_while || negative_do_while)) {
...
uint32_t Compiler::get_work_group_size_specialization_constants(SpecializationConstant& x, SpecializationConstant& y, SpecializationConstant& z) const {
    auto& execution = get_entry_point();
    x = {0, 0};
    y = {0, 0};
    z = {0, 0};
...
bool Compiler::is_vertex_like_shader() const {
    ...
    return model == ExecutionModelVertex || model == ExecutionModelGeometry || model == ExecutionModelTessellationControl ||
...
void Compiler::inherit_expression_dependencies(uint32_t dst, uint32_t source_expression) {
    ...
    if (forwarded_temporaries.find(dst) == end(forwarded_temporaries) || forced_temporaries.find(dst) != end(forced_temporaries)) {
        return;
    }
...
SPIREntryPoint& Compiler::get_entry_point(const std::string& name, ExecutionModel model) {
    auto itr = find_if(begin(ir.entry_points), end(ir.entry_points),
                       [&](const std::pair<uint32_t, SPIREntryPoint>& entry) -> bool { return entry.second.orig_name == name && entry.second.model == model; });
...
bool Compiler::interface_variable_exists_in_entry_point(uint32_t id) const {
    ...
    auto& execution = get_entry_point();
    return find(begin(execution.interface_variables), end(execution.interface_variables), VariableID(id)) != end(execution.interface_variables);
}

void Compiler::CombinedImageSamplerHandler::push_remap_parameters(const SPIRFunction& func, const uint32_t* args, uint32_t length) {
...
void Compiler::CombinedImageSamplerHandler::register_combined_image_sampler(SPIRFunction& caller, VariableID combined_module_id, VariableID image_id,
    ...
    auto texture_itr = find_if(begin(caller.arguments), end(caller.arguments), [image_id](const SPIRFunction::Parameter& p) { return p.id == image_id; });
    auto sampler_itr = find_if(begin(caller.arguments), end(caller.arguments), [sampler_id](const SPIRFunction::Parameter& p) { return p.id == sampler_id; });
    ...
    auto itr = find_if(begin(caller.combined_parameters), end(caller.combined_parameters), [&param](const SPIRFunction::CombinedImageSamplerParameter& p) {
        return param.image_id == p.image_id && param.sampler_id == p.sampler_id && param.global_image == p.global_image &&
    ...
    if (itr == end(caller.combined_parameters)) {
        ...
        bool relaxed_precision = compiler.has_decoration(sampler_id, DecorationRelaxedPrecision) ||
        ...
        compiler.set_name(combined_id, join("SPIRV_Cross_Combined", compiler.to_name(image_id), compiler.to_name(sampler_id)));
...
bool Compiler::CombinedImageSamplerHandler::handle(Op opcode, const uint32_t* args, uint32_t length) {
    ...
    auto itr =
        find_if(begin(compiler.combined_image_samplers), end(compiler.combined_image_samplers),
                [image_id, sampler_id](const CombinedImageSampler& combined) { return combined.image_id == image_id && combined.sampler_id == sampler_id; });
    ...
    if (itr == end(compiler.combined_image_samplers)) {
        ...
        // If any of OpSampledImage, underlying image or sampler are marked, inherit the decoration.
        bool relaxed_precision = (sampler_id && compiler.has_decoration(sampler_id, DecorationRelaxedPrecision)) ||
...
static bool exists_unaccessed_path_to_return(const CFG& cfg, uint32_t block, const unordered_set<uint32_t>& blocks, unordered_set<uint32_t>& visit_cache) {
...
void Compiler::analyze_parameter_preservation(SPIRFunction& entry, const CFG& cfg, const unordered_map<uint32_t, unordered_set<uint32_t>>& variable_to_blocks,
...
Compiler::AnalyzeVariableScopeAccessHandler::AnalyzeVariableScopeAccessHandler(Compiler& compiler_, SPIRFunction& entry_)
    : OpcodeHandler(compiler_), entry(entry_) {
}
...
Compiler::StaticExpressionAccessHandler::StaticExpressionAccessHandler(Compiler& compiler_, uint32_t variable_id_)
    : OpcodeHandler(compiler_), variable_id(variable_id_) {
}
...
void Compiler::find_function_local_luts(SPIRFunction& entry, const AnalyzeVariableScopeAccessHandler& handler, bool single_function) {
    ...
    for (auto& accessed_var : handler.accessed_variables_to_block) {
        ...
        if (!var.is_written_to) {
            var.is_written_to = handler.complete_write_variables_to_block.count(var.self) != 0 || handler.partial_write_variables_to_block.count(var.self) != 0;
        }
...
void Compiler::analyze_variable_scope(SPIRFunction& entry, AnalyzeVariableScopeAccessHandler& handler) {
    ...
    analyze_parameter_preservation(entry, cfg, handler.accessed_variables_to_block, handler.complete_write_variables_to_block);
    ...
    for (auto& var : handler.accessed_variables_to_block) {
        ...
        if (find(begin(entry.local_variables), end(entry.local_variables), VariableID(var.first)) == end(entry.local_variables))
            continue;
        ...
        if (dominating_block && potential_continue_block != 0 && potential_continue_block != ~0u) {
            ...
            if (merge_candidate != 0 && cfg.is_reachable(merge_candidate)) {
                ...
                if (!cfg.is_reachable(potential_continue_block) || cfg.get_visit_order(merge_candidate) > cfg.get_visit_order(potential_continue_block)) {
...
bool Compiler::ActiveBuiltinHandler::handle(Op opcode, const uint32_t* args, uint32_t length) {
    ...
    case OpPtrAccessChain: {
        ...
        auto& flags = var->storage == StorageClassInput ? compiler.active_input_builtins : compiler.active_output_builtins;
        ...
        for (uint32_t i = 0; i < count; i++) {
            ...
            // Structs
                ...
                if (index < uint32_t(compiler.ir.meta[type->self].members.size())) {
                    ...
                    if (decorations.builtin) {
                        flags.set(decorations.builtin_type);
                        handle_builtin(compiler.get<SPIRType>(type->member_types[index]), decorations.builtin_type, decorations.decoration_flags);
                    }
                }
...
void Compiler::build_function_control_flow_graphs_and_analyze() {
    ...
    for (auto& f : function_cfgs) {
        ...
        for (auto block : func.blocks) {
            ...
            for (auto loop_variable : b.loop_variables) {
                if (flags != get_decoration_bitset(loop_variable) || type != get<SPIRVariable>(b.loop_variables.front()).basetype) {
    ...
    if (!single_function) {
        for (auto& id : global_variables) {
            ...
            if (is_array(type) && var.storage == StorageClassPrivate && var.initializer && !var.is_written_to &&
...
Compiler::CFGBuilder::CFGBuilder(Compiler& compiler_) : OpcodeHandler(compiler_) {
}
...
bool Compiler::reflection_ssbo_instance_name_is_significant() const {
    ...
    ir.for_each_typed_id<SPIRVariable>([&](uint32_t, const SPIRVariable& var) {
        ...
        bool ssbo = var.storage == StorageClassStorageBuffer || (var.storage == StorageClassUniform && has_decoration(type.self, DecorationBufferBlock));
...
bool Compiler::instruction_to_result_type(uint32_t& result_type, uint32_t& result_id, Op op, const uint32_t* args, uint32_t length) {
    if (length < 2)
        return false;
...
bool Compiler::type_is_opaque_value(const SPIRType& type) const {
    return !type.pointer && (type.basetype == SPIRType::SampledImage || type.basetype == SPIRType::Image || type.basetype == SPIRType::Sampler ||
...
Compiler::PhysicalStorageBufferPointerHandler::PhysicalStorageBufferPointerHandler(Compiler& compiler_) : OpcodeHandler(compiler_) {
}
...
void Compiler::analyze_non_block_pointer_types() {
    ...
    ir.for_each_typed_id<SPIRType>([&](uint32_t id, SPIRType& type) {
        ...
        if (type.self == id && (has_decoration(type.self, DecorationBlock) || has_decoration(type.self, DecorationBufferBlock))) {
...
void Compiler::InterlockedResourceAccessHandler::access_potential_resource(uint32_t id) {
    if ((use_critical_section && in_crit_sec) || (control_flow_interlock && call_stack_is_interlocked) || split_function_case) {
...
bool Compiler::InterlockedResourceAccessHandler::handle(Op opcode, const uint32_t* args, uint32_t length) {
    ...
    case OpPtrAccessChain: {
        ...
        auto& type = compiler.get<SPIRType>(result_type);
        if (type.storage == StorageClassUniform || type.storage == StorageClassUniformConstant || type.storage == StorageClassStorageBuffer) {
            uint32_t id = args[1];
            uint32_t ptr = args[2];
            compiler.set<SPIRExpression>(id, "", result_type, true);
            compiler.register_read(id, ptr, true);
    ...
    case OpCooperativeVectorStoreNV: {
        ...
        uint32_t ptr = args[0];
        auto* var = compiler.maybe_get_backing_variable(ptr);
        if (var && (var->storage == StorageClassUniform || var->storage == StorageClassUniformConstant || var->storage == StorageClassStorageBuffer)) {
            access_potential_resource(var->self);
        }
    ...
    case OpCopyMemory: {
        ...
        if (src_var) {
            ...
            if (src_var->storage == StorageClassUniform && !compiler.has_decoration(compiler.get<SPIRType>(src_var->basetype).self, DecorationBufferBlock)) {
                break;
            }
...
void Compiler::analyze_interlocked_resource_usage() {
    if (get_execution_model() == ExecutionModelFragment &&
        (get_entry_point().flags.get(ExecutionModePixelInterlockOrderedEXT) || get_entry_point().flags.get(ExecutionModePixelInterlockUnorderedEXT) ||
         get_entry_point().flags.get(ExecutionModeSampleInterlockOrderedEXT) || get_entry_point().flags.get(ExecutionModeSampleInterlockUnorderedEXT))) {
        ...
        interlocked_is_complex = !handler.use_critical_section || handler.interlock_function_id != ir.default_entry_point;
    }
}
