...
namespace Math {
	...
	struct Vec {
		...
		Vec(const Vec<U, N>& other) {
			for (size_t i = 0; i < N; ++i) data[i] = static_cast<T>(other[i]);
		}
	...
	template <typename T, size_t N>
	inline T lengthSqr(const Vec<T, N>& v) {
		return dot(v, v);
	}
	template <typename T, size_t N>
	...
	template <typename T>
	inline Vec3 cross(const Vec<T, 3>& a, const Vec<T, 3>& b) {
		return Vec3(
			a.y() * b.z() - a.z() * b.y(),
			a.z() * b.x() - a.x() * b.z(),
			a.x() * b.y() - a.y() * b.x()
