...
namespace Axiom {
	...
	std::shared_ptr<Material> RendererSystem::testMaterial = nullptr;
	...
	void RendererSystem::init(Window* window) {
		...
		TextureSystem::init(context.get());
		MaterialSystem::init(context.get());
		testMaterial = MaterialSystem::createAndLoadMaterial("test");
	}
	...
	void RendererSystem::drawFrame() {
		if (context && context->beginFrame(0.0f)) {
			...
			GeometryRenderData contextData(Math::Mat4::rotateZ(Math::AX_QUARTER_PI));
			contextData.setMaterial(testMaterial);
