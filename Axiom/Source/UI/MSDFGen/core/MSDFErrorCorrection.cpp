...
#define CLASSIFIER_FLAG_ARTIFACT 0x02
    ...
    class BaseArtifactClassifier {
      public:
        ...
        inline int rangeTest(double at, double bt, double xt, float am, float bm, float xm) const {
            // For protected texels, only consider inversion artifacts (interpolated median has different sign than boundaries). For the rest, it is sufficient
            // that the interpolated median is outside its boundaries.
    ...
    static bool edgeBetweenTexelsChannel(const float* a, const float* b, int channel) {
        ...
        if (t > 0 && t < 1) {
            ...
            float c[3] = {mix(a[0], b[0], t), mix(a[1], b[1], t), mix(a[2], b[2], t)};
                ...
                *stencil(x, y) |=
                    (byte)(ERROR *
                           ((x > 0 && ((l = sdf(x - 1, y)), hasLinearArtifact(BaseArtifactClassifier(hSpan, protectedFlag), cm, c, l))) ||
                            ...
                            (x > 0 && y < sdf.height - 1 &&
                             hasDiagonalArtifact(BaseArtifactClassifier(dSpan, protectedFlag), cm, c, l, t, sdf(x - 1, y + 1))) ||
                            (x < sdf.width - 1 && y < sdf.height - 1 &&
                             hasDiagonalArtifact(BaseArtifactClassifier(dSpan, protectedFlag), cm, c, r, t, sdf(x + 1, y + 1)))));
            }
        }
    }
...
#pragma omp for
            ...
            for (int y = 0; y < sdf.height; ++y) {
                ...
                for (int col = 0; col < sdf.width; ++col, x += xDirection) {
                    ...
                    // Mark current texel c with the error flag if an artifact occurs when it's interpolated with any of its 8 neighbors.
                    *stencil(x, y) |=
                        (byte)(ERROR * ((x > 0 && ((l = sdf(x - 1, y)), hasLinearArtifact(shapeDistanceChecker.classifier(Vector2(-1, 0), hSpan), cm, c, l))) ||
                                        (y > 0 && ((b = sdf(x, y - 1)), hasLinearArtifact(shapeDistanceChecker.classifier(Vector2(0, -1), vSpan), cm, c, b))) ||
                                        (x < sdf.width - 1 &&
                                         ((r = sdf(x + 1, y)), hasLinearArtifact(shapeDistanceChecker.classifier(Vector2(+1, 0), hSpan), cm, c, r))) ||
                                        (y < sdf.height - 1 &&
                                         ((t = sdf(x, y + 1)), hasLinearArtifact(shapeDistanceChecker.classifier(Vector2(0, +1), vSpan), cm, c, t))) ||
                                        (x > 0 && y > 0 &&
                                         hasDiagonalArtifact(shapeDistanceChecker.classifier(Vector2(-1, -1), dSpan), cm, c, l, b, sdf(x - 1, y - 1))) ||
                                        (x < sdf.width - 1 && y > 0 &&
                                         hasDiagonalArtifact(shapeDistanceChecker.classifier(Vector2(+1, -1), dSpan), cm, c, r, b, sdf(x + 1, y - 1))) ||
                                        (x > 0 && y < sdf.height - 1 &&
                                         hasDiagonalArtifact(shapeDistanceChecker.classifier(Vector2(-1, +1), dSpan), cm, c, l, t, sdf(x - 1, y + 1))) ||
                                        (x < sdf.width - 1 && y < sdf.height - 1 &&
                                         hasDiagonalArtifact(shapeDistanceChecker.classifier(Vector2(+1, +1), dSpan), cm, c, r, t, sdf(x + 1, y + 1)))));
                }
