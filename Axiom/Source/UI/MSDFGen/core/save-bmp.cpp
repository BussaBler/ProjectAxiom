...
namespace msdfgen {
    ...
    static const byte BMP_LINEAR_COLOR_SPACE_SPECIFICATION[48] = {
        0xf8, 0xc2, 0x64, 0x1a, 0x08, 0x3d, 0x9b, 0x0d, 0x11, 0x36, 0x3c, 0x01, 0x1c, 0xeb, 0xe2, 0x16, 0x39, 0xd6, 0xc5, 0x2d, 0x09, 0xf9, 0xa0, 0x07,
    ...
    bool saveBmp(BitmapConstSection<byte, 3> bitmap, const char* filename) {
        ...
        for (int y = 0; y < bitmap.height; ++y) {
            for (int x = 0; x < bitmap.width; ++x) {
                byte bgr[3] = {bitmap(x, y)[2], bitmap(x, y)[1], bitmap(x, y)[0]};
    ...
    bool saveBmp(BitmapConstSection<byte, 4> bitmap, const char* filename) {
        ...
        for (int y = 0; y < bitmap.height; ++y) {
            for (int x = 0; x < bitmap.width; ++x) {
                byte bgra[4] = {bitmap(x, y)[2], bitmap(x, y)[1], bitmap(x, y)[0], bitmap(x, y)[3]};
    ...
    bool saveBmp(BitmapConstSection<float, 3> bitmap, const char* filename) {
        ...
        for (int y = 0; y < bitmap.height; ++y) {
            for (int x = 0; x < bitmap.width; ++x) {
                byte bgr[3] = {pixelFloatToByte(bitmap(x, y)[2]), pixelFloatToByte(bitmap(x, y)[1]), pixelFloatToByte(bitmap(x, y)[0])};
                fwrite(bgr, sizeof(byte), 3, file);
            }
            fwrite(padding, 1, padLength, file);
        }
    ...
    bool saveBmp(BitmapConstSection<float, 4> bitmap, const char* filename) {
        ...
        for (int y = 0; y < bitmap.height; ++y) {
            for (int x = 0; x < bitmap.width; ++x) {
                byte bgra[4] = {pixelFloatToByte(bitmap(x, y)[2]), pixelFloatToByte(bitmap(x, y)[1]), pixelFloatToByte(bitmap(x, y)[0]),
