...
namespace msdfgen {
        ...
        for (int l = 0, r = 1; r < sdf.width; ++l, ++r) {
            ...
            for (int i = 0; i < 3; ++i) {
                if (lv[i] != rv[i]) {
                    ...
                    if (lr >= 0 && lr <= 1) {
                        float v[3] = {mix(lv[0], rv[0], lr), mix(lv[1], rv[1], lr), mix(lv[2], rv[2], lr)};
    ...
    template <int N>
    double estimateSDFErrorInner(const BitmapConstSection<float, N> &sdf, const Shape &shape, const Projection &projection, int scanlinesPerRow,
                                 FillRule fillRule) {
