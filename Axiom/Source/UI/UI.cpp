...
namespace Axiom {
    ...
    UI::UI() {
		...
		std::unique_ptr<Buffer> stagingBuffer = Application::getRenderer()->createBuffer({ .size = asciiAtlas.width * asciiAtlas.height * asciiAtlas.channels, .usage = BufferUsage::TransferSrc, .memoryUsage = MemoryUsage::GPUandCPU });
    ...
    void UI::createBaseRenderObjects() {
        Buffer::CreateInfo vertexBufferCreateInfo = { .size = sizeof(UIVertex) * 4 * maxButtonQuads, .usage = BufferUsage::Vertex, .memoryUsage = MemoryUsage::GPUandCPU };
        baseVertexBuffer = Application::getRenderer()->createBuffer(vertexBufferCreateInfo);

        Buffer::CreateInfo indexBufferCreateInfo = { .size = sizeof(uint32_t) * 6 * maxButtonQuads, .usage = BufferUsage::Index | BufferUsage::TransferDst, .memoryUsage = MemoryUsage::GPUOnly };
        baseIndexBuffer = Application::getRenderer()->createBuffer(indexBufferCreateInfo);

        Buffer::CreateInfo stagingBufferCreateInfo = { .size = sizeof(uint32_t) * 6 * maxButtonQuads, .usage = BufferUsage::TransferSrc, .memoryUsage = MemoryUsage::GPUandCPU };
    ...
    void UI::createFontRenderObjects() {
        Buffer::CreateInfo vertexBufferCreateInfo = { .size = sizeof(UIVertex) * 4 * maxFontQuads, .usage = BufferUsage::Vertex, .memoryUsage = MemoryUsage::GPUandCPU };
        fontVertexBuffer = Application::getRenderer()->createBuffer(vertexBufferCreateInfo);

        Buffer::CreateInfo indexBufferCreateInfo = { .size = sizeof(uint32_t) * 6 * maxFontQuads, .usage = BufferUsage::Index | BufferUsage::TransferDst, .memoryUsage = MemoryUsage::GPUOnly };
        fontIndexBuffer = Application::getRenderer()->createBuffer(indexBufferCreateInfo);

        Buffer::CreateInfo stagingBufferCreateInfo = { .size = sizeof(uint32_t) * 6 * maxFontQuads, .usage = BufferUsage::TransferSrc, .memoryUsage = MemoryUsage::GPUandCPU };
        std::unique_ptr<Buffer> stagingBuffer = Application::getRenderer()->createBuffer(stagingBufferCreateInfo);
    ...
    bool UI::button(const std::string& label, Math::Vec2 pos, Math::Vec2 size) {
        ...
        bool isHovering =
            instance->mousePosition.x() >= pos.x() && instance->mousePosition.x() <= pos.x() + size.x() && instance->mousePosition.y() >= pos.y() && instance->mousePosition.y() <= pos.y() + size.y();
        if (isHovering) {
            instance->hotItem = id;
    ...
    void UI::text(const std::string& text, Math::Vec2 pos, Math::Vec4 color, uint16_t size) {
        ...
        for (char c : text) {
            ...
            instance->addFontQuad(
                Math::Vec2(minX, minY),
                Math::Vec2(maxX - minX, maxY - minY),
                metrics.uv0,
                metrics.uv1,
                color
            );
