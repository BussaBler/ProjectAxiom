...
namespace Axiom {
	...
	public:
		...
		void copyTo(Resource& destination, uint64_t srcOffset = 0, uint64_t dstOffset = 0, uint64_t size = VK_WHOLE_SIZE) override;
		void bind(uint64_t offset = 0);

		Vk::Buffer getHandle() const { return buffer; }

	private:
		static Vk::BufferUsageFlags getBufferUsage(uint32_t usage);
